## Update 2026-09-04 - Période Stripe autoritaire pour l'affichage
...
Pour une facture créée par `invoice.paid`, le webhook résout séparément la période historique depuis les Invoice Lines récurrentes de la Subscription. Il transmet ce résultat aux trois chemins `subscription_create|subscription_cycle|subscription_update`; la ligne de commande le persiste dans son nom et les PDF continuent de lire ce snapshot. Des périodes identiques sont admises, des périodes divergentes utilisent le fallback Cotton legacy journalisé, jamais la période courante de la Subscription. Si `invoice.lines` est tronqué, seules les lignes complètes de cette Invoice sont récupérées auprès de Stripe.

## Update 2026-09-04 - Cible Pro pour le futur changement d'offre
...
La V1 des clients en propre impose une hausse stricte de jauge sans réduction de durée de période. Contrat complet: `canon/interfaces/ecommerce-offer-change.md`. Les offres déléguées hors cadre restent exclues.

## Update 2026-09-03 - Renouvellement Stripe contractuel des délégations hors cadre

Le checkout d'une offre déléguée hors cadre persiste sur la subscription Stripe le dernier snapshot contractuel explicitement validé: `cotton_contract_reference`, assiette brute HT, offre client, offre catalogue, jauge et fréquence. Ce snapshot reste stable entre deux intentions commerciales explicites; un renewal le lit sans le recalculer depuis Cotton et sans imposer une référence `checkout_v1`.

`invoice.upcoming` est l'unique chemin de write du `SubscriptionItem`: assiette contractuelle + remise réseau courante = Price net, avec `proration_behavior=none` et no-op si le montant/intervalle est déjà aligné. `invoice.created` contrôle seulement; `invoice.paid` contrôle sans repricing puis crée/aligne la commande sur l'Invoice réellement payée.

Le guard exige l'offre active exacte, la subscription locale exacte, la délégation exacte et `commercial_status=offre_deleguee_hors_cadre`. Le support technique est résolu depuis le snapshot par lookup contractuel: Price actif, unique, devise/récurrence compatibles et Product explicitement actif. Son `unit_amount` n'entre jamais dans le calcul. Snapshot/support absent, ambigu ou incompatible échoue fermé; offres propres, déléguées cadre, schedules et subscriptions legacy restent hors chemin.

La configuration Stripe PROD n'est pas modifiée par le patch code. `invoice.upcoming` et `invoice.created` doivent être ajoutés manuellement seulement après déploiement et vérification du code.

## Etat 2026-07-27 - Pro EC: facturation des remises decimales

Le checkout Stripe ABN conserve le taux de remise exact et le transmet au coupon Stripe. L'incident de juillet 2026 venait du snapshot Cotton, pas du moteur Stripe ni du moteur de calcul facture.

Regle documentee:
- `ecommerce_offres_to_clients` est un snapshot contractuel de l'offre souscrite;
- `remise_pourcentage` doit rester decimal (`DECIMAL(5,2) UNSIGNED`) pour conserver des remises comme `28,61 %`;
- le runtime de facturation relit ce snapshot et calcule volontairement le TTC depuis le prix de reference HT et la remise exacte;
- ne pas modifier ce moteur pour repartir du HT net snapshoté sans audit metier, car avec le taux exact il produit le meme montant que Stripe sur le cas verifie.

Cas de validation incident:
- reference HT `69,90`;
- remise `28,61 %`;
- net HT snapshoté `49,90`;
- TTC runtime/Stripe `59,88`;
- TVA affichee `9,98`.

Fichiers de reference:
- checkout/remise Stripe: `pro/web/ec/modules/ecommerce/offres/ec_offres_script.php`;
- webhook facture ABN: `pro/web/ec/ec_webhook_stripe_handler.php`;
- moteur facture: `global/web/app/modules/ecommerce/app_ecommerce_functions.php`.
