...
## Dashboard mobile : choix explicite du Master individuel — 16/09/2026

Sous992px, le CTA officiel individuel d'un Dashboard Hub conserve le producteur `app_session_get_link(..., flag_direct_launcher=1)` et ajoute `master_view=individual`, puis le `return_url` historique : `/master/{id_securite}?master_view=individual&return_url=...`. Cette intention existante empêche le garde Games des anciens favoris de convertir le clic en Hub Master. Aucun `hub_launch`, nouvelle autorisation ou orchestration. Le header desktop conserve `/hub/{token}/master`. Hors Hub : URL inchangée ; démos : POST historique ; résultats : producteur terminal existant ; papier : surface et UI historiques conservées. Refresh Programme : même renderer et href. [Preuve Git et tests](../../../notes/dashboard-mobile-individual-master-2026-09-16.md). Correctif local non déployé.
