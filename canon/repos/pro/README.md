<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-pro-readme" owner="codex" -->
...
Home, Agenda/archives/réseau, first-party futur/aujourd’hui, dates occupées et candidats initiaux du Dashboard utilisent le prédicat Global partagé, avant comptage/pagination. Un programme Hub relu vide ne réaffiche pas l’ancienne liste. Les résultats historiques dédiés restent conservés.
...
Dashboard mobile sous992px : include DEV isolé hors cartes/refresh, CTA sticky caché jusqu'à SDK + API + appareil Cast disponible. Guard canonique strict `$conf['server'] === 'dev'`, y compris endpoint JS. Configuration opt-in `web/config.cast-dev.php` (template `tools/cast-dev/config.example.php`, clé `CAST_DEV_APPLICATION_ID`) ; ID absent = aucun CTA. Sélecteur, diagnostics/export JSON, timeout et arrêt dédiés ; aucune Remote/session/DB. Cartes historiques inchangées. **POC expérimental Cast DEV — non disponible en production.** [Architecture, fichiers, configuration manuelle, recette et rollback](../../../notes/cast-dev-poc-2026-09-17.md).


## Supports papier Pro — correction locale du 17/09/2026

La route `/extranet/start/game/{jeu}/print/{id_securite}` accepte désormais `_`, comme les identifiants `scheduleplan_…` créés par le planificateur. Les identifiants historiques et la destination PHP restent inchangés. La 404 observée en PROD pour la session 30455 était due à la regex Pro, avant génération PDF ; le même support était servi par le Hub Games. Patch DEV sur `hub_soiree`, non déployé ; aucun changement de droits, données ou générateur PDF.



## Dashboard Hub mobile canonique — 18/09/2026, patch local non déployé

Complément UI local du 19/09 : CTA mobile sticky, libellé serveur Préparer/Lancer/Reprendre selon la projection existante ; terminal inchangé. Libellé mobile strictement masqué desktop, composition Diffusion/Remote préservée. [Tests et recette](../../../notes/hub-mobile-ui-2026-09-19.md).

Remplace la décision du 16/09 sur le Master individuel. Sous `<992px`, le CTA d’état mobile ouvre `/hub/{hub_token}/master` dans le même onglet après le contrôle d’accès existant. Aucune action individuelle historique (lancement, résultats, session_duplicate) n’est générée pour le Programme Hub ; démos canoniques depuis le Hub. Hors Hub inchangé. Desktop conserve accès Master/Remote et nouvel onglet. Resize ferme les dialogs en cours ; le refresh conserve le routage canonique. Aucun mode mobile persisté. [Tests, preuves et recette](../../../notes/hub-master-responsive-2026-09-18.md).


### Observer les retraits Hub — 16/09/2026, local non déployé

Le Dashboard conserve la suppression historique des sessions jamais démarrées. Aucun accès à la suppression des sessions démarrées n’est ajouté. Le poll de révision de 12 s reste actif après cutoff pour observer les retraits Master/Remote et reconstruire le programme sans reload manuel. Les gardes d’édition locale existants peuvent différer le refresh jusqu’à fermeture de l’édition.

Le programme canonique actif remplace aussi une liste devenue vide. La résolution Global reconnaît l’historique explicitement retiré pour ne pas le réinjecter comme programme legacy ; le conteneur vide reste rendu et rafraîchissable après le dernier retrait. Les règles temporelles/commerciales de quick-add restent inchangées.
...
Le bandeau V&B et sa condition ID92 sont supprimés : plus de promesse de doublement de jauge dans ce parcours ; jauge sélectionnée conservée, catalogue10 et historique préservés. La création du coupon réseau legacy5% (affiliation1774 / siège1294) est supprimée pour les nouvelles commandes. Le step1 refuse tout contexte délégué présent mais invalide ; le step2 vérifie propriétaire, état pending et absence de subscription avant Stripe. Une pending legacy marquée Coupon Stripe/PERCENTOFF, ou propre portant un pourcentage sans règle, est refusée avec message. Pas de migration implicite. Le reset2026 précède la préparation du Price de base ; les échecs de reset/coupon/snapshot interrompent le checkout. Le détail qualifie le propriétaire exact, pas le spectateur. Au signup, seule la classification canonique de règle décide du routage2026/legacy.
...
<!-- AUTO-UPDATE:END id="remises-affilies-20260910-pro-readme" -->

<!-- AUTO-UPDATE:BEGIN id="hub-empty-805-canon-repos-pro-readme" owner="codex" -->

## Reprogrammation après suppression de la dernière partie — 09/09/2026

Home/Agenda generated et Bibliothèque fixed utilisent la réconciliation centrale SchedulePlan d’un Hub actif vide admissible. Le probe Bibliothèque et le rendu Agenda restent inchangés : aucun affichage de soirée vide, aucun choix supplémentaire. La prochaine session fait réapparaître la même soirée. Le quick-add Dashboard conserve son chemin distinct à Hub source explicite.

Après session_init Bibliothèque, une preuve côté session PHP identifie le brouillon créé. Un refus définitif du commit le compense seulement s’il est encore non configuré, sans produit et sans membership ; retour Bibliothèque pour une nouvelle sélection. Busy/conflit conserve la tentative en cours ; préexistant sans preuve ou déjà configuré jamais supprimé comme brouillon. Pas de purge rétrospective. [Preuves, routes et validation](../../../notes/hub-empty-reconciliation-2026-09-09.md). Local hub_soiree, non déployé.

<!-- AUTO-UPDATE:END id="hub-empty-805-canon-repos-pro-readme" -->

## Correctif local 2026-09-09 — Quick Add : Hub source autoritaire

Sur Pro `hub_soiree`, la modale Dashboard `dashboard_quick_session_create` transmet désormais `id_hub`. Le contrôleur exige cet identifiant, recharge le Hub et refuse explicitement une source absente, mal formée, inexistante, hors compte, inactive, en suppression ou de date/contexte invalide. Aucune résolution compte/date, aucun pivot événement et aucun `get_or_create` ne sont appelés dans cette branche. La date utilisée par le service et l'URL de retour vient du Hub revalidé ; son contexte et son opération sont conservés. Le programme servant aux majorités mode/format est chargé exclusivement depuis ses memberships actifs.

`start_dashboard_quick_program_context_get(client, date, client_detail, source_hub_id = null)` conserve une branche générique seulement lorsque le quatrième argument est absent/null. L'action Dashboard ne peut pas atteindre ce fallback : un POST sans `id_hub` lui transmet une valeur invalide et échoue. La sémantique générique est corrigée : `id_solution_usage=2` signifie gamification/événement, pas dynamisation/soirée.

Ce Quick Add appelle toujours `app_programming_quick_hub_create_from_dashboard_preparation`, puis le service partagé `create_from_game` / `create_from_proposal` ; il n'est pas un appel au commit commun `app_schedule_plan_commit`. La source de membership `schedule_plan` ne signifie pas que ce commit a été appelé. L'idempotence et les contrôles de rattachement du service restent inchangés. Home/Agenda `generated_program`, Bibliothèque `fixed_content` et récurrence gardent leurs entrypoints et le commit commun.

Audit complémentaire : Master poste `quick_session_create` sur sa route contenant le token Hub, résolu en Hub canonique ; Remote poste directement sur `hub_remote` avec son `remoteToken` (ou utilise sa session liée au Hub). Les deux handlers appellent le service Global avec ce Hub déjà résolu, sans résolution générique et sans redirection vers Quick Schedule. Ils sont `SAFE_EXPLICIT_HUB` ; Dashboard était `VULNERABLE_TO_SAME_BUG` et devient `SAFE_EXPLICIT_HUB`. Le garde `ambiguous_existing_hubs` reste inchangé.

Validation locale : `ec_start_dashboard_quick_source_hub_test.php`, 38 contrôles sur fixtures isolées sans DB, dont les handlers réels Master/Remote, le service de création Global et le resolver de paramètres ; authentification, catalogue et persistance sont des doubles de test. Suites Dashboard, Quick Schedule UI, Bibliothèque, Global Quick Hub et comportement SchedulePlan vertes. Deux assertions statiques préexistantes de `schedule_plan_commit_contract_test.php` restent en échec, reproduites avec les sources HEAD avant patch. Aucune recette serveur/navigateur, aucun déploiement. Voir [diagnostic et vérifications opérateur](../../../notes/hub-quick-add-source-2026-09-09.md).

## Correctif local 09/09/2026 — Parité Agenda DEV/PROD

Correctif CSS complémentaire du 09/09, non déployé : le lien de commande `.ec-discovery a` conserve son état normal. Ses états `:hover` et `:focus-visible` imposent le texte blanc sur fond `#31065a`, sans ombre ni translation ; le focus garde son contour de 2 px. La règle globale `ec_custom.css` `a:hover {color:#240445!important}` égalait la spécificité de `.ec-discovery a` et arrivait après. Le sélecteur local d'état augmente seulement cette spécificité ; `!important` est requis pour dépasser la déclaration globale importante. Aucun changement aux autres liens ni aux fichiers de parité préexistants.

Sur `hub_soiree`, la carte Agenda utilise quick-schedule `source=agenda` pour les événements comme les soirées, y compris CSO, dans tous les environnements. La tête de réseau reste hors self-service et les URL explicites gardent leur priorité. Le formulaire date transmet `quick_schedule_entry=1` partout ; le serveur conserve les validations de date avant la redirection. Sans ce marqueur, le traitement historique est conservé. Trois fichiers à livrer ensemble, aucun nouveau endpoint ni changement DB. Correctif non déployé ; [audit, tests et recette restante](../../../notes/hub-dev-prod-parity-2026-09-09.md).
...
## Update 2026-09-07 — Dashboard : découverte, Lots et compteurs live (patch local)
...
Sticky : `position:sticky; top:.75rem; z-index:30`, marge basse 1,25rem, conteneur teinté opaque. Le sélecteur correct est `.event-dashboard--v2 .container-fluid > .event-dashboard__header-actions` : le wrapper `.mx-3.mx-lg-5.mt-3.mt-lg-4` interdit de supposer que `.container-fluid` est enfant direct du Dashboard. Cette erreur de sélecteur empêchait auparavant toute application du sticky; ce n’était pas un défaut de z-index. La chaîne de parents reproduite localement n’expose ni overflow capturant, ni transform/filter/contain bloquant. Aucun fixed ni gestionnaire de scroll ajouté. Les hooks de refresh retrouvent le même groupe; header Hub, Lots, Programme, découverte et mobile ne changent pas.

Vérification : Chromium local, CSS réels et DOM de parents reconstitué avec les contrôles rendus depuis la vue PHP, contenu de Programme représentatif. Avant correction : position calculée static et disparition au scroll; après : sticky, top 12 px pour des scrolls de 550/1100/1600 px sur 992/1024/1100/1280/1920, retour en haut sans décalage et focus Programme non masqué dans la fixture. Mobile 360/991 : commandes masquées. Ce test navigateur n’est pas une connexion à la page DEV authentifiée; confirmation utilisateur finale sur DEV reste nécessaire.

Pour `access_state === inactive`, le Dashboard remplace le bloc commercial des Actions par un bandeau `Mode découverte`, avant la fiche/identité Hub, visible seulement sous `992px`. Le lien reprend `url_ecommerce`; le label reprend `tarifs_commande_cta_label` (`Essai gratuit` devient `Activer mon essai gratuit`, sinon `Je m'abonne` / `Je commande`). Aucun nouveau choix commercial. Les comptes actifs ne rendent pas ce bandeau. Après recette, le bandeau partage directement la classe `ec-discovery` de la navigation : fond jaune `#ffedbf`, texte brun, CTA sombre arrondi et mêmes états hover/focus; seules sa disposition compacte et sa visibilité mobile sont spécifiques. Cette règle remplace les descriptions historiques du bloc `Mode démo` dans Actions.

Le CTA prospect conserve son comportement et affiche : `Découvre l’écran diffusé aux joueurs et teste le déroulé de ta soirée en démo.` (ou `de ton événement`). Les cartes Master/Remote et leur visibilité mobile restent inchangées.

Les Lots expliquent leur présentation aux joueurs et le recours aux intitulés génériques. Une seule action `Modifier les lots`, sombre et indépendante du titre, est alignée à droite du titre et de l’aide sur desktop, sans ligne supplémentaire, et après les champs sous `992px` (même bouton réordonné en CSS). Elle ouvre les trois champs du formulaire inline existant; sauvegarde, annulation, focus et lecture seule conservent leurs contrats. Elle remplace les crayons par cellule décrits dans les anciennes entrées.

Le polling Pro `dashboard_program_revision_check` transporte désormais `participation_html`, rendu par le même helper de présentation que le premier affichage. Il appelle le compteur canonique `app_games_hub_participation_counters_get($hub)` sans changer sa définition. Le client transmet uniquement son état temporel d'affichage et remplace le contenu de `[data-dashboard-participation]` si différent, avant la comparaison de révision : même sans changement de préparation et pendant une édition, les compteurs convergent sans reconstruire les cartes. Cadence 12 s, protections de concurrence, suspension et visibilité préservées; aucun timer supplémentaire. Le champ existant `sql_queries` reste celui des révisions, pas une mesure du coût total ajouté du helper de participation; coût réel serveur non mesuré.

Navigation : pour `offre_effective_state === inactive`, `pro/web/ec/ec.php` entoure le CTA commercial d’un cartouche jaune compact `Mode découverte` et du seul lien sombre. Le label essai est `Activer mon essai gratuit`; les autres labels et l’URL sont repris tels quels. L’ancien chemin de navigation reste inchangé pour les autres états. L’utilisateur a précisé que le cartouche de la capture était à créer autour du CTA local existant; aucun rechargement serveur n’était donc nécessaire.

Tous comptes : `Personnaliser le visuel` remplace le libellé `Design` du CTA du visuel, avec le même guard, crayon et ouverture de modale. Largeur bornée au cadre, retour à la ligne autorisé. Le lien `Page publique` n’est temporairement plus rendu par le Dashboard; son footer n’est rendu que si l’action d’information subsiste. URLs, données, helpers et interfaces publiques www préservés.
...
Complément UX local du 2026-09-07 : dans la liste personnelle seulement (`type=mine`), le contexte existant `$is_session_agenda_context` affiche sous les filtres et avant le compteur un encart `alert alert-info small`, indépendant du nombre de résultats. Recherche globale et vues réseau sont exclues. Le titre distingue playlist personnelle et série de questions; le texte indique de créer depuis « Les jeux », dans la bibliothèque du jeu (Cotton Quiz explicitement), puis de revenir sélectionner le contenu en remplacement. Le wording réutilise `ec_start_day_wording_get`, avec la même projection que le Dashboard : `public_place` → « ta soirée », autres contextes → « ton événement ». Aucun CTA/lien de création; bandeau de remplacement, retour Hub, accès, éligibilité et application conservés. Aucun encart en bibliothèque générale, ajout Hub, Cotton ou Communauté. Le parcours annoncé correspond à la carte personnelle `Ajouter une playlist/série` → `clib_theme_editor_url_get('t_theme_create', ...)`; voir aussi « Etat 2026-06-05 - First_party: contenus personnels et edition themes ». Recette visuelle DEV restante, aucun déploiement.
...
Le Dashboard résout le Hub existant depuis tous les memberships du programme via `app_games_hub_existing_for_program(...)`, puis lit son programme persistant. Une relation manquante ne déclenche ni création ni réparation au GET : le programme historique complet est conservé. Les créations métier écrivent leurs memberships explicitement avant succès.
