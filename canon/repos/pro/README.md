...
- Routes clés:
  ...
  - `/extranet/start/game/choose/{id_offre}` -> choix jeu client (`flag_session_demo=0`)
  - `/extranet/start/game/setting/{id_securite_session}`
...
- Bridge bibliothèque:
  ...
  - après POST `session_setting` (si `from=library`): application directe de `session_theme`, puis redirection `/extranet/start/game/resume/{id_securite_session}` ; Quiz multi-séries transporté via `quiz_lot_ids`
...
- Contrat MVP:
  ...
  - statut POC data:
    - `preset=now` (`A la une`, onglets `Cotton` et `Communauté`):
      ...
      - onglet `Cotton`: priorité aux entrées “à la une” historiques, complété si besoin (`<12`) par les plus populaires
  ...
  - métas popularité (list + fiche):
    ...
    - source sessions:
      ...
      - Quiz: incrémentation par série via `id_produit` et tokens `L<id>` dans `lot_ids` (sessions multi-séries)
  ...
  - verrou contenu perso “used by session” (pas de versioning/archivage/clone):
    ...
    - détection backend (références réelles de programmation):
      - Quiz: `championnats_sessions.id_produit` + `championnats_sessions.lot_ids` (tokens `L<id>`) sur `id_type_produit IN (1,5)`
    ...
    - exception admin bibliothèque:
      - client admin `id_client=10` peut modifier/supprimer/éditer le contenu des thématiques communauté (sources `id_client_auteur>0`)
  ...
  - itération UX `view` (2026-02-13):
    ...
    - extraits multimédias courts:
      - audio: bouton `Extrait 10s` (YouTube audio-only ou fichier audio) avec arrêt auto
