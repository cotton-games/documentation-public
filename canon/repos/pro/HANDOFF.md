...
## Update 2026-09-02 — Stabilisation du commit Quick Schedule

- Passe logs post-déploiement: les tentatives de 17:59 étaient toutes refusées avant écriture par `PLAN_SESSION_INVALID`. Cause: les constructeurs generated simple/récurrent copiaient `entry.heure` (`19h00`) dans le champ canonique `SchedulePlan.time` (`HH:MM`). Ils utilisent désormais `entry.time`, avec assertion inter-repos dédiée.
- Passe Bibliothèque à 18:14: les erreurs `SESSION_ORDER_MISMATCH` venaient de la représentation `HHhMM` toujours écrite par le configurateur fixed; la postcondition compare maintenant des minutes normalisées. Pour un Hub général existant, le champ horaire désactivé n'est plus attendu dans le POST: le serveur recalcule l'horaire suivant depuis les sessions du Hub. Le scope `$conf` est également rétabli dans le writer generated appelé par include.
- Passe Bibliothèque à 18:24–18:29: les ajouts existants et une création aboutissent, mais la source de membership `library_hub_add` prouve que l'include `session_setting -> session_theme` perdait le marqueur `SchedulePlan`; une création a aussi échoué sur l'ordre. Le writer fixed maintient maintenant un marqueur global strictement scoped, propage le marqueur au POST imbriqué et empêche toute seconde résolution d'heure ou adhésion legacy. Le commit central reste seul producteur du membership `schedule_plan`.
- Cause reproduite à 16:48:55 pour le client `2316`: trois sessions `generated_program` ont été créées avant `app_games_hub_get_or_create_for_context(...)`; la clé du Hub inactif `265` ne pouvait pas être libérée car `games_hubs_publication` subsistait. `inactive_context_conflict` laissait le postcheck sans Hub actif et produisait « Les parties sont créées, mais le Hub n’est pas cohérent… ».
- Architecture livrée: `SchedulePlan` commun, Hub fixé avant writer, memberships explicites, postconditions complètes, compensation MyISAM et replay du résultat persisté. Les sessions generated ont une identité déterministe; une interruption `writing_plan` devient reprenable après 30 secondes sur le Hub mémorisé. Home/Agenda, fixed général/explicite et récurrence appellent ce commit; le quick-add n'utilise plus `reconcile` et compense ses échecs.
- Limite de preuve: la connexion DB locale dev refuse l'utilisateur configuré. Les logs prouvent le Hub inactif `265`, le blocage publication et la création annoncée de trois sessions, mais ne donnent pas leurs IDs ni l'état SQL exact de l'opération.
- À recetter: Agenda 3 Blind Test sur `2026-09-03` après suppression, double clic/replay, fixed nouveau, fixed Hub exact, choix multi-Hub et échecs forcés. Vérifier 1 Hub actif, N sessions complètes et exactement N memberships actifs vers ce Hub.

## Update 2026-09-02 — Quick Schedule Bibliothèque générale et Hub explicite

- `fixed_content` couvre désormais les deux entrées Bibliothèque: programmation générale et ajout depuis un Dashboard Hub explicite; Home/Agenda restent en `generated_program`.
- Le faux positif du jour venait du résolveur qui considérait toute ligne Hub active à la date, même vide ou temporaire. Un Hub n'est maintenant proposé comme existant que s'il possède une session membre active et complètement configurée.
- En général, une date occupée reste sélectionnable avec un curseur cohérent; les dates `generated_program` restent désactivées. Pour un Hub unique, le bloc Nom reste visible avec le libellé autoritatif en lecture seule; en ambiguïté, il reprend le Hub choisi, tandis qu'une création restaure la saisie. La légende Date disparaît, Programme et récapitulatif affichent `Jeu · Thème`, le réglage fixe devient `Durée de la partie` avec le même espacement supérieur que le mode de participation, et un seul message contextuel précède le CTA.
- Depuis un Hub explicite, l'écran compact ne redemande ni nom, date, récurrence, jeu, quantité ou horaire. Le serveur revalide ownership et contexte, reprend la date Hub, calcule le prochain horaire depuis le Programme et ignore les valeurs forgées.
- Le write path reste `session_setting -> session_theme -> Hub/membership`; annulation, retour et succès reviennent au même Dashboard sans dépendre du référent HTTP. Le renderer settings n'est conservé que pour ses parcours legacy/compatibles et les opérations distinctes.
- Contrats ajoutés ou renforcés: Hub vide/temporaire/configuré, date occupée, états publics du probe, rendu compact explicite, libellés/récapitulatif, valeur Hub imposée, idempotence et routage Bibliothèque/Quiz Builder.
- Vérifications locales automatisées vertes; recette navigateur authentifiée générale/Hub explicite sur Blind Test, Bingo et Quiz Builder reste à faire.

## Update 2026-08-31 — Mode démo Hub

- Le parcours Agenda -> Dashboard -> Hub est ouvert aux comptes clients hors tête de réseau; ownership et règles temporelles restent inchangés.
- La projection commerciale vient de `app_ecommerce_offre_effective_get_context(...)`: l'inactivité affiche le bloc `Mode démo`, dont l’URL et la décision sont directement reprises du menu Pro (`$url_ecommerce`, `$tarifs_commande_cta_label`). Le Dashboard adapte seulement le wording: essai gratuit, abonnement ou commande; l'autorisation de tester dépend séparément de la chronologie.
- Desktop/mobile proposent la démo avant fenêtre pour tous, la démo pendant fenêtre seulement sans offre active, et aucune démo après fenêtre; les résultats terminés restent prioritaires.
- `session_duplicate` est un adaptateur du producteur Global canonique source -> démo.
- Tests Pro ciblés passés; recette authentifiée active/network/inactive et tête de réseau recommandée.

## Update 2026-08-30 — Dashboard canonique: renderer V1 supprimé

- `/extranet/start/games/day/YYYY-MM-DD` et l'alias `/dashboard` appellent directement l'unique renderer Dashboard; `?ui=v1|v2` n'est plus lu.
- Suppression du helper/contrat de version, de `EC_START_DAY_DASHBOARD_PROTOTYPE`, des branches et du suffixe markup V1 du contrôleur jour, des champs de version des modales et des compatibilités de rollback.
- Les quatre refreshs partiels valident désormais `[data-dashboard-shell][data-dashboard-renderer="canonical"]`; Master/Remote, Programme, Lots, quick-add, Format/Mode, Bibliothèque, modales, résultats et mobile conservent leurs contrats.
- Les noms historiques `ec_start_sessions_day_dashboard_v2_style.php`, `.event-dashboard--v2`, `--event-dashboard-v2-*` et `session-details-grid--v2` restent provisoirement nominaux pour limiter le risque visuel; ils ne sélectionnent aucun renderer.
- Tests/lints Pro ciblés et recherche finale V1/V2 exécutés; recette navigateur authentifiée recommandée.
- Journal AI Studio inaccessible (`HTTP 403`), dérogation utilisateur explicite, aucun rechargement serveur complémentaire.

## Update 2026-08-27 — Dashboard Hub: faux positif `Actualiser le programme` corrigé

- Cause: la baseline initiale était bien issue du SSR, mais les succès locaux n'utilisaient pas tous le même acquittement. Format/Mode remplaçait le Programme via une fonction locale sans émettre la révision; Lots et Personnalisation ne renvoyaient aucune révision. Le contrôle suivant voyait donc la mutation locale comme externe, notamment lors de l'ouverture de la modale Remote.
- Correction: ajout d'un contrat de succès `preparation_revision` pour Personnalisation, Lots, thématique et Format/Mode; acquittement navigateur partagé et répétable; quick-add/refresh carte continuent de relire la révision depuis le HTML canonique.
- Conflit réel: une révision différente sans acquittement local n'est plus rafraîchie silencieusement. Le composant existant affiche `Des changements ont été faits depuis un autre écran.` et `Afficher les changements`, puis le refresh partiel acquitte la révision affichée.
- Invariants: aucun bypass pendant la modale Remote, aucun changement du polling Remote, de `dashboard_master_access_check`, des modes `none|optional|required`, de l'ouverture nouvel onglet ou du calcul Global de révision.
- Code: `ec_start_script.php`, `ec_start_sessions_day_event_script.php`, `ec_start_sessions_day_dashboard_view.php`, `ec_start_sessions_day_dashboard_test.php`.
- Vérifications: lints PHP, tests Dashboard/Programme/paramètres rapides/renouvellement/quick-schedule et `git diff --check` verts. Recette navigateur multi-écrans restante.

## Update 2026-08-27 — Dashboard Hub: CTA Télécommande autonome restauré

- Cause auditée: le bloc autonome avait été retiré dans le diff local de stabilisation, sans commit dédié; le CTA Master était alors le seul accès Remote visible.
- Actions réalisées: restauration sélective depuis `HEAD` du CTA secondaire historique, de sa visibilité éditable/read-only, du service Global de token, de l'URL `/hub/{remote_token}/remote`, de la modale Bootstrap QR, des styles responsive et de l'état indisponible.
- Isolation conservée: le CTA autonome n'appelle pas `dashboard_master_access_check`, ne lit pas la présence Remote et ne lance aucun polling. Le parcours Master garde sa décision fraîche `none|optional|required`, son contrôle final `required`, son polling borné à la modale visible et son ouverture exclusive en nouvel onglet.
- Code touché: `pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_view.php`, `pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`.
- Docs touchées: `canon/repos/pro/README.md`, `TASKS.md`, `HANDOFF.md`, `CHANGELOG.md`.
- Vérifications: lints PHP, test Dashboard complet, suites Programme/paramètres rapides/renouvellement/quick-schedule et `git diff --check`. Les logs Pro rechargés montrent les GET Dashboard et POST `/extranet/start/script` en `200`, sans erreur fonctionnelle Dashboard/Remote; les lignes `games_hub` visibles sont informatives et les autres erreurs récentes correspondent à des sondes externes refusées.
- Aucun commit, déploiement, migration DB ou modification Games/Global effectué.

## Update 2026-07-29 — Dashboard Pro Hub: bilan chargé après clôture Hub
...
- Actions réalisées:
  - diagnostic: le Dashboard Hub chargeait encore le bilan legacy dès qu'une partie du jour était terminée, avant clôture du Hub;
  - déplacement du calcul `app_games_hub_temporal_state(...)` avant le bilan afin de réutiliser l'état canonique du Hub;
  - ajout de `$day_bilan_can_load`: le bilan reste disponible pour l'ancien écran, mais le Dashboard Hub attend `launch_window_open=false`;
  - suppression de l'instrumentation temporaire Pro/Global ajoutée pour l'audit.
- Limite connue:
  - les informations de bilan ne sont volontairement pas hydratées sur le Dashboard Hub tant que la fenêtre Hub reste ouverte.
- Vérifications:
  - `php -l` sur les fichiers Pro/Global touchés;
  - `php web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`.
- Recette restante:
  - client 10, Hub ouvert avec au moins une partie terminée, vérifier le chargement sans bilan; après cutoff Hub, vérifier le retour des informations de bilan.

Date: 2026-02-22  
Sujet: Agenda “Nouvelle session” — mode quick multi-dates + auto-thématique bibliothèque.

## Update 2026-07-28 — EC: menu legacy `Mes événements` retiré

- Cause auditée: le menu EC était rendu dans `web/ec/ec.php` après une requête directe sur `operations_evenements`, conditionnée par des événements non démo récents/à venir et `!$client_is_gamification_usage`. Un compte repassé en dynamisation pouvait donc revoir `Mes événements` si d'anciennes lignes legacy existaient encore.
- Règle canonique: côté client EC courant, l'ancien module `operations/evenements` n'est plus un point d'entrée. Home, Agenda et Communauté doivent pointer vers l'Agenda Hub `/extranet/start/games` et le dashboard journée Hub.
- Actions réalisées: suppression du bloc menu et de sa requête; redirect des pages utilisateur legacy `list`, `view`, `form` vers `/extranet/start/games`; maintien des routes rewrite, modules legacy, scripts, widgets, tables et usages support/BO.
- Home: la branche active gamification garde `ec_widget_client_lieu_sessions_agenda.php`; la référence commentée au widget `ec_widget_operation_evenement_agenda.php` a été retirée pour éviter une réactivation ambiguë.
- Vérifications: `php -l web/ec/ec.php`, `php -l web/ec/modules/communication/home/ec_home_index.php`, `php web/ec/ec_legacy_events_navigation_test.php`, `php web/ec/modules/tunnel/start/ec_start_sessions_list_agenda_test.php`.
- Limite: recette navigateur authentifiée non exécutée localement.

## Update 2026-07-28 — Hub Pro: wording visible `session(s)` -> `partie(s)`

- Scope traité:
  - widgets Home actifs du parcours Hub: `ec_widget_home_first_party_onboarding.php`, `ec_widget_jeux_discover_library.php`, `ec_widget_client_lieu_sessions_agenda.php`, `ec_widget_client_network_affiliate_home.php`, plus les compteurs réseau visibles de `ec_home_index.php`;
  - Agenda Hub: liste, journée/dashboard, création rapide, settings, résumé, modales et helpers directs;
  - Communauté/Bibliothèque/Joueurs: écrans et messages visibles liés à la sélection/remplacement de contenus, classements et statistiques joueurs.
- Hors scope volontaire: tunnels historiques inutilisés, anciennes pages de programmation non Hub, BO/admin, facturation, onboarding historique, Global/Games et identifiants techniques `session`.
- Actions réalisées:
  - remplacement des libellés visibles `session(s)` par `partie(s)`;
  - messages d'expiration de page reformulés sans employer `partie`;
  - tests source mis à jour pour les nouvelles chaînes attendues.
- Vérifications:
  - lints PHP ciblés sur les fichiers modifiés;
  - `php web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`;
  - `php web/ec/modules/tunnel/start/ec_start_quick_schedule_ui_test.php`;
  - `php web/ec/modules/tunnel/start/ec_start_sessions_list_agenda_test.php`;
  - `php web/ec/modules/tunnel/start/ec_start_library_hub_add_flow_test.php`;
  - `git -C /home/romain/Cotton/pro diff --check`.
- Recette restante:
  - navigateur authentifié Home Hub, Agenda, Dashboard journée, ajout depuis Bibliothèque et Communauté/Joueurs.

## Update 2026-07-26 — Dashboard Programme: renouvellement garde les exclusions
...
- Diagnostic:
  - l'adaptateur Pro et Global acceptaient déjà `temporary_exclusions`;
  - Games garde la carte Hub Master en place après renouvellement, donc son état de carte survit;
  - Pro remplace `.event-dashboard__program` après succès, ce qui détruisait l'état `WeakMap` attaché à l'ancien élément et vidait les exclusions au clic suivant.
- Actions réalisées:
  - état de renouvellement conservé par `data-session-id` dans la durée de vie de la page;
  - réattachement automatique de cet état aux cartes recréées par le refresh Programme;
  - contrat inchangé: exclusions bornées à 24, alimentées par `previous_selection` serveur, envoyées au plan Global, sans stockage persistant navigateur.
- Vérifications:
...
- Recette restante:
  - navigateur authentifié: dashboard Hub avec une carte renouvelable, cliquer `↻` plusieurs fois et vérifier que les propositions ne reviennent pas immédiatement aux thématiques déjà remplacées malgré le refresh du Programme.
...
- Scope code:
  ...
  - `pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`
- Actions réalisées:
  - conservation du CTA Hub Master en nouvel onglet et absence de modification du Hub;
  - ajout d'une URL canonique `data-program-refresh-url` sur le shell dashboard;
  - ajout d'un refresh focus idempotent qui écoute `visibilitychange`, `pageshow` et `focus`;
  - relecture serveur de `/extranet/start/games/day/{date}` puis remplacement ciblé de `.event-dashboard__program`;
  - mise à jour de la méta header dépendante du nombre de parties;
  - déduplication des événements rapprochés, une seule requête active, séquence de réponse et `AbortController`;
  - garde local contre remplacement pendant édition des lots, Format/Mode, modale personnalisation, ajout rapide, renouvellement de thématique ou requête occupée;
  - notice discrète `Actualiser le programme` quand le refresh automatique est bloqué par un état local.
- Vérifications:
  - lints PHP dashboard/test;
  - `php /home/romain/Cotton/pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`;
  - extraction JS inline + `node --check`;
  - `git diff --check`.
- Recette restante:
  - navigateur authentifié: retour depuis Hub après ajout/renouvellement/lancement/suspension/fin, cas bfcache, retour avec édition locale ouverte, et bouton manuel de refresh.

## Update 2026-07-23 — Bibliothèque Hub: ajout depuis Programme corrigé en recette navigateur

- Addendum Bibliothèque générale:
  - settings reproduit quick-schedule mono-partie après sélection d'un contenu;
  - `library_schedule_context_probe` résout dynamiquement tous les Hubs actifs/accessibles pour la date sans création, sans filtre par contexte préféré `event`/`soiree`;
  - aucun Hub => champ `À partir de` requis, CTA `Créer la soirée` / `Créer l'événement`;
  - Hub unique => heure masquée/non requise, CTA `Ajouter au programme`, POST sans heure accepté et horaire recalculé;
  - Hubs multiples => radios obligatoires sans présélection, POST refusé sans choix compatible;
  - le POST rerésout toujours côté serveur et ne fait pas confiance à `id_hub`, à l'heure cachée, au libellé de contexte ni au `return_url`.
- UI settings Bibliothèque:
  - suppression du header historique pour les routes settings Bibliothèque;
  - shell quick-schedule en trois colonnes `Quand ?`, `Quel programme ?`, confirmation;
  - même rendu pour création depuis thématique, ajout à un Hub détecté depuis settings et ajout à un Hub explicite depuis dashboard;
  - état initial neutre avant choix de date, puis contexte création/ajout après probe serveur;
  - dates déjà programmées marquées dans le calendrier sans désactivation;
  - Cotton Quiz conserve le nombre de séries choisi et ne rend pas le sélecteur Format;
  - carte confirmation préremplie côté serveur, avec nom du jeu à la place du libellé technique d'horaire recalculé;
  - le formulaire garde les champs POST historiques pour ne pas modifier le write path.
- Scope code:
  - `pro/web/ec/modules/tunnel/start/ec_start_sessions_day.php`
  - `pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_view.php`
...
- Diagnostic recette:
  - le parcours `Dashboard Hub -> Ajouter une partie -> Bibliothèque -> contenu -> settings` pouvait arriver à settings sans `id_hub`;
  - la vue contenu ne relisait pas le contexte Hub depuis `$_GET`, et Quiz Builder repartait sur une URL catalogue générique.
- Actions réalisées:
  - `id_hub` explicite conservé dans CTA dashboard, liste, cartes, filtres, pagination, détails, sélection directe et Quiz Builder;
  - le script Bibliothèque force le tunnel agenda uniquement depuis un `id_hub` explicite transporté;
  - settings active le mode Hub seulement après validation serveur du Hub actif/propriétaire;
  - en mode Hub, date/heure ne sont pas rendues, ne sont pas obligatoires, le bloc `Programmation` vide disparaît et le CTA devient `Ajouter au programme`;
  - le POST Hub ignore toute date/heure navigateur, reprend la date Hub, recalcule l'heure depuis le Programme réel, rattache la session et revient au dashboard du même Hub;
  - la Bibliothèque générale garde date/heure visibles et obligatoires, puis crée/récupère le Hub canonique avant retour dashboard.
- Vérifications:
  - lints PHP ciblés;
  - tous les tests PHP `pro/web/ec/modules/tunnel/start/*_test.php`;
  - `git diff --check`.
- Recette restante:
  - navigateur authentifié final sur Blind Test, Bingo Musical, Cotton Quiz et Quiz Builder, avec POST date/heure falsifié en mode Hub et validation hors Hub.

## Update 2026-07-23 — Dashboard Programme: détail intégré et conversions format/version
...
- Actions réalisées:
  - `Voir le détail` ouvre un panneau inline dans la carte Programme; une seule carte reste ouverte;
  - le panneau affiche format/version persistés, micro-formulaires inline, liens détail bibliothèque par série/playlist, `Lancer une démo` via `session_duplicate` et réduction;
  - ajout de `dashboard_session_options_update` dans le script Start, JSON + CSRF dashboard + compte Pro courant;
  - conversion Quiz basée sur `lot_ids`: `id_format=5` -> 2 séries, `id_format=2` -> 4 séries, recettes canoniques papier/numérique, cas 100% `L` sans injection `T/N`;
  - fallback contrôlé et journalisé pour compositions mixtes non canoniques; pas d'inférence arbitraire par préfixe seul pour les recettes reconnues;
  - garde serveur contre sessions verrouillées, terminées, démarrées, avec résultats ou joueurs runtime;
  - Bingo régénère le playlist client sur changement format ou version pour maintenir supports papier/smartphone cohérents;
  - liens bibliothèque en contexte `agenda/library/session` avec retour dashboard soirée/événement et libellé de retour contextualisé.
- Vérifications:
  - `php -l` sur les fichiers PHP modifiés;
  - `php /home/romain/Cotton/pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`;
  - `php /home/romain/Cotton/pro/web/ec/modules/tunnel/start/ec_start_dashboard_program_helpers_test.php`.
- Recette restante:
  - navigateur authentifié dashboard futur, Quiz papier/numérique 2↔4, compositions 100% catalogue, refus session démarrée/terminée, retour bibliothèque, démo, Bingo papier/smartphone.

## Update 2026-07-23 — Dashboard personnalisation: actions design locales et footer compte
...
- Scope code:
  ...
  - `pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`
- Actions réalisées:
  - remplacement de `Revenir au design par défaut` par `Annuler les changements`;
  - ajout de `Revenir au design Cotton` seulement si la cascade héritée contient une couche réseau ou compte;
  - déplacement de l'action dans `Identité et design`, après les champs de couleurs;
  - résolution serveur du design par défaut via la cascade sans couche Hub courante (`app_general_branding_get_detail_merged(0, 0, réseau, compte, array())` + fallback Cotton Hub);
  - actions strictement locales au clic, avec restauration couleurs, inputs fichier et aperçus logo/visuel;
  - suppression Hub différée au submit `Enregistrer` uniquement pour l'intention `inherit`, avec confirmation contextualisée et suppression limitée au branding Hub;
  - modification manuelle du design après reset héritage basculée en intention `inherit_custom`, donc sauvegarde Hub explicite au lieu d'une suppression;
  - intention `cotton` enrichie d'intentions média explicites pour logo/visuel, appliquées via `app_general_branding_visuel_uploader(...)` sans dépendre des inputs fichier vides;
  - sources logo/visuel Cotton Hub centralisées dans `ec_start_day_personalization_cotton_logo_*` et `ec_start_day_personalization_cotton_visual_*`, remplaçables plus tard par des assets dédiés;
  - footer réduit à une case toujours décochée, contextualisée `prochaines soirées` ou `prochains événements`, placée dans le même groupe que `Annuler` / `Enregistrer`;
  - retrait de l'action de suppression du design compte depuis cette modale.
- Invariants:
  - sauvegarde Hub type 5, copie compte type 4 depuis la ligne Hub finale persistée, traitement uploads, publication Hub, lieu, sessions, Hub Master/Play et routes publiques inchangés;
  - source de contexte soirée/événement côté serveur: `clients.id_solution_usage` via `ec_start_day_personalization_context_get(...)`.
- Vérifications:
  - `php -l` sur les fichiers PHP modifiés;
  - `php /home/romain/Cotton/pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`;
  - extraction JS inline + `node --check`;
  - `php /home/romain/Cotton/global/web/tests/hub_operation_branding_cascade_test.php`;
  - `php /home/romain/Cotton/global/web/tests/hub_delete_service_contract_test.php`;
  - `git -C /home/romain/Cotton/pro diff --check`.
- Recette restante:
  - navigateur authentifié: clics actions sans persistance avant `Enregistrer`, fermeture/annulation sans suppression, submit reset Hub persisté avec confirmation, retour réseau/compte/Cotton réel, modification manuelle après reset, case cochée/non cochée, réouverture modale et responsive.

## Update 2026-07-21 — Dashboard soirée/événement: finition modale et colonne droite
...
- Actions réalisées:
  - modale de personnalisation:
    - section `Identité et design` limitée au nom, visuel, logo et couleurs;
    - section `Informations publiques` avec accroche/description directement visibles;
    - Soirée: résumé + formulaire du lieu dans la même section UX, mais lecture/écriture exclusivement sur les champs natifs du compte;
    - Événement: ordre direct accroche, description, lieu, adresse, lien, sans sous-titres intermédiaires ni `Libellé du lien`;
    - conseil médias réduit, `text-muted`, italique, sans cadre ni fond.
  - dashboard desktop:
    - CTA opérationnels placés en haut de la colonne droite;
    - Programme placé sous ces CTA, limité à l'espace disponible;
    - hauteur globale de carte déterminée par la colonne gauche (`summary` + Lots, padding/espacements inclus);
    - liste de sessions scrollable sur débordement réel, avec `Ajouter une partie` visible sous la liste;
    - CTA principal `Diffuser aux joueurs` en fond plein sur l'accent branding avec texte contrasté calculé; Remote conservée en carte légère teintée.
- Vérifications:
  - `php -l` sur les fichiers PHP modifiés;
  - `php /home/romain/Cotton/pro/web/ec/modules/tunnel/start/ec_start_sessions_day_dashboard_test.php`;
  - extraction JS inline + `node --check`;
...
- Recette restante:
  - navigateur authentifié soirée/événement, 1/3/4+ parties, scroll Programme, ajout visible, sauvegarde modale lieu/événement, contrastes CTA sur couleurs réelles.

## Update 2026-07-02 — Bibliotheque Quiz papier: serie unique preservee
- Scope code:
  - `pro/web/ec/modules/tunnel/start/ec_start_script.php`
- Diagnostic:
  - depuis la bibliotheque, un Quiz papier V2 avec une seule serie catalogue arrivait dans `start_quiz_v2_apply_lots_to_session(...)` sous forme `L{id}`;
  - ce cas etait confondu avec l'initialisation papier automatique standard, qui appelle `qz_build_paper_auto_lot_ids_csv(...)` et remplace la composition par 3 lots temporaires `T` + la serie selectionnee.
- Action realisee:
  - ajout d'un garde-fou borne a `from=library`, Quiz V2 papier, une seule serie catalogue `L{id}`;
  - la composition reste `lot_ids=L{id}`, sans auto-completion `T,T,T,L`;
  - ajout d'un log `QUIZ_PAPER_LIBRARY_SINGLE_SERIES` via `start_info_log(...)`.
- Invariants:
  - programmation Quiz papier automatique multi-series conservee hors flux bibliotheque serie unique;
  - Quiz numerique, quick numerique, Blind Test, Bingo et first_party inchanges.
- Verification:
  - `php -l` OK sur le fichier PHP touche.
...
- Vérifications:
  - `php -l /home/romain/Cotton/pro/web/ec/modules/communication/home/ec_home_index.php` OK
## Pro: Bibliothèque Hub remplacement avec Hub explicite — 2026-07-23

- Scope traité: remplacement de contenu depuis le dashboard Hub via Bibliothèque, avec conservation explicite de `id_hub` jusqu'au POST final.
- Diagnostic: le retour Hub documenté précédemment n'était pas assez robuste car le write path pouvait encore retomber sur la fiche session via `session_theme`.
- Livré: propagation `id_hub` Programme/Bibliothèque/Quiz Builder/formulaires; validation serveur Hub actif + compte + relation active `games_hubs_sessions`; retour canonique dashboard construit depuis le Hub validé et prioritaire sur `game/view`/`resume`.
- Tests: lints PHP ciblés, test dashboard, test helpers Programme, test quick-schedule UI, tests Global quick/recurrent/idempotence et Hub delete OK.
- Reste: recette authentifiée sur Blind Test, Bingo Musical, Cotton Quiz et contrôle de la redirection HTTP finale.
# 2026-09-02 — Dashboard Hub: régression de date déplacée

- Le split était réel et déterministe: quick-add consommait `app_games_hub_temporal_state(hub_date)`, tandis que CTA global et cartes consommaient `app_session_get_chronology(session.date)` sur les memberships persistés. `app_games_hub_sessions_get(...)` signale les dates divergentes mais retourne volontairement ces memberships, et `app_games_hub_sessions_reconcile(...)` les considère autoritaires.
- Le correctif centralise la chronologie d'un membre sur la date du Hub dans `ec_start_day_dashboard_session_chronology_get(...)`; CTA, Programme, accès individuel et Remote partagent cette valeur. Aucun statut DB, membership, focus ou résultat n'est muté.
- À compléter en recette avec le SQL d'audit fourni: relever le Hub reproduit, ses memberships actifs, les dates/heures sessions, leurs phases modernes et événements de lancement. Cette preuve distinguera la ligne historique incohérente du défaut de projection désormais neutralisé. Aucune réparation manuelle de données avant attribution du write path.
- Validation locale: huit scripts `web/ec/modules/tunnel/start/*_test.php`, lints PHP ciblés et `git diff --check`, tous verts. Recette authentifiée/DB non disponible localement.
# 2026-09-02 — Preuve finale déplacement/suppression Hub

- Les logs rechargés prouvent une mutation partielle, pas un cache: Pro commit les dates sessions, Global retourne `source_missing` car le contexte était redéduit en `event` au lieu du Hub `soiree`, puis Pro annonçait `date_move=ok`.
- Le flux transporte désormais l'id Hub exact de la modale jusqu'à Global, valide son compte/date/activité, utilise ses memberships autoritatifs et conserve son type de contexte. Un Hub actif cible bloque avant toute écriture; un échec Global n'est plus annoncé comme succès.
- La suppression de `263` était correcte pour ses trois membres. Les cinq sessions de `265`, déplacées en date mais pas en ownership, ne pouvaient pas être supprimées avec lui. Les GET du 02/09 ont ensuite créé les Hubs vides `270..272`; le contrôleur est maintenant read-only sur date vide ou sessions déjà membres.
- État recette restant après logs: `265` possède encore des membres datés du 02/09 avec une racine au 03/09; `272` peut encore être la racine vide active du 02/09. Ne pas réparer en SQL. Après livraison, supprimer `272` via le service canonique s'il existe encore, puis déplacer `265` du 03/09 au 02/09 via le Dashboard pour exercer le chemin de récupération testé.
