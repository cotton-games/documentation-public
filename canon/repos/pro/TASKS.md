...
## REPORTÉ SUR MAIN 2026-09-04 - Période de facturation Stripe
- [x] Reporter uniquement le commit `periode` sur `main` (`f5af072`) après déploiement Global vert.
- [x] Synchroniser la période sur `customer.subscription.created/updated` et `invoice.paid` via le résolveur partagé Basil/legacy.
- [x] Afficher la période stockée sans modifier prix, fréquence, logique commerciale ni contrat renewal TdR.
- [x] Résoudre la période historique depuis les Invoice Lines et la transmettre aux trois créations de commande.
- [x] Valider les lints PHP, le contrat période, le contrat renewal TdR et `git diff --check` avant push.
- [ ] Redélivrer un événement PROD choisi et contrôler les bornes, Pro et les fallbacks selon la recette guidée.

## FUTUR 2026-09-04 - Parcours Pro de changement d'offre
...
## REPORTÉ SUR MAIN 2026-09-03 - Renewal Stripe contractuel hors cadre
- [x] Reporter directement depuis `renew_hors_cadre` le webhook, le snapshot checkout et le test Pro, sans merge ni fichier Hub/DEV hors périmètre.
- [x] Vérifier le guard exact, `invoice.upcoming` seul write, `invoice.created`/`invoice.paid` sans repricing, logs structurés, fail-closed et no-op.
- [x] Vérifier la parité fonctionnelle avec DEV, les lints PHP, le test contractuel et `git diff --check`.
- [ ] Déployer le code PROD et le vérifier avant toute modification de destination Stripe.
- [ ] Ajouter ensuite manuellement `invoice.upcoming` et `invoice.created` à la destination PROD; traiter les trois subscriptions legacy séparément.

## DOC 2026-07-27 - Pro EC: remises decimales Stripe/Cotton
- [x] Documentation livree:
  - consigner que Stripe conserve le taux exact de remise au checkout;
  - consigner que l'incident facture venait du snapshot Cotton `tinyint`, pas du coupon Stripe ni du runtime;
  - rappeler que les factures ABN standard creees via `invoice.paid` sont correctes si le snapshot conserve `28,61`.
- [x] Validation documentee:
  - reference `69,90 HT`, remise `28,61 %`, net `49,90`, TTC `59,88`;
  - aucune modification runtime requise.
- [ ] Suivi hors documentation:
  - garder le controle Stripe/Cotton sur les futures remises decimales en recette serveur.
