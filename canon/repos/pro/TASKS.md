<!-- AUTO-UPDATE:BEGIN id="audit-activation-684-20260910-pro" owner="codex" -->
...
## À PLANIFIER — Diffusion Smart TV et viewport CSS — 17/09/2026

- [x] Périmètre retenu pour la mise en production en préparation : patch QR joueurs Hub Master conservé ; aucun patch QR Remote et breakpoint992px inchangé. Aucun déploiement réalisé dans cette passe.
- [ ] Chantier transverse diffusion Pro/Games : prendre en compte les navigateurs Smart TV dont le viewport CSS est inférieur à992px, ce qui sélectionne le parcours mobile malgré un grand écran physique. Distinguer ce choix responsive d’un QR absent pour cause JavaScript/CDN.
- [ ] Recetter sur appareils réels le viewport CSS, le zoom, le bandeau Diffusion/Télécommande, les accès individuels mobiles et les accès de secours à la Remote ; décider ensuite du comportement adapté, sans présupposer un changement de breakpoint.
- Aucun incident Remote démontré : l’audit était préventif. Le signalement client concernait uniquement le QR joueurs Hub Master.

[Décision et contexte dans le Handoff](../../../HANDOFF.md#décision-de-périmètre--diffusion-et-navigateurs-smart-tv).


## Supports papier Pro — 17/09/2026, patch DEV non déployé

- [x] Audit main/logs : Pro 12:09:25 HTTP200 avec identifiant historique, 12:10:01 HTTP404 avec `scheduleplan_…` ; Games 12:15:17 HTTP200 pour session30455/Hub185. Regex Pro excluant `_`.
- [x] `pro/web/.htaccess:230` : ajout de `_` à la classe autorisée, destination et autres routes conservées ; fins de ligne CRLF conservées.
- [x] Vérification PCRE PHP sur la règle réelle : 21 cas, trois jeux, identifiants historiques/planifiés/tirets et refus de chemins avec slash, points ou slash encodé. Diff vérifié avec `core.whitespace=cr-at-eol`.
- [ ] Après diffusion : ouvrir les supports Quiz/Blind Test historiques et planifiés depuis Pro. Aucune DB réelle, aucun déploiement/rechargement ou commit effectué. Rollback : retirer uniquement `_` de cette règle (réintroduit la 404 des identifiants planifiés).



## CORRIGÉ LOCALEMENT — Dashboard Hub mobile canonique — 18/09/2026
...
- [x] Remplace le routage individuel du 16/09 : CTA principal vers Hub Master, même onglet sur mobile, contrôles d’accès conservés, actions historiques retirées du Programme Hub.
- [x] 154 contrôles routage, renderer/refresh et décisions d’accès 991 ↔ 992, suite Dashboard verts.
- [ ] Recette navigateur soirée/événement, actif/prospect, offre absente, fenêtres, démos et résultats.

[Preuves, commandes et rollback](../../../notes/hub-master-responsive-2026-09-18.md).
...
<!-- AUTO-UPDATE:END id="audit-activation-684-20260910-pro" -->

<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-pro-tasks" owner="codex" -->

## CORRIGÉ LOCAL / À RECETTER — Remises2026 affiliés — 10/09/2026

- [x] Appliquer le contrat d’acquisition propre aux surfaces pro, conformément aux décisions métier ; source locale déclarée à jour par l’opérateur pour DEV.
...
<!-- AUTO-UPDATE:END id="remises-affilies-20260910-pro-tasks" -->

<!-- AUTO-UPDATE:BEGIN id="hub-empty-805-canon-repos-pro-tasks" owner="codex" -->

## Cas805 / Hub148 — patch local programmation, 09/09/2026

- [x] Home/Agenda/fixed passent par la réconciliation commune ; aucune UX de Hub vide ajoutée.
- [x] Enregistrer la propriété du brouillon Bibliothèque ; compensation bornée, idempotente, sous verrou ; protéger les sessions préexistantes/configurées.
- [x] Suites source Hub (38 contrôles), Bibliothèque, Quick Schedule UI, Dashboard et paramètres vertes ; syntaxe et diff vérifiés.
- [ ] Recette DEV des cycles ajout/suppression/reprogrammation, conflits et compensation ; récupération d’une configuration suivie d’un échec distincte d’une purge de brouillon. [Rapport](../../../notes/hub-empty-reconciliation-2026-09-09.md).

<!-- AUTO-UPDATE:END id="hub-empty-805-canon-repos-pro-tasks" -->

## VALIDÉ LOCALEMENT 2026-09-09 — Quick Add Dashboard conserve le Hub source

- [x] Diagnostic confirmé par les SELECT opérateur : client1726 usage2 ; Hub132 `event/620` à14:35, Hub142 `soiree/0` et session30230 créés par Quick Add à16:57:31 ; ambiguïté dès le GET suivant.
- [x] Comparer les fichiers Pro concernés sur main/hub_soiree : même défaut ; modifier le code uniquement sur hub_soiree. Transmettre/revalider le Hub source, conserver date/contexte/opération et programme des majorités ; refuser une source invalide sans fallback. Corriger la sémantique générique usage2.
- [x] Auditer Master et Remote séparément : identité canonique portée par leurs tokens, ajout direct via Global, aucun get_or_create. Les handlers réels sont inclus dans la nouvelle suite locale.
- [x] 38 contrôles source Hub verts ; suites Dashboard, Quick Schedule UI, Bibliothèque, Quick Hub et SchedulePlan comportement vertes ; syntaxe et diff contrôlés. Deux échecs statiques SchedulePlan reproduits avant patch, sans changement de ce test.
- [x] Documenter le contrat et les lectures opérateur ; documentation sur develop autorisée explicitement, travaux préexistants préservés ; index générés.
- [ ] Recette authentifiée DEV puis livraison séparée ; aucun accès serveur ou déploiement effectué ici.
- [ ] Réparation PROD132/142/30230 par l'opérateur après contrôle des références ; aucune écriture SQL préparée. [Détail](../../../notes/hub-quick-add-source-2026-09-09.md).

## Diagnostic PROD — CTA Agenda événement / CSO — 2026-09-09

- [x] Nouvelles copies des logs PROD09/09 examinées : à11:54:00, POST `/extranet/start/agenda/mode/script`302 puis GET `/extranet/start/games/day/2026-09-09`200 ; quick-schedule répond200 à11:54:14 et11:54:41. Cela corrobore l’ancien parcours de la modale, sans preuve d’identité de l’utilisateur au seul access_log. Dernier fatal Pro observé10:43:43 (fonction ec_programming_quick_schedule_url_get indisponible), cohérent avec le transfert partiel désormais résolu ; autre ancien fatal10:10:51 pour dashboard_program_helpers.php manquant. Les deux fichiers figurent au manifeste106/108. Global : aucune erreur fatale dans cette copie.
- Le diagnostic `missing_memberships` à11:54:00 pour client1402/date09/09 contient une liste vide de sessions manquantes ; il ne prouve pas un oubli de backfill. Le helper peut produire ce message lorsqu’aucune session ni Hub actif n’est trouvé pour la date. Aucun complément DB à en déduire automatiquement.

- [x] Parcours ciblé examiné, sans modification applicative : widget `ec_widget_jeux_sessions_cta.php` lignes25–28 garde la modale de création événement en PROD et la désactive seulement en DEV ; lignes59–62 remplacent alors l’URL quick-schedule par une ancre/modale. Le helper de blocage commercial n’exclut pas CSO (seule tête de réseau hors self-service).
- [x] Fichiers widget, agenda, helper, quick_schedule et rewrite présents au manifeste final (ordres137/141/106/118/147), blobs main égaux aux SHA archivés et aux fichiers de travail Hub. Aucun script omis établi ; comparaison avec les octets réellement présents sur serveur non effectuée.
- [x] Utilisateur confirme la carte : le comportement signalé correspond à la branche PROD du widget. L’en-tête Ajouter normal utilise déjà quick-schedule.
- [x] Correctif du 09/09 sur `hub_soiree` : carte événement, marqueur du formulaire Agenda et traitement `sessions_day_create` alignés DEV/PROD. Trois fichiers runtime, test dédié 40 contrôles, suites Quick Schedule/Dashboard et lint verts. [Audit de parité](../../../notes/hub-dev-prod-parity-2026-09-09.md).
- [ ] Recette authentifiée du correctif puis intégration/livraison opérateur ; aucun merge/push/déploiement effectué. Remote Bingo conserve un écart distinct à traiter dans Games.

## Migration du code Hub vers main — 2026-09-09

- [x] [Audit branches/code et manifeste candidat](../../../notes/hub-prod-code-migration-2026-09-09.md) : Merge bloqué : deux blocs whitespace conflictuels checkout ; auto-merge Bibliothèque préserve le retrait du filtre/pagination main, donc ne pas copier le fichier Hub brut. 48 fichiers candidats, deux sources finales à reconstruire ; 13/14 suites vertes sur Hub uniquement.
...
- Reprise09/09 : [vérification AI Studio fraîche](../../../tmp/hub-prod-code-2026-09-09/REPRISE-AI-STUDIO.md) et refs contrôlées. Pro déjà fusionné/validé selon utilisateur, main=Hub=a7b596570d99c64ecb768f9b84339e712146f7a4 ; comparaison auth/contacts serveur avant écrasement, aucun nouveau merge. Les contrôles précédents restent historiques.

## Audit dépendances schéma Hub PROD — 2026-09-08

- [x] [Inventaire partagé](../../../tmp/hub-prod-initial-schema-2026-09-08/README.md) : programmation, communauté, catalogue et chemin clib_list_get vers ensures validation/communauté identifiés.
- [x] PRECHECK analysés : deux tables Quick Schedule absentes, retenues comme prérequis associés au paquet final de 13 créations InnoDB ; aucune modification legacy/Pro.
- [ ] Livraison du code distincte : ensures Bibliothèque encore opportunistes, garantie zéro émission sur ces parcours non revendiquée. Import schéma par opérateur puis POSTCHECK avant toute phase suivante.
...
## UX 2026-09-07 — Aide à la création personnelle pendant un remplacement Hub

- [x] Encart secondaire dans `ec_bibliotheque_list.php`, sous les filtres et avant les résultats, uniquement remplacement/personnel, même liste vide. Textes playlist/Quiz et resolver soirée/événement; création, sélection, navigation et règles métier préservées.
- [x] Sources publiques parcourues depuis START/SITEMAP; journal AI Studio consulté avant patch (aucune modification bibliothèque Pro signalée). Parcours « Les jeux » → bibliothèque personnelle → éditeur confirmé dans le code et la doc.
- [x] Syntaxe PHP, diff check, suites `ec_start_library_hub_add_flow_test.php` et `ec_start_sessions_day_dashboard_test.php` OK; sonde temporaire de rendu du fragment réel : 126 cas (3 jeux × 3 contextes × vide/non vide × 7 parcours) OK, textes exacts et aucun CTA.
- [ ] Recette DEV desktop/mobile : remplacement personnel sur Blind Test, Bingo et Quiz; listes vides/non vides; soirée/événement; retours à la ligne et hiérarchie secondaire. Vérifier absence dans Cotton/Communauté, bibliothèque générale et ajout Hub; sélectionner/appliquer un remplacement puis vérifier le retour au même Hub, ainsi que le lien du bandeau existant. Aucun accès DEV/PROD/DB ni déploiement effectué.

## 2026-09-07 — Passe UX Pro découverte / Lots / compteur Programme

- [x] Correctif DEV 09/09 du CTA de commande découverte : spécificité locale hover/focus-visible supérieure à `a:hover!important`, blanc sur fond sombre, contour clavier conservé. État normal inchangé ; quatre fichiers Pro préexistants vérifiés par empreintes SHA-256.
- [x] Chromium local avec CSS réels chargés dans leur ordre : défaut avant patch reproduit, état normal et lien voisin strictement identiques après patch, hover/focus blanc sur sombre, pas de débordement/ombre, contour clavier 2 px. Fixture DOM locale, pas de session DEV authentifiée. Artefacts `/tmp/hub-dev-fixes/css-check.cjs`, `css-results.json`, `pro-hover.png`.
- [ ] Confirmer sur DEV authentifié le hover et le focus clavier du CTA de commande.

- [x] Points 2–6 : ancien bloc Actions supprimé, bandeau mobile avant identité Hub, texte prospect contextualisé, aide Lots et édition unique. Contrats de lancement et persistance conservés.
- [x] Barre finale : Bandeau sticky desktop >=992 : composition icône/label/aide/flèche et ratio canonique conservés, surtitres masqués. Surface du bandeau dérivée du texte contrasté de Diffusion avec 4 % de secondaire : claire si Diffusion sombre, sombre si Diffusion claire. Remote dispose de sa propre surface neutre teintée à 16 %, encre calculée sur cette surface et bordure contrastée autonome. Rayon/hauteur identiques entre CTA. Diffusion reste sur primaire pleine; hover mélange 8 % de son encre contrastée, Remote renforce sa bordure. Aucune ombre ni translation, focus intérieur currentColor; gap/sticky/espacement conservés. Calculs de présentation uniquement, wordings, routes, guards et mobile inchangés. Programme : compteur absent lorsque le critère canonique dashboard_commercial_demo_mode (access_state inactive) est vrai; aucune ligne vide si la capacité est également absente. Comptes actifs : renderer et polling 12 s inchangés; le JS ne met à jour que le conteneur présent. CTA Ajouter une partie déplacé après la liste et hors de sa condition non-vide, ancien order:-1 retiré. Ordre métier des sessions, permissions, action et responsive conservés.
- [x] Chromium local : reproduction avant (position static), après top=12 px à scroll 550/1100/1600 sur 992/1024/1100/1280/1920; retour en haut, focus et largeur sans empiétement sidebar dans la fixture. Sous 992, masque inchangé. Logs rechargés examinés; aucune preuve de style navigateur dans ces logs.
- [ ] Confirmation finale du scroll sur Dashboard DEV authentifié par l’utilisateur; la recette Chromium locale emploie DOM/CSS réels et contenu représentatif sans accès DB/session DEV.
- [x] Lisibilité sticky desktop : fond opaque légèrement teinté par branding, marge basse 20 px, aides/surtitres/icônes contrastés et séparation centrale renforcée; pistes de texte bornées et gap réduit. Ratio d’origine 2:1 rétabli en faveur du Hub Master, bouton Remote sous texte si sa zone devient étroite. Wordings strictement conservés, CSS/test seuls côté Pro. Tests Dashboard et lints/diff check OK; validation navigateur des cinq largeurs restante.
- [x] Variante sticky remplaçant les cartes non retenues : groupe unique sous Retour/avant identité, sticky top .75rem z30, surface unique de 80 px minimum; boutons explicites via cibles natives existantes, deux moitiés horizontales, bouton sous texte entre 992–1199. Aucun clone/scroll JS; mobile masqué comme avant; ancien emplacement sans rangée vide. Deux suites Dashboard et sonde JS vertes; lints/diff check OK. Recette navigateur 992/1024/1100/1280/large, focus et recouvrements restante.
- [x] Variante claire des deux CTA desktop : surfaces blanches, boutons plein/contour dérivés du branding, libellés existants réordonnés en CSS; boutons sous le texte de 992 à 1399,98 et à droite au-delà. Un seul lien/bouton natif par carte, aucune modification HTML/JS. Deux suites Dashboard, lints PHP et diff check verts. Recette visuelle réelle 992/1024/1100/grand desktop et variantes branding restante faute de navigateur.
- [x] Complément desktop cartes opérationnelles : surface/bordure/flèche Diffusion renforcées via tokens branding, Télécommande secondaire avec `Piloter depuis mon mobile` et aide QR. Bouton Lots aligné à droite du titre/aide sans rangée dédiée. CSS limité à >=992; mobile et comportement fonctionnel préservés. Deux suites PHP Dashboard, sonde JS existante, lints/diff check verts; validation visuelle réelle non disponible.
- [x] Retour recette navigateur : action Lots sortie du titre, contraste explicite, avant les champs desktop/après les champs mobile; bandeau Dashboard utilisant la classe visuelle commune `ec-discovery`. Deux suites PHP et sonde JS existantes repassées, lints/diff check OK. Nouvelle validation visuelle utilisateur restante.
- [x] Point 7 : compteurs dans le polling existant, renderer partagé SSR/JSON, remplacement ciblé même si révision identique; pas de nouveau timer ni de remplacement des cartes pour une arrivée de joueur.
- [x] Journal AI Studio consulté, audit des chemins ciblés : aucune entrée signalant ces fichiers Dashboard. Copie locale Pro et documentation initialement propres.
- [x] Tests : cinq suites PHP existantes, sept lints PHP, exécution du vrai JS polling et édition avec doubles DOM locaux, `git diff --check`.
- [x] Point 1 : cartouche créé autour du CTA existant dans `pro/web/ec/ec.php`, conformément à la clarification utilisateur; condition inactive, labels/URL canoniques, comptes actifs inchangés.
- [x] Points 8–9 tous comptes : CTA `Personnaliser le visuel`, masquage UI du lien `Page publique`, footer vide évité; aucune route publique modifiée.
- [ ] Recette visuelle 360/430/768/991/992/desktop large et connexion réelle joueur : non exécutée (aucun navigateur/DEV/DB disponible). Vérification CSS locale : bandeau sous 992, hors grille identité, liens repliables et lots empilés sous 576.
- Docs : README Pro, TASKS Pro, HANDOFF, CHANGELOG; index/sitemap via générateur. Aucun déploiement, aucun autre repo applicatif modifié.
