<!-- AUTO-UPDATE:BEGIN id="hub-suspend-20260910-quiz-readme" owner="codex" -->
...
`registerPlayer` utilise la preuve ciblée avec exécution officielle fiable, hors papier ; contrôle lifecycle après await. Capacité historique conservée en contexte incomplet, papier/démo/autonome. Course bootstrap Quiz du chantier complet non traitée. [Fichiers, dépendances, tests et limites](../../../notes/hub-soiree-hotfix-2026-09-23.md). Les sections readiness ci-dessous décrivent la branche `hub_session_readiness`, pas ce hotfix.


## 23/09/2026 — Préparation Hub E1

**Branche `hub_session_readiness` : NON DÉPLOYÉE EN PROD, exclue du hotfix `hub_soiree`.** Les recettes DEV et tests locaux ci-dessous ne valent pas livraison PROD. Preuve liée au bind/E1/incarnation, coordinateur de départ commun, admission WS ciblée, cohortes capacité et instrumentation. [Contrat, reprise sélective, fichiers, validations et limites](../../../notes/hub-session-readiness-2026-09-23.md). Les descriptions sas_players antérieures ne constituent pas le contrat de lancement de cette branche.

**Complément DEV du 23/09 :** l’opérateur a livré/testé readiness et rechargé Quiz, sans déploiement préalable des branches `sas_players`. La correction locale ultérieure rétablit le callback d’expiration E1 dans `connection.js` ; `runtimeExpiry.js`, déjà versionné mais absent DEV selon opérateur, doit être livré avec lui. Le [lot explicite de livraison](../../../notes/hub-session-readiness-2026-09-23.md#livraison-depuis-un-dev-sans-sas_players) inclut les modules capacité et diagnostics importés, même sans charge. 61/61 tests readiness/grâce après correction, aucune livraison ni restart exécuté par l’agent. Le statut « patch local » précédent décrit la préparation initiale, pas une preuve de l’état actuel du serveur.
...
### 22/09/2026 — Harnais WS A/B et récupération de la baseline DEV

**Complément opérateur et snapshot DEV : [état figé](../../../notes/quiz-dev-envutils-baseline-2026-09-22.md).** envUtils est exactement l’AVANT instrumenté, sans cohortes ; autres fichiers du lot identiques aux non commités locaux selon opérateur, processus redémarré. Marker /03 selon cette attestation, /04 seulement préparé dans le pack. Base du futur harnais : préserver ce transport direct et les hooks, livrer le métier performance séparément. Les réserves historiques ci-dessous décrivent la précédente passe.

**Incident DEV signalé après copie des fichiers complets non commités de `sas_players`.** Ce mode de copie embarque l’import performance `capacity_reads` de `envUtils.js` sans le module. Panne reproduite ; utiliser la version AVANT exportée et le pack de récupération décrit dans le [rapport](../../../notes/ws-loadtest-ab-instrumentation-2026-09-22.md#incident-quiz-dev-après-copie-des-fichiers-non-commités). Marker récupération /04 préparé hors worktree. Déploiement partiel déclaré ; logs suivants : écoute3032 rétablie à15:27:05 Paris,50 joueurs Hub connectés à15:31. Version exacte déployée non attestée ; aucun résumé instrumenté pour ce chemin Hub navigateur. Aucun déploiement PROD de ce lot.
...
Bind papier idempotent sur participation persistée, réconciliation active au bootstrap/reconnexion et toutes les5s en papier vivant ; scores existants conservés. Seule la publication réconciliée couvrante peut supprimer par absence. Marker21/09 /03 ; aucun restart exécuté.
...
`actions/hubLifecycle.js` et `hubSuspension.js` pilotent Pause/gel et reprise explicite. Les closes suspendus détachent les sockets sans désactivation DB ni grâce. Le snapshot vivant de progression prime sur le preload.
...
<!-- AUTO-UPDATE:END id="hub-suspend-20260910-quiz-readme" -->
...
- Le serveur fige et persiste classement/podium avant `hub_session_natural_ended`. Double clic et retry sont coalescés par `finalizing_scores`; un write échoué restaure la revue et n'émet ni podium ni signal Hub.
- La persistance finale force `gameStatus=Partie terminée` pendant la transaction papier; le statut précédent est restauré en mémoire si le write terminal ou la completion Hub échoue. Cela garantit que Canvas reconnaît les résultats comme terminés avant de valider l'exécution Hub.
- `completed` et `HUB_SESSION_FINISHED` ne suivent qu'un terminal persisté et, en Hub, une completion confirmée. Hors Hub, le payload `endGame` historique reste diffusé.
...
- Pour une démo lancée depuis le Hub, le WS diffuse d'abord le payload `endGame`, puis `HUB_SESSION_FINISHED` marqué `runtimeMode=demo`; le résultat reste visible 2,5 s avant retour. Une démo Dashboard Pro mobile sans exécution Hub garde exclusivement le terminal historique. Marker: `restart 01-09-2026/01`.
...
- Le primary organizer Quiz reste reference par `session.primarySocket` / `session.primaryInstanceId`.
- `sendMessageToPrimary()` recupere maintenant une socket organizer ouverte deja connue de la session si la reference primary est absente ou fermee, puis logge `PRIMARY_ORGANIZER_RECOVERED`.
- Les fermetures d'anciennes sockets primary qui ne correspondent plus au primary courant sont ignorees avec `PRIMARY_ORGANIZER_CLOSE_IGNORED_STALE`, avant toute suppression de session.
- Si aucune socket organizer ouverte n'existe, `WS_SEND_NO_PRIMARY_ORGANIZER` reste emis avec les compteurs `active_organizer_count`, `active_secondary_count` et `secondary_count`.
...
## Update 2026-03-24 — Observability prod reprise joueur Quiz
...
- Organizer Quiz (front `games`) : le classement inter-question affiché en overlay (`#quiz-leaderboard-overlay`) est désormais figé en haut (reset `scrollTop=0`) sans auto-scroll ; auto-scroll des autres contextes (pause/fin de partie) inchangé.
- Reveal remote/options: les payloads de session propagent `correctOptionKey` (en plus de `correctOption`) pour permettre un matching key-first côté remote (évite les divergences de format texte).
...
- Player : message `registerPlayer` avec `sessionId`, `player_id` canon obligatoire (`p:<uuid>`), `playerName`, et `playerId` numérique optionnel (`player_db_id` compat). Socket taggé `isPlayer`, `playerId=<player_id canon>`, `playerDbId=<db id?>`. Sources: registration.js; wsHandler.js.
...
- `update_score`  
  ```json
  { "game": "quiz", "sessionPrimaryId": 123, "player_id": "p:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "playerId": 1, "score": 10 }
  ```  
...
- `deactivate_player`  
  ```json
  { "game": "quiz", "playerId": 1, "player_id": "p:uuid-or-stable-key", "sessionPrimaryId": 123 }
  ```  
...
- `player_register`  
  ```json
  { "game": "quiz", "sessionPrimaryId": 123, "username": "Alice", "player_id": "p:uuid-session-scoped" }
  ```  
...
- Loadtest bots (`web/server/actions/loadtest.js`) : `player_id` est généré de manière déterministe par bot/session (`p:uuidv5-ish( "cotton-bot-player-id-v1|quiz|<sid>|<botId>" )`), envoyé sur `registerPlayer` et `checkAnswer`; `playerId` n’est joint que s’il est connu et numérique.
...
- Enrichissement front ingéré : `entry.game` forcé à `quiz` si absent, `meta.ingested_by="CQ_WS"` + `meta.ws_role/ws_client_id` si dispo, `msg` fallback `evt`, `src` compat `entry.src || entry.source`, `meta` aplatie (pas de `meta.meta`).
