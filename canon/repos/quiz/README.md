...
- WS + HTTP unique : `web/server/server.js` monte un HTTP server (port `WS_PORT`, défaut 3032) avec `GET /logs?sessionId=<sid>[&limit=&page=]` (pagination, max 5000) et attache `WebSocketServer` (ping 30s, terminate si pas de pong).
...
## Runtime surfaces
- WebSocket : orchestration dans `web/server/actions/wsHandler.js` + `messaging.js` (logging `WS_IN/WS_OUT` throttle, gestion primary obsolète, loadtest, log_event/log_batch). Heartbeat serveur 30s ; clients envoient `heartbeat`.
...
- `CONFIG` au boot (endpoint Canvas, tokenPresent, envPathUsed). Source: web/server/actions/envUtils.js:146-199.
- `WS_IN` / `WS_OUT` (types whitelisted + throttle). Sources: wsHandler.js:22-186; messaging.js:10-88.
...
- `log_event` / `log_batch` (logs front injectées). Source: wsHandler.js:132-199.
- Canvas writes OK/KO : `CANVAS_WRITE_NO_TOKEN`, `Canvas API write failed`, timeouts. Source: actions/envUtils.js:146-199.
- Filtres utiles : `source=CQ_WS`, `sessionId=<sid>`, `event` in (`CONFIG`,`WS_IN`,`WS_OUT`,`CANVAS_*`); debug activable via `LOG_DEBUG=1`.
- Accès HTTP : `GET /logs?sessionId=<sid>[&limit=&page=]` (CORS `*`, GET/OPTIONS) retourne `entries` + `statusSeed`. Source: server.js:56-150.
- Stockage : `web/server/server-logs.log` + rotations `server-logs.N.log`. Source: logger_ws.js:6-52.
