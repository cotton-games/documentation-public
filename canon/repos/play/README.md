<!-- AUTO-UPDATE:BEGIN id="play-session-token-routing-20260920" owner="codex" -->

## 23/09/2026 — Sas Hub E1

Aucun changement applicatif Play : watcher, mapping et preuve Player sont dans Games. [Contrat et limites](../../../notes/hub-session-readiness-2026-09-23.md). Les retardataires continuent vers la session courante sans nouvel E1/reset.
...
Révision 21/09 : la page EP est probable-only et n’affiche plus de refus de capacité. Le refus d’une admission explicite Hub Play reste dans le helper partagé ; jauge et upsell inchangés.
...
## 20/09/2026 — Routage des tokens de session

`play/web/.htaccess` (syntaxe Nginx) accepte lettres, chiffres, tirets et underscores dans les huit routes transportant `id_securite_championnat_session` : signup public (avec/sans session_join), signup privé, signin public (avec/sans session_join), inscription manage s1/s2 et player-connect. Les tokens persistés `scheduleplan_…` atteignent ainsi le contrôleur PHP existant. Cibles et paramètres inchangés ; patch local non déployé.

Depuis SchedulePlan, `id_securite` peut contenir `_` ; les routes Play session doivent accepter le format persistant complet, avec un segment non vide (`[a-zA-Z0-9_-]+`). Vérification locale : `php web/tests/ep_session_routing_contract_test.php`.

<!-- AUTO-UPDATE:END id="play-session-token-routing-20260920" -->
