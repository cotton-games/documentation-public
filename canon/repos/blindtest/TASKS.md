...
- [x] Base propre `hub_soiree` vérifiée ; extraction minimale, sans merge/cherry-pick. `registerPlayer` utilise la preuve ciblée avec exécution officielle fiable, hors papier ; contrôle lifecycle après await. Capacité historique conservée en contexte incomplet, papier/démo/autonome. Aucune modification gameplay/équipes/publication.
...
- [x] Préflight propre, branche opérateur `hub_session_readiness` depuis `hub_soiree`, reprise sélective sans merge. Même protocole ; publication/index conservant classement et mode équipes ; observateur Browser BT et admission ciblée.
...
- [ ] Migration unifiée Blind Test : DEV `hub_soiree` attesté opérateur, aucun snapshot préalable sauf contradiction démontrée. Adaptateur réel et observateurs encore à développer ; [hooks à prévoir après Bingo](../../../notes/hub-bingo-browser-baseline-2026-09-22.md). Réutiliser Browser/scénario/stop/cleanup communs, qualifier AVANT50/200 avant performance/sas ; Server5000 ultérieur, moteur direct diagnostic.
...
- [x] Même contrat de bind/réconciliation papier que Quiz ; aucune activation équipes. Projection équipes/membres et rankingEntriesTotal restent distincts. Marker21/09 /03 ; aucun restart exécuté.
