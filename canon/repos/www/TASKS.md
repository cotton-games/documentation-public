...
## REPORTÉ SUR MAIN 2026-09-04 - Période Stripe dans le BO
- [x] Reporter uniquement le commit `periode` sur `main` (`962dae1`) après déploiements Global et Pro verts.
- [x] Afficher la période partagée dans les surfaces BO concernées, avec fallback visible et offres non Stripe inchangées.
- [x] Utiliser temporairement la resynchronisation unitaire et le lot manuel de 25 offres NULL pour initialiser l'existant.
- [x] Préserver l'absence de write Stripe et de mutation offre/jauge/pricing.
- [x] Valider les lints PHP, les contrats BO période/lifecycle et `git diff --check` avant push.
...
## FUTUR 2026-09-04 - Observabilité BO du changement d'offre
...
## DOC 2026-07-27 - BO ecommerce: lecture factures remises decimales
- [x] Documentation livree:
  - documenter que les PDF/listings BO relisent les montants stockes;
  - rappeler que `ecommerce_offres_to_clients.remise_pourcentage` est decimal pour conserver les snapshots contractuels;
  - consigner que le BO n'a pas besoin de patch runtime si `ecommerce_commandes_lignes` est coherent.
- [x] Validation documentee:
  - avec snapshot `28,61 %`, les montants attendus sont `49,90 HT`, `9,98 TVA`, `59,88 TTC`;
  - les factures historiques concernees ont ete corrigees en production pour etre realignees avec Stripe.
- [ ] Suivi hors documentation:
  - surveiller les prochains cas de remise decimale dans les exports/listings facture.
