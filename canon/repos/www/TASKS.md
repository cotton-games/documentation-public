<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-www-tasks" owner="codex" -->

## 22/09/2026 — Sessions Hub retirées sur WWW/Play

- [x] Filtrage public partagé et lecteurs directs cohérents, tests PHP/SQLite et contrats locaux verts. **Déploiement PROD prévu par l’opérateur ; réalisation non confirmée.**
- [ ] Confirmer la livraison coordonnée Global, WWW, Play (Global avant ou avec WWW/Play) et la recette navigateur/DB réelle ; vérifier autonome/membre actif, retrait, anciennes URL et historique personnel. [Rapport](../../../notes/hub-public-removed-sessions-2026-09-22.md).
...
### QR permanent : retrait des statistiques du compte — 21/09/2026

Complément QR permanent : les cartes soirée/événement (y compris le repli session membre Hub) ne proposent plus de détail WWW, ni via le bouton ni via le visuel. Le lien organisateur de la carte événement est également neutralisé en contexte QR. Seul le CTA canonique Games reste disponible pendant la fenêtre `open` ; les cartes futures/expirées ne donnent pas accès au jeu. Les pages classiques conservent les liens de consultation. Fichier supplémentaire à déployer : `www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php`. Suite `play/web/tests/ep_session_hub_cta_test.php` étendue (413 contrôles au total), lint PHP et diff-check verts. Journal relu inchangé ; aucun déploiement. Rollback ciblé : `/tmp/cotton-qr-detail-shared-before.php`.

`/place/{code_client}` ne rend plus `place-dynamic-summary` et ne déclenche plus sa requête AJAX après l’agenda. Le chargement des autres sections ne dépend plus de ce bloc. Le endpoint AJAX ignore aussi `include_summary` en contexte QR et retourne une réponse vide pour `section=summary`, sans appeler le dashboard statistique. Les pages établissement classiques conservent leurs statistiques ; agenda, accès Hub, classements et archives restent inchangés.

Fichiers à déployer (WWW uniquement) : `web/fo/modules/entites/clients/fr/fo_clients_view.php` et `web/fo/modules/entites/clients/fr/fo_clients_view_ajax.php`. Journal AI Studio relu inchangé avant patch ; aucun nouveau fichier ciblé signalé hors workspace. Vérification PHP isolée du endpoint (aucun calcul dashboard), exécution JS de `loadSummary` sans cible (aucun fetch), suite partagée389 contrôles et lint2 PHP verts ; diff-check valide. Tests locaux sans DB ni navigateur ; aucun déploiement/restart. Rollback : inverser uniquement les hunks de ce correctif ; copie préalable `/tmp/cotton-qr-summary-baseline/`.


### Signin/signup : présentation Hub unifiée — 21/09/2026
...
Les entrées depuis WWW session membre et Games Hub présentent désormais la soirée/événement : branding Hub canonique (sans surcharge session), titre organisateur avec libellé SOIRÉE/ÉVÉNEMENT, et `app_games_hub_schedule_label_get` identique à Games (date, début du programme, nombre de parties). Le helper Play partagé `ep_hub_auth_branding.php` résout soit le token Hub soit l’appartenance de la session ; aucune admission au chargement du branding. La récupération HTTP du visuel session est évitée pour les sessions Hub. Les sessions autonomes gardent leur présentation historique.

Intention probable depuis WWW : « annoncer et partager ta participation », accès par QR sur place ; retour fiche EP source conservé. Intention `join` depuis Games : « participer à cette soirée/cet événement 🔥 » ; retour d’admission Hub Play conservé. Signin et signup partagent cet habillage et ces variantes. [Preuves, fichiers et tests](../../../notes/play-unified-hub-auth-2026-09-21.md).


### Horaires publics et sous-titre EP — 21/09/2026

WWW/Play affichent le début du programme Hub « À partir de HHhMM » via le même helper canonique que Pro, jamais l’heure propre à S2/S3 ni une plage début/fin. Fiches, cartes, widgets, historique et entêtes auth concernés ; dates et tris inchangés. La résolution publique est mise en cache par session/Hub pendant la requête. Sans Hub ou horaire connu, aucune heure n’est inventée. Sous-titre de fiche EP Hub adapté à soirée/événement et accès indiqué sur place par QR. Complément : « J’invite mes amis ! » est réaffiché sur la fiche session Hub après probable déclarée/confirmée ; annulation le masque. Il partage la consultation WWW sans bouton runtime. [Fichiers, preuves et contrôles](../../../notes/www-play-program-schedule-2026-09-21.md).


### CTA Hub EP / WWW / QR — 20/09, révisé le 21/09/2026

- [x] Fiche/agenda/page Hub EP probable-only avant et pendant fenêtre ; anciens POST `ep_join`/`hub_ep_join` refusés, aucune admission/capacité déclenchée.
- [x] Lecture individuelle Hub sans fallback probable session ; déclaration/annulation et CSRF conservés, équipe legacy distincte.
- [x] CTA WWW existant vers fiche EP source après auth ; retrait des boutons Hub supplémentaires classiques ; QR permanent `open` seul accès direct WWW.
- [x] Retours `probable` et intention absente/invalide sans admission ; `join` explicite depuis Hub Play et sessions autonomes préservés.
- [x] 310 contrôles CTA/rendus/retours, 16 contrôles probable SQL à 50/50, suites de routage/jauge/QR temporel vertes. Test QR serveur indépendant en échec sur fixture `$conf` manquante.
- [ ] Recette navigateur et persistance réelle après livraison coordonnée autorisée ; aucun accès DB/DEV/PROD. Présence réelle de plusieurs Hubs ouverts non vérifiable localement. [Rapport](../../../notes/hub-access-probable-only-2026-09-21.md).

Cette révision remplace le join depuis la page EP décrit dans les tâches historiques du 31/07 ; leur recette d’admission EP n’est plus applicable.




## CORRIGÉ LOCAL / À RECETTER — Remises2026 affiliés — 10/09/2026

- [x] Appliquer le contrat d’acquisition propre aux surfaces www, conformément aux décisions métier ; source locale déclarée à jour par l’opérateur pour DEV.
- [x] Valider le test transverse `pro/web/ec/ec_remises_2026_own_checkout_test.php` : 429 contrôles hors connexion ; six contrats existants Global/Pro verts. Doubles DB/Stripe exclusivement, aucune exécution sur serveur.
- [x] Documenter scope, suppression du coupon legacy et V&B, refus des pending anciennes et absence de rétroactivité ; sitemap/index générés.
- [ ] Recette authentifiée DEV de la chaîne BO → preview → pending → checkout → webhook → commande, selon le [rapport](../../../notes/remises-2026-affilies-patch-2026-09-10.md).
- [ ] Avant PROD : vérifier le script serveur et les sources coordonnées Global/Pro/WWW, comme convenu avec l’opérateur. Aucun déploiement autoréalisé.

<!-- AUTO-UPDATE:END id="remises-affilies-20260910-www-tasks" -->

## Migration du code Hub vers main — 2026-09-09

- [x] [Audit branches/code et manifeste candidat](../../../notes/hub-prod-code-migration-2026-09-09.md) : Merge simulé sans conflit, ecommerce main=Hub exclu ; routes publiques et includes examinés, test ecommerce vert sans certification fonctionnelle Hub FO. 9 fichiers applicatifs candidats.
...
- [x] Reprise09/09 : [réconciliation PROD appliquée sur hub_soiree](../../../tmp/hub-prod-code-2026-09-09/reconciliation/RECONCILIATION-PROD.md). Seul le fichier ciblé modifié, résultat exact PROD + ajouts Hub, patches et contrôles verts. LP extérieure préservée. SERVER_RECONCILIATION CLEARED — MERGES MAY CONTINUE ; commits des réconciliations à effectuer avant merges. Aucun commit/merge/push/déploiement/SQL par cette passe.

## FUTUR 2026-09-03 - Observabilité BO du changement d'offre
- [ ] Définir l'affichage BO séparant intention Cotton, projection Stripe, exécution financière et synchronisation retour.
- [ ] Ne jamais reconstruire l'offre canonique depuis le Price Stripe.
- [ ] Exposer les états d'échec/replay sans ajouter de writer commercial BO implicite.
- [ ] Conserver les offres déléguées hors cadre hors de la V1.

## CORRIGÉ/À RECETTER 2026-09-03 - Période Stripe dans le BO
- [x] Faire consommer le helper de période partagé aux deux rendus des contrats réseau.
- [x] Ajouter aux détails d'une offre Stripe une carte dédiée avec alerte de fallback transitoire.
- [x] Préserver les offres non Stripe et l'absence de toute écriture commerciale/Stripe côté BO.
- [x] Étendre le contrat autonome du cycle de vie BO.
- [x] Utiliser temporairement la resynchronisation forcée et le backfill manuel de 25 offres pour initialiser l'existant.
- [x] Retirer après convergence les deux POST, boutons, curseur, compteurs, flashs, CSRF dédié, helpers BO et test manuel.
- [x] Conserver la carte de période, l'avertissement fallback, les offres legacy et les lectures réseau canoniques.
- [x] Ne créer aucun traitement spécifique des anciennes références `sub_...` inexistantes dans Stripe.

## PATCH 2026-07-28 - FO Place: classement saisonnier agrégé
- [x] Relire START, SITEMAP, SITEMAP.ndjson, DOCS_MANIFEST et le journal AI Studio raw avant patch.
- [x] Identifier le rendu Place `fo_clients_view_leaderboards_html_get(...)` et l'endpoint AJAX `fo_clients_view_ajax.php`.
- [x] Remplacer les sections par jeu par une section `Classement général` alimentée par `aggregate_ranking`.
- [x] Afficher `victoires · podiums · parties` et ne plus rendre le score agrégé dans le tableau Place.
- [x] Consommer le contrat d'identité agrégée Global sans fusion locale par pseudo.
- [x] Restaurer le podium Place au-dessus du tableau agrégé en réutilisant les rangs 1 à 3 du `aggregate_ranking`.
- [x] Conserver routes, onglets, SEO, agenda, sessions passées et fiches résultats de partie.
- [x] Incrémenter la version du cache Place côté Global.
- [x] Vérification locale: lint PHP ciblé OK.
- [ ] Recette navigateur: Place publique avec classement, Place sans classement, cache froid/chaud, desktop/mobile.

# Repo `www` — Tasks

## CORRIGÉ/À RECETTER 2026-09-03 - BO des offres déléguées hors cadre
- [x] Rendre les vues `reseau_contrats` explicitement read-only et afficher l’offre effective depuis son snapshot Cotton.
- [x] Supprimer l’attribution hors cadre du BO et refuser aussi cette action côté serveur; le checkout Pro reste obligatoire.
- [x] Retirer la terminaison locale BO au profit du parcours de résiliation et bloquer la désaffiliation pendant toute subscription active, y compris `cancel_at_period_end`.
- [x] Faire lire `offres_clients` depuis `prix_reference_ht`, `prix_ht` et `remise_*` persistés, sans reconstruction depuis le catalogue courant.
- [ ] Recette BO DEV: affichage révisé, cadre inchangé, refus d’attribution/fin locale, refus puis autorisation de désaffiliation à l’échéance.
## PATCH 2026-07-17 - FO Hub public: compléments contexte
- [x] Rendre la page publique Hub accessible dès que le Hub existe avec au moins une session liée, sans exiger que cette session soit publiée dans l'agenda global.
- [x] Conserver `/fr/agenda` hors périmètre de cette passe.
- [x] Sur les pages lieu `/fr/place/{slug}`, afficher les soirées Hub à venir comme cartes soirée et ne pas retomber sur les sessions unitaires du Hub.
- [x] Sur les pages Hub soirée, afficher le programme avec les mêmes cartes que la page événement, visuel de session inclus et accès au détail.
- [x] Sur les fiches détail de session, afficher le contexte Hub avant le lieu: nom de la soirée ou de l'événement, puis lien retour vers la page Hub.
- [x] Depuis la page Hub soirée, conserver l'accès à la page publique du lieu; depuis la page Hub événement, ne pas exposer de page lieu si le compte n'est pas publié.
- [x] Ne pas afficher les lots sur la page publique Hub pour le moment.
- [ ] Recette navigateur: page Hub soirée, page Hub événement, fiche session issue d'un Hub, page lieu avec soirée multi-session, ancienne page événement sans Hub.

## PATCH 2026-07-17 - FO Hub public soirée/événement
- [x] Ajouter la route publique `/fr/soirees/{hub_token}` vers une vue Hub commune, résolue par `app_games_hub_public_resolve(...)`.
- [x] Ajouter une vue partagée `fo_hubs_view_shared.php` capable de rendre soirée ou événement depuis le même contrat: visuel, titre, date, horaires, lieu/adresse, description, lien et programme.
- [x] Aligner le programme de la page Hub sur la page événement en réutilisant les cartes `fo_sessions_list_bloc.php`, avec accès au détail de chaque session.
- [x] Ne pas afficher les lots Hub sur la page publique soirée/événement pour le moment.
- [x] Faire basculer l'ancienne URL événement `/fr/evenements/{slug}` vers la vue Hub commune seulement si un Hub événement publiable existe; sinon conserver le rendu legacy.
- [x] Conserver l'URL canonique historique des événements Hub sur `/fr/evenements/{slug}` pour éviter deux pages indexables.
- [x] Faire évoluer l'agenda des pages lieu `/fr/place/{slug}`: les sessions rattachées à un Hub soirée publiable sont représentées par une seule carte Hub, les sessions autonomes restent visibles.
- [x] Respecter le complément produit: aucune modification de l'agenda public global `/fr/agenda` dans cette passe.
- [x] Vérification locale: `php -l` sur les vues/SEO Hub et événement OK.
- [ ] Recette navigateur: vérifier `/fr/soirees/{token}`, `/fr/evenements/{slug}` avec Hub publiable, `/fr/evenements/{slug}` sans Hub publiable, 404 Hub non publiable, page lieu avec une soirée à plusieurs sessions et absence de changement sur `/fr/agenda`.

## PATCH 2026-07-17 - FO événement: visuel hero issu du branding opération
- [x] La page publique événement `/fr/evenements/{slug}` utilise `app_games_hub_operation_branding_get_for_event(...)` pour son visuel principal quand un Hub événement existe.
- [x] La cascade du hero devient opération -> réseau -> compte -> fallback Cotton, sans deuxième visuel public et sans copie vers les sessions.
- [x] Les anciens événements sans Hub gardent le fallback legacy `app_evenement_branding_get_detail(... place_bandeau_1)`.
- [x] Aucune fiche publique de session n'est modifiée.
- [x] Vérification locale: `php -l /home/romain/Cotton/www/web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK.

## PATCH 2026-07-07 - FO Blind Test: masquer badge equipe solo
- [x] Correctif livre:
  - les fiches session publiques et les classements FO refusent le badge `ÉQUIPE` si `team_member_count`, `teamMemberCount` ou `members` indique moins de 2 membres;
  - les pages publiques événement/place réutilisent le même helper de ligne équipe.
- [x] Garde-fous:
  - aucun changement de requête, score ou rang;
  - pas de modification Quiz/Bingo.
- [x] Verification locale:
  - `php -l /home/romain/Cotton/www/web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK;
  - `php -l /home/romain/Cotton/www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK.

## PATCH 2026-07-06 - FO evenement/session: participants mixtes Blind Test
- [x] Correctif livre:
  - l'onglet classements des pages publiques evenement/place consomme `participants` / `participants_podium` pour Blind Test;
  - une equipe runtime Blind Test est affichee dans le meme Top 10 que les joueurs solos;
  - le libelle de colonne devient `Participants` pour Blind Test.
- [x] Garde-fous:
  - pas de classement public separe `equipes` pour les equipes runtime Blind Test;
  - pas de changement pour Quiz, Bingo ou sessions sans `blindtest_session_teams`.
- [x] Verification locale:
  - `php -l /home/romain/Cotton/www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK.

## PATCH 2026-07-06 - FO session: badge equipe Blind Test
- [x] Correctif livre:
  - la fiche publique de session affiche le badge compact `ÉQUIPE` sur les lignes podium et classement issues d'une equipe runtime Blind Test;
  - le libelle affiche reste le nom brut fourni par le socle global.
- [x] Garde-fous:
  - aucun changement sur les requetes, scores, rangs ou sessions non Blind Test;
  - les pages evenement continuent de reutiliser le helper FO communauté deja badge.
- [x] Verification locale:
  - `php -l /home/romain/Cotton/www/web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK.

## PATCH 2026-07-06 - FO communauté: badge equipe Blind Test
- [x] Correctif livre:
  - le podium FO et le tableau Top 10 savent afficher une ligne Blind Test identifiee equipe avec nom brut + badge `ÉQUIPE`;
  - le rendu retire un eventuel suffixe legacy `(n)` uniquement pour les lignes equipe Blind Test.
- [x] Garde-fous:
  - aucune modification de sélection, scoring ou requête leaderboard;
  - badge limite aux lignes `participant_type/team:*` du jeu `blindtest`.
- [x] Verification locale:
  - `php -l /home/romain/Cotton/www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK.

## PATCH 2026-07-03 - BO logs clients: secondes et CTA session
- [x] Audit:
  - confirmer le tri `clients_logs` principal `date_ajout DESC, id DESC`;
  - confirmer le tie-break `id` au clic sur la colonne Date;
  - confirmer que `french_time()` masque les secondes alors que `date_ajout` est un datetime;
  - confirmer que le BO transforme `Session #... > CTA ... cliqué [ contexte ]` dans `bo_clients_logs_functions.php`.
- [x] Correctif livre:
  - afficher les secondes dans le listing principal `Tracking > Logs clients` pour `clients_logs`;
  - afficher aussi les secondes dans les blocs Logs utilisant le hook `bo_clients_logs_list_cell_html_get(...)`;
  - conserver le libelle `CTA session : ...` uniquement pour les logs bruts `Session #... > CTA ...`;
  - ne pas modifier les valeurs historiques `clients_logs.nom`.
- [x] Verification locale:
  - `php -l www/web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php` OK;
  - `php -l www/web/bo/master/bo_master_list.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - `Tracking > Logs clients`: heure visible avec secondes;
  - fiche client/contact: bloc Logs heure visible avec secondes;
  - clic Date: ordre stable `date_ajout`, puis `id`;
  - anciens logs `CTA session` restent lisibles sans migration.

## PATCH 2026-07-03 - BO home: activation commerciale performance
- [x] Audit:
  - fonction cible `bo_resumes_activation_opportunities_get()` dans `www/web/bo/www/modules/syntheses/resumes/bo_resumes_list.php`;
  - tables touchees: `clients`, `clients_contacts_to_clients`, `clients_contacts`, referentiels client, `ecommerce_offres_to_clients`, `ecommerce_offres`, `ecommerce_commandes`, `reporting_games_sessions_detail`, `championnats_sessions`, `reporting_games_demos_detail`, `clients_logs`;
  - requete principale avec `GROUP BY`, `COUNT(DISTINCT ...)`, `UNION ALL`, `NOT EXISTS` sur factures d'essai, aggregats historiques et logs 30j;
  - filtre metier final: clients non supprimes, Dynamisation, hors siege reseau.
- [x] Correctif livre:
  - conserver toutes les regles de categories/priorites;
  - restreindre les sous-agregats offres/factures/usages/sessions/demos/logs au meme perimetre client que le bloc final;
  - ajouter instrumentation fine sous `perf_home_bo=1`: preparation, requete principale, priorisation, rappels, tri;
  - ne pas modifier le rendu ni passer le bloc en AJAX.
- [x] Correction regression:
  - mesures prod apres restriction: requete principale ~1688-1715 ms, calcul ~1697-1724 ms, total home ~2109 ms;
  - decision: restaurer la strategie SQL historique par defaut sur la home normale;
  - retirer ensuite la variante debug `perf_home_bo_activation_scoped=1`, regressive en production;
  - conserver uniquement la mesure `Activation commerciale - requete principale` sur le chemin historique valide.
- [x] Nettoyage final:
  - supprimer le flag debug `perf_home_bo_activation_scoped=1`;
  - supprimer les joins de scope ajoutes seulement pour cette variante;
  - ne pas modifier les seuils, categories, rappels, tri ou rendu du bloc.
- [ ] Mesure recette serveur:
  - comparer `Activation commerciale - calcul` au niveau avant patch, ~194-354 ms avec pic observe ~484 ms;
  - relever `Activation commerciale - requete principale`;
  - relever `Activation commerciale - priorisation`;
  - relever `Activation commerciale - rappels`;
  - objectif: bloc idealement <150 ms, total home idealement <800-900 ms.
- [ ] Index SQL a envisager seulement si la requete principale reste dominante:
  - analyser `EXPLAIN` prod avant toute migration;
  - candidats probables a valider selon plans: index sur tables reporting par `(id_client, session_date)` / `(id_client, players_count)` et sur `clients_logs(id_log_type, date_ajout, id_client)`.

## PATCH 2026-07-03 - BO home: dernieres connexions clients performance
- [x] Audit:
  - bloc cible dans `www/web/bo/www/modules/syntheses/resumes/bo_resumes_list.php`;
  - requete source `clients_logs` filtre `id_log_type=4`, tri `date_ajout DESC, id DESC`, `LIMIT 30`;
  - joins d'affichage `clients` et `clients_contacts`;
  - enrichissement N+1 identifie sur `module_get_detail("referentiels_clients_pipeline_etats", ...)` par ligne;
  - schema local: `clients_logs` a `PRIMARY(id)` et `KEY(id_client)`, pas d'index local visible sur `(id_log_type, date_ajout, id)`.
- [x] Correctif livre:
  - borner les 30 lignes `clients_logs` dans une sous-requete avant les joins client/contact;
  - precharger les pipelines en une seule requete `referentiels_clients_pipeline_etats WHERE id IN (...)`;
  - ajouter les mesures detaillees sous `perf_home_bo=1`: requete, referentiels, rendu;
  - conserver le nombre de lignes, les informations affichees et le rendu metier.
- [ ] Mesure recette serveur:
  - relever `Dernieres connexions clients - requete`;
  - relever `Dernieres connexions clients - referentiels`;
  - relever `Dernieres connexions clients - rendu`;
  - comparer au bloc avant patch, environ 1888-1898 ms.
- [ ] Index SQL a valider separement si la requete reste lente:
  - `ALTER TABLE clients_logs ADD INDEX idx_clients_logs_type_date_id (id_log_type, date_ajout, id);`

## PATCH 2026-07-03 - BO home: instrumentation temporaire performance + KPI facturation rapide
- [x] Audit / contexte:
  - confirmer le point d'entree BO `www/web/bo/bo.php`;
  - confirmer le rendu principal `www/web/bo/www/modules/syntheses/resumes/bo_resumes_list.php`;
  - confirmer que `Suivi mensuel` est charge via AJAX `monthly_metrics_ajax=1`.
- [x] Instrumentation temporaire livree:
  - ajouter le flag super-admin `perf_home_bo=1`;
  - mesurer le total home, KPIs, include facturation active, activation commerciale, sessions Blind Test/Quiz/Bingo, connexions, inscrits, offres, resiliations, feedback et placeholder mensuel;
  - mesurer l'endpoint `bo_resumes_monthly_metrics.php` par agregat;
  - afficher les mesures uniquement sous flag, sans log permanent et sans modification des requetes metier.
- [x] Correctif performance livre:
  - remplacer l'include par defaut de `bo_facturation_pivot_saas.php` dans les KPIs home par un helper dedie aux IDs `end_active`;
  - valider la comparaison compteur avec `BO_HOME_FACTURATION_COMPARE fast_count=75 legacy_count=75 only_fast_count=0 only_legacy_count=0`;
  - retirer ensuite le flag debug `perf_home_bo_compare_facturation=1`, l'include legacy compare et le commentaire HTML associe;
  - conserver l'instrumentation temporaire et ne pas toucher au `Suivi mensuel`.
- [x] Mesure performance communiquee:
  - total home: ~1832 ms avant, ~558 ms apres;
  - facturation active: ~1336 ms via include legacy, ~19 ms via helper rapide;
  - KPIs home: ~1411 ms avant, ~85 ms apres;
  - comparaison legacy: ~1538 ms, uniquement sous flag explicite;
  - `Suivi mensuel`: ~124-132 ms en AJAX, hors rendu initial.
- [x] Verification locale:
  - `php -l web/bo/www/modules/syntheses/resumes/bo_resumes_list.php` OK;
  - `php -l web/bo/www/modules/syntheses/resumes/bo_resumes_monthly_metrics.php` OK.
- [ ] Mesure recette serveur:
  - ouvrir `bo.php?a=www&t=syntheses&m=resumes&p=list&perf_home_bo=1`;
  - verifier que la metrique `KPIs - include facturation legacy compare` et le commentaire `BO_HOME_FACTURATION_COMPARE` ne sont plus emis;
  - verifier que le fetch `monthly_metrics_ajax=1&perf_home_bo=1` affiche aussi le detail du suivi mensuel.

## PATCH 2026-07-02 - BO contacts multi-comptes: home et comptes lies
- [x] Audit:
  - localiser le bloc Home BO `Dernières connexions clients` dans `syntheses/resumes/bo_resumes_list.php`;
  - confirmer qu'il lit `clients_logs` filtre `id_log_type=4`;
  - confirmer que `clients_logs.id_client` correspond au compte actif loggue au moment de la connexion;
  - confirmer que `id_client_contact` est disponible et permet de recuperer le contact;
  - confirmer que la fiche contact est une vue BO generique avec un hook central `entites/clients_contacts/bo_module_view_top.php` et un aside `bo_module_aside.php`.
- [x] Correctif livre:
  - enrichir `Dernières connexions clients` avec le contact connecte;
  - afficher le contact sur la meme ligne que le client sous la forme `Client · Contact`;
  - ajouter dans la partie centrale de la fiche contact un bloc `Comptes liés` avec client, role, pipeline, typologie, usage, derniere connexion et lien fiche client;
  - conserver le bloc Logs contact directement visible en aside avec colonne Client, Action, IP et tri `date_ajout DESC, id DESC`;
  - ne pas ajouter d'action `Ouvrir dans EC`.
- [x] Verification locale:
  - `php -l web/bo/www/modules/entites/clients_contacts/bo_module_view_top.php` OK;
  - `php -l web/bo/www/modules/syntheses/resumes/bo_resumes_list.php` OK;
  - `php -l web/bo/www/modules/entites/clients_contacts/bo_module_aside.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - Home BO: connexion mono-compte affiche client + contact;
  - Home BO: connexion multi-compte affiche client actif et contact sur une seule ligne;
  - fiche contact mono-compte: bloc `Comptes liés` affiche 1 ligne;
  - fiche contact multi-compte: bloc `Comptes liés` affiche tous les comptes et liens client;
  - fiche contact logs: les actions de comptes differents restent lisibles avec la colonne Client;
  - acces BO/admin vers EC: pas de pollution `clients_logs` ordinaire.

## PATCH 2026-07-02 - BO contacts: logs multi-comptes lisibles
- [x] Audit:
  - verifier le listing principal `Tracking > Logs clients`, qui affiche deja Client + Contact;
  - verifier le bloc `Logs` de fiche client, filtre par `id_client`;
  - verifier le bloc `Logs` de fiche contact, filtre par `id_client_contact`;
  - confirmer que la fiche contact melangeait les logs multi-comptes sans colonne Client.
- [x] Correctif livre:
  - ajouter la colonne `Client` au bloc `Logs` de fiche contact;
  - conserver le filtre `id_client_contact=...`;
  - conserver le tri `date_ajout DESC, id DESC`;
  - transmettre le `id_client` cible dans les liens temporaires EC generes depuis la fiche client.
- [x] Verification locale:
  - `php -l web/bo/www/modules/entites/clients/bo_clients_script.php` OK;
  - `php -l web/bo/www/modules/entites/clients_contacts/bo_module_aside.php` OK.
- [ ] Verification recette serveur:
  - fiche client A: seuls les logs A sont visibles;
  - fiche client B: seuls les logs B sont visibles;
  - fiche contact multi-comptes: chaque ligne affiche le client concerne;
  - lien temporaire EC depuis fiche client B: ouverture sur B.

## PATCH 2026-07-02 - BO logs clients: libelles offres et CTA session
- [x] Audit:
  - verifier le mapping lisible dans `bo_clients_logs_functions.php`;
  - confirmer que `ecommerce > offres > form_step_1/2/3` etait masque par le libelle generique `Souscription`;
  - confirmer que les nouveaux logs CTA session doivent etre rendus lisiblement sans modifier la valeur brute.
- [x] Correctif livre:
  - `form_step_1` affiche `Offres / Choix des offres`;
  - `form_step_2` affiche `Offres / Sélection de l’offre`;
  - `form_step_3` affiche `Offres / Confirmation commande`;
  - `Session #... > CTA ... cliqué [...]` affiche `Agenda / CTA session : ...`;
  - conserver le detail brut pour audit.
- [x] Verification locale:
  - `php -l web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - BO logs offre: verifier les 3 libelles metier;
  - BO logs session: verifier les CTA `Préparer`, `Ouvrir`, `Reprendre`, `Résultats` avec leur contexte.

## PATCH 2026-07-02 - BO logs clients: tri deterministe et libelles first_party
- [x] Audit:
  - verifier le listing generique `www/web/bo/master/bo_master_list.php`;
  - verifier le module `www/web/bo/www/modules/tracking/clients_logs/bo_module_parametres.php`;
  - verifier les blocs logs client/contact qui utilisent `module_aside_get_html(...)`;
  - confirmer que le listing principal affiche deja `french_time(date_ajout)`, donc les secondes existent cote UI;
  - confirmer que les libelles BO mappaient `tunnel > start` de facon trop generique.
- [x] Correctif livre:
  - trier `clients_logs` par `date_ajout DESC, id DESC`;
  - ajouter `id` visible sur le listing principal pour disposer d'un repere technique discret;
  - conserver `id` comme tie-breaker quand l'utilisateur trie manuellement par date;
  - aligner les blocs logs client/contact/branding sur le meme tri;
  - rendre lisibles les routes `first_party_onboarding`, `sessions_day`, `sessions_list`, `sessions_view`, `agenda_mode`, `step_1_game`;
  - afficher les logs `Session officielle ... > ajout` comme creation effective.
- [x] Verification locale:
  - `php -l web/bo/www/modules/tracking/clients_logs/bo_module_parametres.php` OK;
  - `php -l web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php` OK;
  - `php -l web/bo/master/bo_master_list.php` OK;
  - `php -l web/bo/www/modules/entites/clients/bo_module_aside.php` OK;
  - `php -l web/bo/www/modules/entites/clients/bo_clients_view.php` OK;
  - `php -l web/bo/www/modules/entites/clients_contacts/bo_module_aside.php` OK;
  - `php -l web/bo/www/modules/entites/clients_branding/NA_bo_module_aside.php` OK;
  - `php -l web/bo/www/modules/general/branding/NA_bo_module_aside.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - `Tracking > Logs clients`: ordre stable a seconde egale via `date_ajout DESC, id DESC`;
  - clic sur la colonne Date: ordre stable avec `id` comme tie-breaker;
  - fiche client/contact: blocs `Logs` dans le meme ordre;
  - routes first_party/classiques affichees avec libelles distincts.

## PATCH 2026-07-02 - Page evenement gamification: resultats publics
- [x] Audit:
  - identifier la page publique evenement `www/web/fo/modules/operations/evenements/fr/fo_evenements_view.php`;
  - confirmer que les sessions rattachees passent par `championnats_sessions.id_operation_evenement` et les helpers `app_evenement_sessions_get_count(...)` / `app_sessions_get_liste(...)`;
  - reutiliser le moteur global `app_client_joueurs_dashboard_context_compute(...)` qui produit `players_full`, `teams_full`, `players_podium` et `teams_podium`;
  - verifier que le pivot pro conserve deja un CTA vers `/fr/evenements/{slug}` quand la page evenement existe.
- [x] Correctif livre:
  - ajouter un scope evenement optionnel au moteur global de classement, sans changer les appels existants place/pivot;
  - ajouter `app_client_joueurs_dashboard_get_context_for_event(...)` pour consolider uniquement les sessions d'un evenement;
  - remplacer la section sessions evenement par une section a onglets proche de `place-dynamic-tabs-section`;
  - afficher les onglets `Sessions à venir`, `Classements`, `Sessions terminées` selon les contenus disponibles;
  - prioriser l'onglet `Sessions à venir` s'il existe des sessions accessibles, puis `Classements` sinon, puis `Sessions terminées`;
  - afficher les classements de l'evenement des qu'au moins une session terminee porte des resultats consolidables, meme si l'evenement n'est pas completement termine;
  - reutiliser le rendu de l'onglet `Classements` de la page place pour l'onglet classements evenement;
  - afficher les blocs dans l'ordre public Blind Test, Bingo Musical, Cotton Quiz, avec joueurs pour Blind Test/Bingo et equipes pour Cotton Quiz;
  - conserver l'acces aux fiches detail des sessions terminees depuis la page evenement, avec le CTA `Voir les résultats`;
  - corriger le compteur local de sessions evenement pour inclure Blind Test.
- [x] Verification locale:
  - `php -l global/web/app/modules/entites/clients/app_clients_functions.php` OK;
  - `php -l www/web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK;
  - `php -l www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK.
- [ ] Verification recette serveur:
  - evenement gamification futur avec sessions non terminees: aucun bloc classement agrege;
  - evenement termine Blind Test: podium + Top 10 joueurs;
  - evenement termine Bingo Musical: podium + Top 10 joueurs;
  - evenement termine Cotton Quiz: podium + Top 10 equipes;
  - evenement multi-jeux termine: blocs par jeu sans melange saisonnier;
  - evenement termine sans resultats exploitables: aucun bloc vide;
  - page place classique et page session classique: aucun changement visuel/logique;
  - pivot pro termine: lien vers la page evenement toujours disponible.

## PATCH 2026-07-01 - CTA affiliation reseau expires
- [x] Audit:
  - relire la logique `$_SESSION['url_pro_signup']` / `$_SESSION['url_pro_signin']` cote `www/web/fo/fo.php`;
  - relire la pose des CTA PRO par `www/web/lp/lp.php`;
  - utiliser `pro/logs/access_log` pour confirmer que le site public relancait `/utm/reseau/collectif-culture-bar-bars` apres purge PRO.
- [x] Correctif livre:
  - horodater les CTA WWW reseau stockes depuis la LP avec expiration 2 heures;
  - neutraliser les anciens CTA WWW `/utm/reseau/...` sans echeance;
  - neutraliser les CTA WWW reseau expires au rendu FO;
  - revenir au CTA standard quand le contexte WWW est invalide;
  - conserver la recreation volontaire du contexte quand l'utilisateur repasse par la LP reseau.
- [x] Verification locale:
  - `php -l web/fo/fo.php` OK;
  - `php -l web/lp/lp.php` OK.
- [ ] Verification recette serveur:
  - vieille session WWW pre-patch avec CTA reseau: ouvrir une page standard et verifier que le CTA pointe vers `/signup`;
  - LP reseau puis navigation WWW: CTA reseau conserve pendant moins de 2 heures;
  - apres expiration: CTA standard et absence de nouvel appel `/utm/reseau/...`;
  - retour explicite sur LP reseau: CTA reseau recree pour 2 heures.

## PATCH 2026-06-30 - BO Supports image Google Drive: upload manuel
- [x] Correctif livre:
  - ajouter l'entree BO `Jeux > Supports image Google Drive` a cote de `Questions quiz culture G`;
  - lister les questions `id_type_support=1` dont `lien_support` pointe vers Google Drive, avec ID, question, lien Drive cliquable et upload manuel;
  - ajouter un filtre lot pour traiter en priorite les questions hors lot `id_lot=0`, avec option associees a un lot;
  - ajouter un export CSV global de fallback avec `id_question`, `id_lot`, question, reponse et lien Google Drive, en conservant le filtre courant;
  - uploader manuellement JPG/PNG/WebP par question, valider via `getimagesize`, limiter a 15 Mo, redimensionner/normaliser a 1600 px max et refuser un fichier final > 5 Mo;
  - stocker les images dans `upload/quiz/images/questions/question/`, creer la ligne `medias_images`, puis remplacer `questions.lien_support` par une URL serveur absolue seulement si l'ancien lien n'a pas change;
  - retirer de `Questions quiz culture G` le bloc de migration automatique par lots, trop expose aux timeouts BO.
- [x] Verification locale:
  - `php -l web/bo/bo.php` OK.
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_functions.php` OK.
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_list.php` OK.
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_script.php` OK.
  - `php -l web/bo/www/modules/jeux/supports_drive/*.php` OK.
- [ ] Verification recette serveur:
  - ouvrir `?t=jeux&m=supports_drive`;
  - exporter le CSV fallback avant migration;
  - ouvrir un lien Drive, telecharger l'image puis l'uploader sur la ligne;
  - verifier le fichier public, la ligne `medias_images`, la mise a jour de `questions.lien_support` et la disparition de la ligne migree.

## PATCH 2026-06-30 - BO logs clients: inscription sans demo
- [x] Correctif livre:
  - traduire les logs bruts `Inscription standard` et `Inscription sans demo` dans le BO tracking clients;
  - conserver la traduction existante de `MAJ date session demo` sans modifier les requetes de sessions.
- [x] Verification locale:
  - `php -l web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php` OK.
- [ ] Verification recette serveur:
  - ouvrir la fiche BO d'un client avec le log `Inscription standard`;
  - ouvrir la fiche BO d'un client avec le log `Inscription sans demo`;
  - verifier l'affichage zone `Compte`, libelle dedie, detail brut conserve.

## PATCH 2026-06-29 - BO Questions quiz culture G: import CSV partiel
- [x] Audit:
  - reprendre l'historique Git du module `web/bo/www/modules/jeux/questions_numeriques`;
  - confirmer que `question_source_required` venait du commit initial de reorientation et non d'une modification posterieure;
  - consulter le journal AI Studio raw avant patch, sans entree cible trouvee sur `questions_numeriques` / `question_source_required`.
- [x] Correctif livre:
  - distinguer un champ source absent du payload d'un champ source explicitement envoye vide;
  - reprendre les valeurs existantes de `questions` pour les champs source absents lors d'une mise a jour;
  - conserver le verrou de securite si `question` ou `reponse` est explicitement videe.
  - limiter la vue BO, les compteurs et l'export CSV aux questions `id_etat=2` et `id_lot=0`.
  - imposer `id_etat=2` et `id_lot=0` pour la creation d'un lot `N` de test depuis le BO.
- [x] Ajustement UI:
  - afficher les questions papier et numeriques completes dans le tableau BO, sans troncature `...`;
  - garder les reponses, commentaires et propositions en version compacte.
- [x] Export CSV:
  - ajouter `lien_support` comme colonne source read-only pour guider l'adaptation IA;
  - ajouter `jour_associe` comme colonne source read-only pour conserver le repere calendaire dans les questions numeriques;
  - conserver l'import limite aux colonnes numeriques/propositions.
- [x] Discipline documentaire:
  - preciser que `commentaire_numerique` reste vide s'il repete simplement la reponse.
  - preciser que les distracteurs doivent rester visuellement plausibles quand un support est present.
  - ajouter les regles complementaires d'adaptation papier vers numerique pour questions longues, reponses explicatives, rebus/visuels, pieges et relecture joueur.
  - ajouter la regle dingbats pour construire les distracteurs depuis le mecanisme visuel du support.
  - ajouter les gardes contre les formulations contenant la reponse et les reformulations trop resumees d'enigmes logiques.
  - preciser que `jour_associe` prime sur le raccourcissement et doit rester visible dans `question_numerique`, avec une integration naturelle dans la phrase.
- [x] Verification locale:
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_functions.php` OK.
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_list.php` OK.
- [ ] Verification recette serveur:
  - reimporter un CSV numerique-only depuis le BO;
  - verifier que les lignes passent de `Échecs: 100` a des mises a jour effectives;
  - controler une ligne source dans `questions` pour confirmer que question/reponse papier n'ont pas ete effacees.
  - verifier que la liste et l'export BO ne remontent pas les questions `id_etat!=2`.
  - verifier qu'une question certifiee mais inactive ou liee a un lot ne peut pas etre ajoutee a un lot `N` de test.

## PATCH 2026-06-25 - BO Questions numériques: source `questions`
- [x] Audit:
  - verifier que le module BO `web/bo/www/modules/jeux/questions_numeriques` lisait/ecrivait encore l'ancien stock `questions_numeriques`;
  - confirmer que les distracteurs historiques sont les lignes communes `questions_propositions` rattachees a `questions.id`;
  - confirmer que le runtime global a ete reoriente vers les lots `N` source `questions`.
- [x] Correctif livre:
  - remplacer la liste BO par une lecture de `questions WHERE id_lot=0`;
  - afficher les champs papier source en lecture seule dans le formulaire;
  - sauvegarder uniquement `question_numerique`, `reponse_numerique`, `commentaire_numerique`, `statut_numerique` dans `questions`;
  - editer les distracteurs communs dans `questions_propositions`, sans colonne de typage papier/numerique;
  - refuser `certified` sans distracteur exploitable distinct, en laissant `question_numerique` et `reponse_numerique` optionnelles avec fallback papier;
  - creer les lots `N` de test avec des IDs `questions` certifies et `question_source='questions'`;
  - ajouter un export CSV read-only de controle.
- [x] Garde-fous:
  - aucune lecture/ecriture SQL vers `questions_numeriques` dans le BO;
  - aucune creation de table legacy durable;
  - aucun branchement quick, WS, scoring, front joueur ou session automatique.
- [x] Verification locale:
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_module_parametres.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_functions.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_list.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_form.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_script.php` OK.
- [ ] Verification recette serveur:
  - controler si des lignes residuelles existent dans `questions_numeriques` et les exporter si besoin avant abandon;
  - appliquer la migration cible sans creation de `questions_numeriques`;
  - ouvrir `?t=jeux&m=questions_numeriques&p=list`;
  - editer une variante numerique, certifier une question avec proposition valide, puis creer un lot `N` de test.

## PATCH 2026-06-24 - BO Questions numériques: commentaire et dates source
- [x] Audit:
  - localiser le tableau candidates et le formulaire dans `web/bo/www/modules/jeux/questions_numeriques`;
  - confirmer que `Utilisée dans T` venait de `questions_lots_temp.question_ids`;
  - confirmer que `questions.commentaire`, `questions.jour_associe` et `questions.jour_associe_v1` existent au schema source;
  - confirmer que `jour_associe` est edite en source via une saisie `jj/mm` puis stocke en `MM-DD`;
  - confirmer qu'aucun usage PHP source probant de `jour_associe_v1` n'a ete trouve hors lecture/portage/import.
- [x] Correctif livre:
  - afficher `commentaire` source dans le tableau candidates;
  - remplacer la colonne `Utilisée dans T` par `jour_associe` et `jour_associe_v1`;
  - afficher les champs numeriques deja adaptes dans la ligne candidates quand `Stock N` existe, pour aligner vue et modification;
  - resserrer le tableau candidates a la largeur de page et garder l'action visible;
  - pre-remplir et sauvegarder `commentaire`, `jour_associe` et `jour_associe_v1` dans le formulaire numerique, avec `jour_associe_v1` conserve en champ masque cote UI;
  - conserver `commentaire_interne` comme champ separe;
  - ecrire les colonnes additifs seulement si elles existent dans `questions_numeriques`.
- [ ] Verification recette serveur:
  - appliquer/revoir la migration additive `questions_numeriques.commentaire` et `questions_numeriques.jour_associe_v1`;
  - adapter une candidate avec commentaire et deux dates non vides;
  - rouvrir la fiche numerique et verifier la persistance brute des valeurs.

## PATCH 2026-06-24 - BO Jeux: Questions numériques
- [x] Audit:
  - consulter `START.md`, `SITEMAP.txt`, `DOCS_MANIFEST.md`, la carte repo `www` et le journal AI Studio raw avant patch;
  - identifier le menu BO `Jeux` et ses modules existants `playlists` / `series`;
  - confirmer que la documentation ne decrivait pas encore de module BO `questions_numeriques`.
- [x] Correctif livre:
  - ajouter l'entree `Jeux > Questions numériques`;
  - creer un module BO dedie `questions_numeriques`;
  - remplacer la source exclusive `questions_lots_temp.question_ids` par la source principale `questions WHERE id_lot=0`;
  - exclure les lots catalogue `L` via `questions.id_lot=0`;
  - conserver l'usage dans les lots `T` comme indication secondaire uniquement, calculee pour les lignes affichees;
  - paginer la liste candidates cote SQL, avec 50 lignes maximum par page;
  - ajouter des filtres serveur: recherche question/reponse, statut calcule, presence dans le stock `questions_numeriques`;
  - compter le total global `id_lot=0`, le total deja adapte et le total filtre par requete SQL;
  - limiter les compteurs avances et exemples representatifs a la page courante;
  - permettre la creation/edition manuelle dans `questions_numeriques` avec statuts `draft`, `reviewed`, `certified`, `rejected`;
  - refuser `certified` si la question n'a pas au moins une mauvaise proposition distincte.
- [x] Action test lot `N`:
  - ajouter des cases a cocher sur les lignes `Stock questions numériques`;
  - rendre seulement les questions `certified` selectionnables;
  - ajouter l'action BO `mode=create_test_lot`;
  - revalider cote serveur que chaque question selectionnee est `certified` et respecte le minimum QCM;
  - creer `questions_lots_num_temp.question_ids` avec les ids `questions_numeriques`, pas les ids sources `questions`;
  - utiliser `source_generation=bo_test` et un statut compatible `draft`;
  - afficher le token `N{id}` et le rappel d'usage sur session de test uniquement.
- [x] Garde-fous:
  - aucune generation automatique de lots `N`;
  - aucun import massif;
  - aucun branchement quick numerique;
  - aucune creation automatique de session;
  - aucune modification WS, scoring, front joueur ou papier.
- [x] Verification locale:
  - `php -l web/bo/bo.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_module_parametres.php` OK;
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_functions.php` OK;
  ...
  - `php -l web/bo/www/modules/jeux/questions_numeriques/bo_questions_numeriques_script.php` OK.
- [ ] Verification recette serveur:
  - appliquer la migration candidate si elle n'est pas deja en base;
  - verifier l'existence de `questions_numeriques`;
  - ouvrir `?t=jeux&m=questions_numeriques&p=list`;
  - verifier que la pagination reste bornee a 50 candidates et que les filtres ne chargent pas toute la table;
  - comparer les compteurs de page BO avec le dry-run CLI global sur le meme `--limit`, `--offset`, `--status` et `--search`;
  - selectionner quelques questions `certified`, creer un lot de test et verifier le token `N{id}`;
  - adapter manuellement une candidate, sauvegarder en `draft`, puis certifier seulement apres controle du QCM.

## PATCH 2026-06-24 - BO activation commerciale: traitement temporaire et commentaires longs
- [x] Audit:
  - identifier le bloc home `Commerce / Activation commerciale` dans `web/bo/www/modules/syntheses/resumes/bo_resumes_list.php`;
  - identifier le bloc fiche client `Suivi commercial` dans `web/bo/www/modules/entites/clients/bo_clients_view.php`;
  - constater que le suivi existant possede deja les champs de masquage/rappel mais que la regle de masquage etait trop globale pour des comptes traites temporairement;
  - constater que les commentaires etaient rendus via un extrait fixe sans acces au texte complet.
- [x] Correctif:
  - ajouter le statut `Traité pour le moment`;
  - rattacher le suivi a la categorie Activation commerciale ouverte depuis la home;
  - masquer le compte du bloc tant que la categorie reste traitee et qu'aucun nouveau signal metier posterieur au suivi n'est detecte;
  - conserver le retour des rappels commerciaux a echeance;
  - afficher un lien `Lire la suite` sur les commentaires longs du suivi commercial;
  - permettre la modification inline d'une ligne de suivi existante depuis la fiche compte.
- [x] Verification locale:
  - `php -l web/bo/www/modules/entites/clients/bo_clients_activation_commerciale_functions.php` OK;
  - `php -l web/bo/www/modules/entites/clients/bo_clients_view.php` OK;
  - `php -l web/bo/www/modules/entites/clients/bo_clients_script.php` OK;
  - `php -l web/bo/www/modules/syntheses/resumes/bo_resumes_list.php` OK.

## PATCH 2026-06-24 - BO offres clients: badges essai/offert homogenes
- [x] Audit:
  - comparer le rendu local `Dernieres offres clients` de la home BO avec le listing `ecommerce/offres_clients` et le bloc `Offres` de fiche client;
  - identifier que la home ajoute `Offert` localement et calcule `Essai` sans exclure `flag_offert=1`;
  - identifier que le helper commun `bo_offres_clients_price_with_discount_html_get(...)` n'affichait pas `Offert` et que sa detection `Essai` excluait les offres offertes.
- [x] Correctif:
  - aligner la detection essai du helper commun sur la home pour les offres offertes;
  - ajouter un badge `Offert` dans le rendu prix commun, en lisant le detail complet de l'offre client;
  - couvrir le listing BO et le bloc `Offres` de la fiche client sans dupliquer la logique de badges.
- [x] Verification locale:
  - `php -l web/bo/www/modules/ecommerce/offres_clients/bo_offres_clients_functions.php` OK.

## PATCH 2026-06-23 - FO page evenement: structure et sessions liees
- [x] Correctif:
  - rapprocher la page evenement de la structure page lieu: titre et accroche sous fil d'ariane, metas principales au-dessus du visuel, infos pratiques a droite;
  - conserver le format visuel evenement `5/2`;
  - limiter le panneau infos pratiques a sa hauteur de contenu pour qu'il s'arrete au niveau du visuel;
  - placer la description longue sous la ligne visuel/panneau pour qu'elle s'etende sur toute la largeur;
  - retirer la mention `Evenement !` avant le nom d'evenement sur les pages/cartes de sessions liees.
- [x] Verification locale:
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_list_bloc.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK.

## PATCH 2026-06-23 - FO evenements: wording nom genere
- [x] Correctif:
  - afficher `Evenement du ...` au lieu de `Evenement Cotton du ...` pour les noms generes historiques;
  - appliquer la normalisation sur page evenement, SEO, carte evenement de page lieu et liens/cartes sessions;
  - ne pas modifier les noms personnalises ni les donnees en base.
- [x] Verification locale:
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK;
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_seo.php` OK;
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_list_bloc.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_list_bloc.php` OK.

## PATCH 2026-06-22 - FO session: wording participation compte / QR
- [x] Audit:
  - CTA principal `Je participe` localise dans `web/fo/modules/jeux/sessions/fr/fo_sessions_view.php`;
  - cartes agenda/evenement relues via `fo_sessions_list_bloc.php`;
  - contextes QR/direct `J'accède au jeu` identifies dans les cartes et la page place.
- [x] Correctif:
  - ajouter avant `Je participe` une phrase expliquant l'inscription pour prevenir l'organisateur, retrouver la session le jour J depuis le compte et rejoindre aussi avec le QR code sur place;
  - conserver le libelle CTA `Je participe`;
  - ne pas modifier les routes, cartes evenement/place, entree QR directe, compteurs ni ecritures de participations probables.
- [x] Verification locale:
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK.
- [ ] Verification recette serveur:
  - fiche session publique future avec CTA `Je participe`;
  - page evenement et page place pour confirmer que les cartes gardent leur comportement existant;
  - entree QR code/directe pour confirmer que `J'accède au jeu` reste prioritaire.

## PATCH 2026-06-21 - FO place agenda AJAX follow-up lourd
- [x] Audit endpoint:
  - endpoint Agenda confirme: `fo/fo_ajax.php?t=entites&m=clients&p=view&section=agenda&id_client=...`;
  - `fo_clients_view_ajax.php` retourne avant `summary`, `overview`, classements, stats, archives et resultats pour `section=agenda`;
  - lenteur percue rattachee aux appels initiaux paralleles `overview`/`summary` quand l'onglet Agenda est actif, plus au helper Agenda encore base sur `app_sessions_get_liste(...)`.
- [x] Correctif:
  - charger d'abord l'Agenda quand il est l'onglet initial, puis lancer `summary` apres son rendu pour conserver les stats visibles sur la page;
  - ne plus precharger `overview` au premier rendu quand l'onglet initial est `Agenda` ou `Sessions passees`;
  - charger `overview` seulement a l'ouverture de l'onglet `Classements`, avec gardes anti-doublons;
  - remplacer, pour les tranches Agenda par dates, l'appel `app_sessions_get_liste(...)` par une requete legere sur `championnats_sessions` + client + type produit + evenement;
  - conserver le filtrage archive du jour, l'exclusion demo, les series Quiz, le regroupement evenement gamification, le CTA QR code et `Afficher plus`.
- [x] Verification locale:
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_ajax.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - comparer les requetes reseau au chargement initial d'une page dont l'onglet actif est Agenda;
  - verifier Agenda avec peu de sessions futures mais gros historique;
  - verifier compte gamification avec sessions rattachees a evenement;
  - verifier entree QR code lieu et CTA `J'accède au jeu`;
  - verifier ouverture tardive de l'onglet Classements.

## PATCH 2026-06-21 - FO place agenda AJAX performance
- [x] Audit:
  - consulter `START.md`, `SITEMAP.txt`, `DOCS_MANIFEST.md`, la carte repo `www` et le journal AI Studio raw avant patch;
  - identifier `fo_clients_view_ajax.php` comme endpoint AJAX de l'onglet Agenda;
  - identifier `fo_clients_view_upcoming_sessions_get(...)` et `fo_clients_view_sessions_html_get(...)` comme chemin de donnees/rendu;
  - constater que le chargement initial pouvait recuperer toutes les sessions futures du lieu.
- [x] Correctif:
  - charger d'abord des dates distinctes a venir bornees;
  - charger ensuite uniquement les sessions des dates retenues;
  - conserver une date supplementaire pour detecter `Afficher plus`;
  - ajouter un append AJAX Agenda sans modifier le rendu des cartes existantes;
  - conserver le regroupement evenement gamification, les cartes session non rattachees, l'exclusion demo et le CTA QR code.
- [x] Verification locale:
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_ajax.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view_shared.php` OK;
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view.php` OK;
  - `git -C /home/romain/Cotton/www diff --check` OK.
- [ ] Verification recette serveur:
  - page publique lieu avec agenda a venir et bouton `Afficher plus` si plusieurs dates;
  - page publique lieu sans agenda a venir;
  - page publique lieu gamification avec sessions rattachees a des evenements;
  - entree QR code lieu avec CTA direct `J'accède au jeu`.

## PATCH 2026-06-19 - Gamification: priorité page événement
- [x] Page événement:
  - retirer les liens vers `/fr/place/{slug}` du breadcrumb et de la ligne `Organisé par`;
  - afficher `operations_evenements.naming_nom` a la place du compte quand il est renseigne, sinon garder le nom du compte comme information non cliquable;
  - ajouter sous les metas une ligne pratique lieu/adresse, avec fallback nom `naming_lieu`, puis `naming_nom`, puis compte, et adresse evenement prioritaire puis fallback compte;
  - forcer la couleur lisible et le `margin-bottom` de l'accroche courte;
  - afficher sur les cartes sessions l'adresse événement si disponible, sinon l'adresse compte;
  - ne pas ajouter de lien vers la page compte dans les cartes sessions.
- [x] Page session événement:
  - remplacer le bloc `Organisateur` par `Lieu` en contexte session rattachee a un evenement gamification;
  - utiliser nom `lieu evenement > organisateur evenement > compte` et adresse evenement en priorité, puis fallback compte;
  - supprimer le lien vers page place dans ce bloc;
  - remplacer `Autres sessions à venir chez cet organisateur` par `Autres sessions de cet événement`;
  - filtrer les autres sessions sur le même `id_operation_evenement`, hors session courante.
- [x] Garde-fous:
  - conserver les liens place des sessions dynamisation classiques;
  - ne pas modifier les cartes sessions elles-mêmes hors infos de lieu/adresse;
  - ne pas supprimer la page compte/place.
- [x] Verification locale:
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK;
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_list_bloc.php` OK.
- [ ] Verification recette serveur:
  - page evenement gamification avec adresse evenement;
  - page evenement gamification sans adresse evenement, fallback compte;
  - fiche session rattachee a un evenement avec plusieurs sessions;
  - fiche session rattachee a un evenement quand d'autres evenements existent pour le meme compte;
  - fiche session dynamisation classique.

## PATCH 2026-06-19 - FO evenement: visuel bannière 1200x480
- [x] Correctif:
  - appliquer au visuel hero evenement un conteneur local au ratio `5 / 2`;
  - ne plus etirer le visuel pour suivre la hauteur du texte;
  - conserver le fallback visuel existant quand le branding evenement ne fournit pas de visuel;
  - deplacer `Organisé par / date / lien externe` au-dessus du titre;
  - supprimer l'ancien rendu de cette ligne sous la description;
  - conserver le lien compte vers `/fr/place/{slug}` quand disponible;
  - ne pas modifier les cartes sessions ni les pages place/session classiques.
- [x] Verification locale:
  - `php -l web/fo/modules/operations/evenements/fr/fo_evenements_view.php` OK.
- [ ] Verification recette serveur:
  - evenement avec visuel `1200 × 480`;
  - evenement sans visuel personnalise / fallback;
  - mobile avec ligne meta sur plusieurs lignes;
  - session rattachee a un evenement et pages place dynamisation/gamification.

## PATCH 2026-06-19 - FO place/evenement/session gamification
- [x] Page place:
  - l'onglet `Agenda` passe en premiere position et devient actif seulement si le compte a au moins une date a venir affichable;
  - les comptes gamification regroupent dans l'agenda les sessions rattachees a un evenement sous une carte evenement cliquable;
  - les sessions sans evenement conservent les cartes session existantes.
- [x] Page evenement:
  - breadcrumb remplace par `{compte} > {evenement}`, avec lien vers `/fr/place/{slug}` si le slug compte existe;
  - hero reorganise en visuel evenement a gauche et informations principales a droite;
  - cartes sessions conservees avec le visuel jeu/theme de chaque session;
  - badge `Evenement !` masque sur les cartes sessions dans le contexte de la page evenement.
- [x] Page session:
  - une session rattachee a un evenement utilise le visuel jeu/theme comme visuel principal;
  - le bloc `Organisateur` affiche le compte porteur et le rend cliquable vers `/fr/place/{slug}` quand le slug existe.
- [x] Verification:
  - `php -l` OK sur les 6 fichiers PHP modifies;
  - `git diff --check` OK sur `www`.

## PATCH 2026-06-18 - Sessions: liens place gamification
- [x] Correctif:
  - ne plus desactiver les liens vers `/place/{slug}` depuis les pages de sessions pour les comptes gamification;
  - activer le lien place des qu'un `seo_slug` client existe;
  - conserver le libelle `Organisateur` pour les comptes gamification;
  - afficher `Autres sessions à venir chez cet organisateur` pour les autres sessions des comptes gamification;
  - supprimer le fil d'Ariane des fiches session pour les comptes gamification;
  - masquer la note de reservation etablissement des fiches session pour les comptes gamification, quel que soit le jeu.
- [x] Page place:
  - pour les comptes gamification, retirer le lien breadcrumb parent `Établissements` et ne conserver que le nom du compte.
- [x] Verification:
  - `php -l web/fo/modules/jeux/sessions/fr/fo_sessions_view.php` OK.
  - `php -l web/fo/modules/entites/clients/fr/fo_clients_view.php` OK.
