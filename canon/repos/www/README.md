...
- resultat livre:
  ...
  - la LP les lit en priorite sur les couleurs issues du design reseau, puis conserve les fallbacks Cotton existants si elles sont vides;
...
- ecrans concernes:
  ...
  - `/lp/operation/{slug}`
- resultat livre:
  - le hero affiche un co-branding discret `[logo partenaire] x [logo Cotton]` en petites pastilles separees quand le branding TdR/reseau fournit un logo; sans logo partenaire, Cotton reste affiche seul mais dans un format reduit;
  - le bandeau 3 arguments utilise un fond plus doux, derive en teinte claire de la couleur de fond hero quand un design reseau existe, sinon blanc/neutre;
  ...
  - le bloc contexte rendu sert d'explication d'operation/invitation partenaire, avec label `Invitation partenaire`, carte claire, accent couleur reseau, logo ou visuel disponible;
  - les accents reseau alimentent les titres et numeros du mode d'emploi avec fallbacks lisibles;
...
- resultat livre:
  ...
  - sans couleurs/visuel reseau, la LP reprend les fallbacks visuels historiques: hero `bg-color-3`, CTA `btn-color-5`, sections 2/8 violettes et image generique `offre-essai/section-01.jpg`;
  - le badge hero utilise la couleur secondaire reseau avec contraste automatique quand un design reseau existe;
...
- resultat livre:
  ...
  - la landing lit le compte TdR et son branding reseau en lecture seule, sans session PRO et sans ecriture;
  - la priorite de hero est maintenant: `visuel` du design reseau, puis fallback Cotton;
