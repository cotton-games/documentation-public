## Update 2026-09-04 - Rôle BO dans le futur changement d'offre

Le futur changement d'offre des clients en propre suit le contrat `canon/interfaces/ecommerce-offer-change.md`: le BO observe l'intention Cotton persistée et la réalité Stripe synchronisée sans déduire l'offre commerciale d'un Price. Le Customer Portal n'est pas un writer commercial canonique. Les offres déléguées hors cadre restent hors périmètre.

## Update 2026-09-04 - Période de facturation Stripe dans le BO
...
## Update 2026-07-27 - BO ecommerce: remises decimales et factures

Le BO ecommerce doit lire les montants de facture stockes sans reinterpretation du taux de remise. L'incident de juillet 2026 sur les remises decimales a ete corrige par schema et donnees, sans patch runtime BO.

Points de reference:
- `ecommerce_offres_to_clients` est le snapshot contractuel de l'offre client;
- `remise_pourcentage` y est decimal (`DECIMAL(5,2) UNSIGNED`) pour conserver des taux comme `28,61 %`;
- les PDF BO/client et listings doivent afficher les montants stockes dans `ecommerce_commandes` / `ecommerce_commandes_lignes`;
- le PDF affiche la TVA par difference entre `total_ttc` et `total_ht`, ce qui donne `9,98` quand le snapshot conserve `28,61 %` et que le TTC stocke est `59,88`.

Fichiers de reference:
- PDF BO facture: `www/web/bo/www/modules/ecommerce/factures/bo_factures_view_pdf.php`;
- listing factures/commandes BO: `www/web/bo/www/modules/ecommerce/factures/bo_factures_list.php`, `www/web/bo/www/modules/ecommerce/commandes/bo_commandes_list.php`;
- schema canon: `documentation/dev_cotton_global_0.sql`.
