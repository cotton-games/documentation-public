<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-www-readme" owner="codex" -->
...
Le filtre de comptes BO conserve uniquement l’exclusion des sièges : les affiliés non-sièges deviennent visibles, ciblables manuellement, en masse et automatiquement selon pipeline/typologie. Le BO ne qualifie aucune future commande : le runtime détermine si l’acquisition est propre. Les conditions de date, d’offre et le matching des critères restent inchangés.
...
<!-- AUTO-UPDATE:END id="remises-affilies-20260910-www-readme" -->

## Update 2026-09-09 — Réconciliation PROD sur Hub

Le fichier `web/.htaccess` de Hub conserve les destinations et chemins temporaires des portails PROD, et ajoute la route `/fr/soirees/<token>` vers `/fo/fo.php?t=operations&m=hubs&l=fr&p=view&hub_token=<token>`. La LP `/lp/fr/coup-denvoi-saison` reste dans la configuration serveur extérieure confirmée par l’utilisateur : aucune règle LP locale ajoutée ni configuration extérieure modifiée.

Application locale non commitée, non déployée ; [preuves](../../../tmp/hub-prod-code-2026-09-09/reconciliation/RECONCILIATION-PROD.md).

## Update 2026-09-03 - Rôle BO dans le futur changement d'offre

Le futur changement d'offre des clients en propre suit le contrat `canon/interfaces/ecommerce-offer-change.md`: le BO observe l'intention Cotton persistée et la réalité Stripe synchronisée sans déduire l'offre commerciale d'un Price. Le Customer Portal n'est pas un writer commercial canonique. Les offres déléguées hors cadre restent hors périmètre et leurs écrans demeurent en lecture du snapshot contractuel.

## Update 2026-09-03 - Période de facturation Stripe dans le BO

Le BO affiche maintenant la période Stripe persistée des offres qui portent une subscription, y compris dans les listes de contrats réseau. Une période manquante conserve temporairement les dates legacy et rend le fallback visible; une offre sans Stripe reste intégralement legacy.

La resynchronisation unitaire et le backfill par lots exposés temporairement au super-administrateur ont été retirés après initialisation des périodes existantes. Aucun endpoint, bouton ou traitement manuel Stripe Period ne reste exposé dans le BO. La carte de période, la valeur synchronisée, l'avertissement fallback et les lectures réseau canoniques restent inchangés. Un éventuel futur outil de réparation devra réutiliser les helpers Global permanents sans être exposé en continu.

## Update 2026-07-28 - FO Place: classement saisonnier agrégé

L'onglet `Classements` des pages publiques Place rend maintenant le classement saisonnier agrégé fourni par Global (`aggregate_ranking`) au lieu de tableaux séparés par jeu.

Le tableau conserve la structure publique existante mais remplace les colonnes visibles de score par `X victoire(s) · Y podium(s) · Z partie(s)`. Le score agrégé reste technique dans Global et n'est plus rendu dans l'HTML visible de ce classement Place. Les identités suivent le contrat Global: fusion seulement sur joueur Play/Global fiable, jamais sur pseudo. Les fiches publiques de résultats de partie continuent d'afficher les scores propres au jeu.

Addendum 2026-07-29: le rendu agrégé conserve le podium Place au-dessus du tableau en projetant les rangs 1 à 3 du `aggregate_ranking` dans le renderer podium FO existant. Le podium ne recalcule pas les scores, rangs ou compteurs; il réutilise strictement les lignes agrégées Global.
...
## Update 2026-09-03 - BO réseau en lecture de la révision commerciale

Les écrans BO `reseau_contrats` et `offres_clients` affichent désormais l’offre client effective et son snapshot stocké (`prix_reference_ht`, `prix_ht`, `remise_*`, jauge, fréquence), sans recalcul catalogue ni write Stripe. Le BO conserve l’activation des offres incluses au cadre mais ne crée ni ne termine directement une offre hors cadre. La désaffiliation est refusée jusqu’à la fin effective de l’offre et de la subscription Stripe; `cancel_at_period_end` reste donc actif jusqu’à échéance.

## Update 2026-07-17 - FO Hub public: contexte et programme

Les pages publiques Hub soirée/événement répondent dès que le Hub existe avec au moins une session liée. La publication de la session dans l'agenda global `/fr/agenda` n'est pas un prérequis; `/fr/agenda` reste hors périmètre de cette passe.

Le programme de la page Hub réutilise strictement les cartes de session de la page événement, visuel inclus, et chaque carte peut ouvrir le détail de session. La fiche publique de session affiche alors le contexte Hub avant le lieu: nom de la soirée ou de l'événement, puis lien de retour vers la page Hub.

Depuis une page Hub soirée, le lieu reste accessible si le lieu public existe. Depuis une page Hub événement, la page lieu n'est pas exposée quand le compte n'est pas publié. Les pages lieu `/fr/place/{slug}` regroupent les sessions à venir d'une soirée Hub dans une carte soirée unique; elles ne retombent plus sur les sessions unitaires du même Hub.

Les lots Hub restent exclus du rendu public pour le moment, même si le resolver métier les expose pour les autres surfaces.

## Update 2026-07-17 - FO Hub public soirée/événement

Le site public possède maintenant une route Hub stable `/fr/soirees/{hub_token}`. Le token est `games_hubs.id_securite`; l'ID SQL du Hub n'est pas exposé. La vue refuse les Hubs non publiables avec une réponse 404.

La vue commune `operations/hubs` consomme `app_games_hub_public_resolve(...)` et rend le visuel principal, le titre public, la date, les horaires, le lieu ou l'organisateur, l'adresse, la description, le lien externe éventuel et le programme des sessions. Le programme réutilise les cartes de session de la page événement, avec accès au détail de chaque session. Les lots Hub restent dans le contrat métier mais ne sont pas affichés publiquement pour le moment.

L'ancienne page publique événement `/fr/evenements/{slug}` reste compatible. Quand l'événement possède un Hub publiable, elle réutilise la même vue commune; sinon elle conserve son rendu legacy. Pour les événements, l'URL canonique reste l'URL historique `/fr/evenements/{slug}` afin d'éviter deux pages indexables concurrentes.

Les pages lieu `/fr/place/{slug}` regroupent désormais les sessions rattachées à un Hub soirée publiable en une seule carte Hub. Une soirée avec plusieurs sessions produit donc une carte soirée, et non plusieurs cartes session. Les sessions autonomes, sans Hub résolu, restent rendues comme avant.

Contrainte de cette passe: l'agenda public global `/fr/agenda` n'a pas été modifié.

Fichiers de référence:
- `www/web/.htaccess`;
- `www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php`;
- `www/web/fo/modules/operations/hubs/fr/fo_hubs_seo.php`;
- `www/web/fo/modules/operations/hubs/fr/fo_hubs_view.php`;
- `www/web/fo/modules/operations/hubs/fr/fo_hubs_view_shared.php`;
- `www/web/fo/modules/operations/evenements/fr/fo_evenements_seo.php`;
- `www/web/fo/modules/operations/evenements/fr/fo_evenements_view.php`.

## Update 2026-07-17 - FO événement: hero branding opération

La page publique événement consomme désormais `app_games_hub_operation_branding_get_for_event(...)` pour le visuel principal si l'événement possède un Hub. La cascade visuelle est opération, réseau, compte, puis fallback Cotton. Les anciens événements sans Hub restent sur le bandeau legacy `place_bandeau_1`.

Les pages publiques propres aux sessions ne sont pas touchées par ce changement.

## Update 2026-07-06 - FO evenement/place: participants mixtes Blind Test
- Les classements publics qui passent par `fo_clients_view_leaderboards_html_get(...)` utilisent `participants` / `participants_podium` pour Blind Test.
- Le Top 10 Blind Test est donc mixte: joueurs solos + equipes runtime, dans le meme tableau.
- Les equipes runtime gardent le nom brut et le badge `ÉQUIPE`; aucun tableau `equipes` separe n'est affiche pour ce cas.
- Depuis le 2026-07-07, le badge `ÉQUIPE` est supprime si les metadonnees runtime indiquent moins de 2 membres.

## Update 2026-07-06 - FO session: equipes runtime Blind Test
- Sur une fiche publique de session terminée, les podiums et classements Blind Test consomment le contexte global normalise.
- Une ligne equipe runtime est affichee avec le nom brut et un badge compact `ÉQUIPE`.
- Les fiches session ne reconstruisent pas d'equipe: elles se bornent aux champs `participant_type=team` / `is_team` fournis par le socle.

## Update 2026-07-06 - FO communauté: equipes runtime Blind Test
- Sur les classements FO établissement, une ligne Blind Test deja identifiee comme equipe runtime (`participant_type=team`, `entry_type=team`, `type=team` ou identite `team:*`) est affichee avec son nom brut et un badge `ÉQUIPE`.
- Le suffixe public `(n)` est retire uniquement dans ce cas d'affichage.
- Le patch ne change ni la sélection des leaderboards, ni les scores, ni les rangs.

## Update 2026-07-03 - BO logs clients: secondes et CTA session
- ecran concerne: BO `Tracking > Logs clients` et blocs `Logs` client/contact.
- resultat livre:
  - le listing principal `clients_logs` affiche l'heure avec secondes pour lever les ambiguites entre logs d'une meme minute;
  - les blocs `Logs` qui passent par le hook de rendu clients logs affichent aussi les secondes;
  - les logs `Session #... > CTA ... cliqué [ contexte ]` restent affiches comme `CTA session : ...`;
  - les ouvertures de pivot date ne doivent plus produire de nouveau log `CTA session`; elles restent couvertes par `Page : tunnel > start > sessions_day`.
- garde-fous:
  - aucune migration historique;
  - aucune modification des valeurs brutes `clients_logs.nom`;
  - tri principal conserve sur `date_ajout DESC, id DESC`.
- fichiers de reference:
  - `www/web/bo/master/bo_master_list.php`;
  - `www/web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php`.

## Update 2026-07-02 - BO contacts multi-comptes: home et comptes lies
- ecrans concernes: Home BO `Dernières connexions clients`, fiche contact BO.
- resultat livre:
  - le bloc Home BO lit toujours `clients_logs` avec `id_log_type=4`, donc le client affiche est le `id_client` du compte actif journalise;
  - chaque ligne de derniere connexion affiche maintenant le contact quand `id_client_contact` est renseigne;
  - le contact est affiche sur la meme ligne que le client sous la forme `Client · Contact`, sans badge multi-compte dans ce bloc compact;
  - la fiche contact ajoute dans sa partie centrale un bloc `Comptes liés` listant les clients rattaches, role du contact, pipeline, typologie, usage et derniere connexion de ce contact sur chaque compte;
  - le bloc Logs de fiche contact reste directement visible en aside et conserve Client, Action, IP et tri deterministe.
- garde-fous:
  - aucune migration SQL;
  - aucune modification auth/session;
  - aucune action `Ouvrir dans EC` ajoutee;
  - aucun usage de `online=0`.
- fichiers de reference:
  - `www/web/bo/www/modules/syntheses/resumes/bo_resumes_list.php`;
  - `www/web/bo/www/modules/entites/clients_contacts/bo_module_view_top.php`;
  - `www/web/bo/www/modules/entites/clients_contacts/bo_module_aside.php`.

## Update 2026-07-02 - BO contacts multi-comptes: logs et liens EC
- ecrans concernes: fiche client BO, fiche contact BO.
- resultat livre:
  - les liens EC temporaires generes depuis une fiche client portent maintenant le `id_client` cible;
  - le bloc `Logs` d'une fiche contact affiche la colonne `Client`, afin de lire les actions multi-comptes sans melanger visuellement les comptes;
  - le filtre de fiche client reste borne a `clients_logs.id_client`;
  - le filtre de fiche contact reste borne a `clients_logs.id_client_contact`.
- garde-fous:
  - aucune migration SQL;
  - aucune modification des valeurs brutes `clients_logs.nom`;
  - aucun usage de `online=0`.
...
## Update 2026-07-02 - BO logs clients: libelles offres et CTA session
- ecran concerne: BO `Tracking > Logs clients`.
- resultat livre:
  - les routes `ecommerce > offres > form_step_1`, `form_step_2`, `form_step_3` affichent respectivement `Choix des offres`, `Sélection de l’offre`, `Confirmation commande`;
  - les logs `Session #... > CTA ... cliqué [ contexte ]` sont affiches comme `CTA session : ...`;
  - le detail brut reste visible sous le libelle pour garder l'audit technique.
- garde-fous:
  - aucune valeur `clients_logs.nom` n'est modifiee;
  - aucun filtre `online` n'est utilise.
- fichier de reference:
  - `www/web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php`.

## Update 2026-07-02 - BO logs clients: tri deterministe
- ecran concerne: BO `Tracking > Logs clients` et blocs `Logs` des fiches client/contact/branding.
- resultat livre:
  - le listing `clients_logs` trie par defaut sur `date_ajout DESC, id DESC`;
  - le clic sur la colonne Date conserve `id` comme tie-breaker;
  - le listing principal affiche aussi l'id technique;
  - les blocs `Logs` repris sur les fiches utilisent le meme ordre deterministe;
  - les routes `first_party_onboarding`, `sessions_day`, `sessions_list`, `sessions_view`, `agenda_mode` et `step_1_game` ont des libelles BO distincts;
  - les logs de creation `Session officielle ... > ajout` sont affiches comme creation effective.
- garde-fous:
  - aucune valeur brute `clients_logs.nom` n'est modifiee;
  - aucun filtre `online` n'est utilise.
- fichiers de reference:
  - `www/web/bo/master/bo_master_list.php`;
  - `www/web/bo/www/modules/tracking/clients_logs/bo_module_parametres.php`;
  - `www/web/bo/www/modules/tracking/clients_logs/bo_clients_logs_functions.php`;
  - blocs aside clients/contact/branding.

## Update 2026-07-02 - Page evenement gamification: resultats publics
- ecran concerne: page publique `www` `/fr/evenements/{slug}`.
- resultat livre:
  - la page evenement remplace la section simple des sessions par une section a onglets proche de `place-dynamic-tabs-section`;
  - les onglets disponibles sont `Sessions à venir`, `Classements` et `Sessions terminées`, selon les contenus de l'evenement;
  - l'onglet actif priorise les sessions a venir, puis les classements s'il n'y a plus de session a venir, puis les sessions terminees;
  - les evenements gamification peuvent afficher les classements consolides des qu'au moins une session terminee porte des resultats exploitables, meme si l'evenement n'est pas encore completement termine;
  - le calcul reprend le moteur global de classements en le bornant a `championnats_sessions.id_operation_evenement`, afin de ne pas melanger les resultats saisonniers de la page place;
  - le rendu des classements reprend le helper de l'onglet `Classements` de la page place;
  - l'ordre public des blocs est Blind Test, Bingo Musical, Cotton Quiz;
  - Blind Test et Bingo Musical affichent joueurs, podium et Top 10; Cotton Quiz affiche equipes, podium et Top 10;
  - les cartes des sessions terminees de l'evenement conservent un acces a la fiche detail avec le CTA `Voir les résultats`, comme les sessions passees de la page place;
  - le compteur de sessions de la page evenement inclut maintenant Blind Test en plus de Cotton Quiz et Bingo Musical.
- garde-fous:
  - aucune nouvelle route publique ni migration SQL;
  - pas de changement sur les pages place classiques ni les fiches session classiques;
  - pas de bloc classement vide;
  - le pivot pro disposait deja du lien vers la page evenement lorsque le slug public est connu.
- fichiers de reference:
  - `www/web/fo/modules/operations/evenements/fr/fo_evenements_view.php`;
  - `www/web/fo/modules/entites/clients/fr/fo_clients_view_shared.php`;
  - `global/web/app/modules/entites/clients/app_clients_functions.php`.

## Update 2026-07-01 - CTA affiliation reseau expires
- ecran concerne: site public FO et LP reseau/operation.
- contexte:
  - les LP reseau stockent en session WWW les CTA PRO `url_pro_signup` / `url_pro_signin`;
  - si ces CTA restent sur `/utm/reseau/{slug}`, ils recreent un contexte affiliation PRO neuf a chaque clic;
  - une purge seulement cote PRO ne suffit donc pas a oublier l'affiliation apres plusieurs jours.
- resultat livre:
  - les CTA WWW vers `/utm/reseau/...` sont horodates avec `url_pro_network_context_created_at` et `url_pro_network_context_expires_at`;
  - la duree retenue est 2 heures, alignee sur PRO;
  - une session WWW pre-patch qui contient encore un CTA reseau sans echeance est neutralisee au prochain rendu FO;
  - apres expiration, le site public revient aux CTA standards au lieu de relancer `/utm/reseau/...`;
  - repasser explicitement par la LP reseau recree volontairement le CTA reseau pendant 2 heures.
- fichiers de reference:
  - `www/web/lp/lp.php`;
  - `www/web/fo/fo.php`.

## Update 2026-06-30 - BO Supports image Google Drive
- ecran concerne: BO `Jeux > Supports image Google Drive`, module technique `supports_drive`.
- resultat livre:
  - ajout d'un module dedie qui liste les questions `id_type_support=1` dont `lien_support` pointe vers Google Drive;
  - tableau avec ID, question/reponse, lien Drive cliquable et upload manuel par ligne;
  - filtre Lot: toutes / hors lot `id_lot=0` / associees a un lot;
  - export CSV global de fallback avant migration (`id_question`, `id_lot`, question, reponse, lien Google Drive), en conservant le filtre courant;
  - l'upload stocke le fichier dans `upload/quiz/images/questions/question/`, cree une ligne `medias_images`, puis remplace `questions.lien_support` par l'URL serveur absolue.
- garde-fous:
  - le traitement automatique par lots n'est plus expose dans `Questions quiz culture G`, car il peut timeouter;
  - validation image reelle via `getimagesize`, formats JPG/PNG/WebP seulement;
  - upload limite a 15 Mo, redimensionnement a 1600 px max, fichier final refuse au-dessus de 5 Mo;
  - update SQL protege par `WHERE id=... AND lien_support=<ancien lien>`.

## Update 2026-06-30 - BO logs clients: inscription sans demo
- ecran concerne: BO logs clients, module technique `tracking/clients_logs`.
- correctif:
  - le log brut `Inscription standard` est traduit en BO avec la zone `Compte` et le libelle `Inscription standard`;
  - le log brut `Inscription sans demo` est traduit en BO avec la zone `Compte` et le libelle `Inscription sans demo`;
  - le libelle existant `MAJ date session demo` reste traduit en `Session demo mise a jour`, mais il ne doit plus etre emis par `pro` sans session demo reelle.
- garde-fou:
  - pas de changement de listing sessions, agenda, first_party ou creation de demo;
  - changement limite a la lisibilite du log client.

## Update 2026-06-29 - BO Questions quiz culture G: import CSV partiel
- ecran concerne: BO `Jeux > Questions quiz culture G`, module technique `questions_numeriques`.
- correctif:
  - l'import CSV peut mettre a jour uniquement les colonnes numeriques (`question_numerique`, `reponse_numerique`, `commentaire_numerique`, `statut_numerique`, `proposition_1..4`) sur une question existante;
  - quand les champs source (`question`, `reponse`, `commentaire`, support, dates associees) sont absents du payload CSV, `bo_qn_save(...)` reprend les valeurs existantes de `questions`;
  - le verrou `question_source_required` / `answer_source_required` reste actif si un formulaire envoie explicitement une question ou reponse source vide.
- UI:
  - la liste BO affiche les questions papier et numeriques completes dans le tableau; les reponses, commentaires et propositions restent compactes.
- export CSV:
  - ajoute `lien_support` comme colonne source read-only afin que l'agent IA puisse consulter le support associe avant de proposer des distracteurs.
  - ajoute `jour_associe` comme colonne source read-only afin que l'agent IA conserve le repere calendaire dans `question_numerique` quand il existe.
- garde-fou:
  - pas de changement de format CSV attendu ni de nouvelle table; `questions` reste la source unique.
  - la vue BO, les compteurs et l'export CSV excluent les questions inactives (`id_etat!=2`) comme les questions deja liees a un lot (`id_lot!=0`);
  - la creation d'un lot `N` de test depuis le BO exige aussi des questions `statut_numerique='certified'`, `id_etat=2` et `id_lot=0`.
