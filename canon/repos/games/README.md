...
- les echecs non critiques (ex. stockage local ou lien joueur/QR) restent visibles, mais basculent en vigilance orange plutot qu'en blocage rouge.
...
- rendu UX:
  ...
  - un profil navigateur offline (`navigator.onLine === false` ou `downlink <= 0`) est libelle comme connexion Internet indisponible avec recommandation de reconnexion, pas comme simple reseau lent;
