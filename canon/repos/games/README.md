<!-- AUTO-UPDATE:BEGIN id="hub-suspend-20260910-games-readme" owner="codex" -->
...
## 24/09/2026 — Open Players V2 en DEV, correctif transport local
...
**Correctif CTA local après recette client10 :** renderer Master aligné sur le précontrôle désactivé sous autorité Hub ; opening/closing et chargement restent bloquants, open autorise le clic avec ACK serveur. Propriété disabled et classe CSS cohérentes après rafraîchissement. Quatre tests front simulés ; aucun déploiement ni nouvelle qualification navigateur.

**Actualisation après première recette DEV Hub350 / Quiz27972 :** V2 déployée par l’opérateur, défaut de transport reproduit (`INIT_UNPROVEN`, puis `CLEANUP_UNPROVEN`). Correctif local uniquement : normalisation centralisée de `proof` JSON/formulaire ou tableau PHP dans le bridge Games ; refus `HUB_PREGAME_PROOF_INVALID` des enveloppes invalides et traces sans valeurs de preuve. Tests du transport réel ajoutés ; aucun nouveau déploiement, accès DB ou qualification navigateur. Les réalisations initiales ci-dessous restent historiques.

Premier lancement officiel numérique : Open réserve une E et un slot unique Hub ; Organizer attend la preuve moteur, Player attend sans gameplay, compteur par identité/socket/E, premier Lancer durable avant gameplay, abandon confirmé sans résultat ni reset. Les reprises, papier et démo gardent leurs chemins historiques. Capacité commerciale/stock restent sous autorité Hub.

[Contrat, fichiers et vérification](../../../notes/hub-open-players-foundations-2026-09-24.md). Le correctif de transport reste non déployé et non qualifié en navigateur.
...
`hub_capacity_get(stage=admit)` réutilise le transport service-only, sans calcul commercial/stock/roster. Attente persistante du runtime officiel numérique ; fence Master fraîche et garde sessionStorage Hub/session/E1 avant continuation unique. Papier/démo gardent leur attente historique. [Fichiers, dépendances, tests et limites](../../../notes/hub-soiree-hotfix-2026-09-23.md). Les sections readiness ci-dessous décrivent la branche `hub_session_readiness`, pas ce hotfix.


## 23/09/2026 — Préparation Hub E1

**Branche `hub_session_readiness` : NON DÉPLOYÉE EN PROD, exclue du hotfix `hub_soiree`.** Les recettes DEV et tests locaux ci-dessous ne valent pas livraison PROD. Lobby Master historique, continuation automatique, preuve Player après état appliqué, API service-only hub_readiness, Remote sans abandon terminal 15 s, harnais Browser Quiz/BT/Bingo froid. [Contrat, reprise sélective, fichiers, validations et limites](../../../notes/hub-session-readiness-2026-09-23.md). Les descriptions sas_players antérieures ne constituent pas le contrat de lancement de cette branche.
...
## Runner Hub v2 — gameplay et cleanup, correctif Games seul

[Audit/contrat/livraison](../../../notes/hub-gameplay-cleanup-2026-09-22.md). Premier run réel AVANT rapporté : Hub346/session27951, 50 prêts sans réponse automatique. Correction locale : gameplay_mode off (défaut hydration) ou legacy Quiz après ready, paramètres exportés et poursuite jusqu'à endGame/Stop/budget10min. Stop sans leave/quit ; cleanup explicite après drain, identités neuves par run et relecture d'inactivité. Historique privé dans l'onglet, pas d'import automatique des anciens runs v1. 46 tests verts ; 7 fichiers Games à livrer depuis update-before, identiques update-after. Aucun fichier serveur Quiz ni marker modifié, aucun restart/déploiement effectué. Anciennes descriptions v1 ci-dessous restent l'historique.

## Hub Bot Test Runner — lot local Browser Quiz

[Contrat générique](../../interfaces/hub-bot-test-runner.md) et [livraison/recette/rollback](../../../notes/hub-test-browser-delivery-2026-09-22.md). Premier scénario hydration, Browser prepared 1–200, polling individuel 3,5 s, identité stable, readiness ACK puis state concordant ; Master et auto-start réels conservés. Résumé unique exporté par le navigateur. Legacy session reste diagnostic. Quiz AVANT livré depuis bundle indépendant : envUtils DEV et ancien hubCapacity, sans cohortes ; APRÈS même harnais + performance existante. Markers /05 et /06 préparés, non activés ; DEV reste /03 déclaré. Observateur DEV (`NODE_ENV=development` ou `HUB_TEST_ENABLED=1`), aucun Server5000/BT/Bingo migré. Tests simulés verts ; recette réelle Hub50 à faire. Les paragraphes datés antérieurs décrivent leurs lots respectifs, pas un déploiement de ce nouveau harnais.
...
Games : seuls outils Python de construction/vérification des bundles et tests locaux modifiés. Le harnais navigateur Hub reste distinct et limité à 200. Les commandes natives mesurées prennent un lot complet ; les anciens helpers navigateur à batches successifs ne pilotent pas ce protocole exclusif.
...
## Hub Master — composition du podium et scrollbar — 22/09/2026, local

Le podium agrégé du Hub Master est une couche de présentation immersive. Sur desktop, il peut recouvrir le visuel principal, le podium de session et le carrousel programme afin d’utiliser au maximum l’espace disponible. Sa composition16:9 reste intégralement visible et n’est jamais rognée. Overlay absolute ancré à `.hub-master-screen`, ancêtres intermédiaires non rognants pendant l’ouverture ; contain maximal dans l’écran avec marges, plafond natif1664px, z-index70 existant. La liste mobile sous992px reste applicable. Le confinement initial à la feature980px est remplacé.

Le carrousel programme du Hub Master reste scrollable horizontalement mais sa scrollbar native est masquée explicitement sur les navigateurs supportés (`scrollbar-width:none` et pseudo-élément WebKit). Snap, sélection et Quick Add inchangés. Patch CSS seul, non déployé ; recette visuelle multinavigateur restante. [Audit, preuves et tests](../../../notes/hub-master-podium-scrollbar-2026-09-22.md).


## Hub vide : cohérence de navigation Pro — 22/09/2026

Le retrait Master/Remote conserve le Hub techniquement existant et son contexte commercial ; aucun changement applicatif Games dans ce lot. Global/Pro masquent désormais les programmes retirés dans la navigation, y compris lorsque le Hub et les memberships ont ensuite été supprimés explicitement. La trace durable de retrait interdit leur retour legacy. Un Dashboard vide existant conserve Quick Add et son identité Hub ; un Hub déjà supprimé ne peut pas être restauré par un GET.

`hub_empty_commercial_test.php` initialise désormais sa configuration Pro de fixture ;105 contrôles locaux verts. Master/Remote gardent leur ajout explicite en fenêtre live, sans nouvelle UX. [Audit, preuves client10/Hub343 et tests](../../../notes/hub-empty-pro-navigation-2026-09-22.md).


## 22/09/2026 — Capacité Hub : cohortes fermées avant lecture
...
[Patch capacité/publication](../../../notes/hub-capacity-publication-patch-2026-09-22.md) : mutualisation WS uniquement des demandes en attente avant fetch ; aucun cache TTL/in-flight pour les nouveaux arrivants. API PHP, offre et stock Bingo inchangés. Index Set par réponse ; pas de garantie de charge5 000. Blind Test : lookup classement indexé, payloads inchangés. Markers Quiz/BT/Bingo22/09 /01 ; restart futur requis, non exécuté.



## 22/09/2026 — Chargement des dépendances du simulateur bots

`test_bots.php?ajax=session_meta` charge le bootstrap Canvas commun avant les résolveurs Quiz/Blind Test/Bingo. Les lecteurs disposent ainsi de `canvas_live_participant_rows` et des dépendances d’admission Hub ; filtres PATCH1 et contrats moteurs inchangés. Corrige les500 observés sur Hub343. Un seul fichier applicatif à livrer : `web/test_bots.php`, aucun restart WS. [Tests et recette](../../../notes/hub343-digital-blindtest-roster-audit-2026-09-22.md).


## PATCH1B — réconciliation à la reprise papier (local)

Complément recette : l’injection avant ACK moteur rencontrait `HUB_SESSION_SUSPENDED`. Grant créé avant ensure ; contexte serveur temporaire validé par Games `php/hub_lifecycle.php` pour ce seul `player_register`. Suspension gameplay conservée jusqu’au vrai ACK. **Deux fichiers applicatifs à livrer : Global `app_games_hubs_functions.php` et Games `php/hub_lifecycle.php`.**


Reprise officielle existante : ensure collectif des actifs Hub avant retour, via le pipeline canonique ; scores et départs explicites conservés. Bilan structuré complet/partiel ; retry par nouvelle reprise. Numérique/démo/terminé hors nouvelle injection. PATCH1 réalise ensuite l’hydratation sans socket joueur, avec `runtime_reconciliation=pending` dans la réponse HTTP. Aucun moteur ni marker WS modifié. [Contrat](../../interfaces/paper-roster.md) · [Rapport et rollback](../../../notes/paper-resume-patch1b-2026-09-21.md).


## 21/09/2026 — PATCH 3 association gagnante Bingo papier

Aucun runtime Games modifié. Les tests exécutent les corrections automatique/manuelle sans joueur et le message D-only Bingo jusqu’au HTML Master/Remote. L’association résolue vient désormais du roster papier serveur, facultativement ; règles de phase conservées. [Rapport PATCH3](../../../notes/bingo-paper-association-patch3-2026-09-21.md).


## 21/09/2026 — PATCH 2 scores papier et podium Hub (local)

Correction Quiz/BT : K prioritaire, baisse/zéro autoritaires, reçu HTTP atomique, ACK live corrélé et retries ordonnés, réconciliation PATCH1 et contrôle transactionnel avant podium final. Aucun succès live supposé depuis le seul HTTP. Master Hub : un podium de session sélectionné prime sur le podium général automatique, y compris1 suspendue+2 terminées et après fermeture du podium automatique ; ajout d’une session prête inutile. Pas de changement winner Bingo ni activation équipes.

[Contrat](../../interfaces/paper-score-corrections.md) · [Rapport, logs Hub341, fichiers/tests/limites](../../../notes/paper-score-patch2-2026-09-21.md). Aucun déploiement/restart ; markers Quiz/BT21/09 /04.


Complément consultation : fermer le podium général puis sélectionner une session terminée quitte immédiatement `hub_idle` pour afficher son podium, même Hub terminé ; aucun effet sur le focus/runtime. Test du vrai renderer étendu (rouge avant/vert après), régressions mobile/projection réussies.

## 21/09/2026 — PATCH 1 inscription et roster papier (local)

Provenance validée, recherche/saisie libre non exclusive sauf nom exact réservé, preload/live actif commun, bind demandé même sur `already_active`, autorité explicite des projections papier Remote/Master et clôture des HTTP anciens. Les compteurs suivent les listes acceptées ; protections équipes BT et fin Bingo conservées.
...
Correction réinscription bots Hub : current_player reprend les actifs ; après départ, nouvelle identité et nom suffixé conformément à la réservation publique des pseudos. Conflits de nom : trois tentatives maximum ; garde de jauge inchangée. Games hub_bots.js/test_bots.php, tests étendus (départ→nouvelle admission, Hub plein, collisions), note mise à jour. Local uniquement.

Complément bots Hub : saisir un token sélectionne le mode Hub et masque l’ID session. Le bouton Arrêter envoie maintenant un départ volontaire de session (Quiz/BT/Bingo), sans quitter le Hub ; transition automatique et fermeture de page restent des déconnexions. Cache du module invalidé par filemtime. Tests orchestration et vraies factories étendus ; aucun déploiement/restart.

## 2026-09-21 — Bots inscrits au Hub (local)

`test_bots.php` propose un mode Hub par token/URL, admissions guest avec identités isolées et persistantes, puis suivi du focus Quiz/Blind Test/Bingo. Arrêt conserve les places ; départ Hub explicite les libère via l’action existante. Deux fichiers Games à déployer, aucun changement WS serveur. [Contrat, usage, tests, limites et rollback](../../../notes/hub-bots-2026-09-21.md).
...
Admission vérifiée dans les adaptateurs PHP avant création runtime ; endpoint hub_capacity_get réservé au service ; preload et métadonnées Remote synchronisés. Historique left non réadmis implicitement ; sessions autonomes et démos préservées.
...
## Entrée Player session vers Hub Play — 20/09/2026, local non déployé

Pour une session membre d’un Hub, l’ancienne URL `/play/{game}/{session_token}` est un alias vers Hub Play. `player_canvas.php` résout la session et sa membership avant le preload, le branding et toute sortie HTML. La fin de la source, sa preuve de fin naturelle et sa présence au focus ne conditionnent plus le transfert. S2 terminée ou remplacée par S3 conduit à Hub Play, qui choisit le focus courant ; aucun preload ni register de S2 n’est exécuté par cette entrée.

Seul un retour Hub canonique peut ouvrir directement le Player numérique : même Hub, jeu correct, joueur actif retrouvé par son token avec ID concordant, session au focus rejoignable, exécution officielle ouverte et mapping actif complet de ce joueur/session. Un simple `hub_token`, un ID joueur isolé, un mapping `left`, un contexte obsolète ou un marqueur démo ajouté à l’URL ne suffisent pas. Les cookies seuls restent une reprise à effectuer sur Hub Play. Le retour manuel reste autorisé après réactivation effective du mapping par le Hub.

L’entrée papier reste sur Hub Play. Une membership vers un Hub inactif/en suppression conduit à la réponse contrôlée du Hub, sans création EP ni repli autonome. Les sessions réellement autonomes et les sessions démo identifiées côté serveur gardent leur Player historique ; le contexte annexe source/exécution démo est conservé. Une exception de lecture arrête le Player en 503, sans fallback register. Le lookup Global existant et ses limites de distinction absence/erreur ne sont pas modifiés.

Un `ep_connect_token` session est vérifié en lecture seule par `canvas_api_ep_link_resolve` (expiration, session et jeu), puis traduit avec `app_games_hub_player_prepare_ep_return` : pendant la fenêtre, identité Hub et nouveau jeton de retour Hub, sans finalisation ni runtime source. Les branches temporelles existantes du helper restent applicables : avant J, fiche EP Hub `pending` sans joueur officiel ; après expiration, fiche EP Hub `closed`. Un jeton invalide retourne à Hub Play avec `ep_connect_error=1`; un échec technique de traduction produit un 503 permettant de réessayer l’URL d’origine.

Fichiers applicatifs : `games/web/player_canvas.php` uniquement. Les adaptateurs, WS, Remote, pages EP/WWW, probables et anciennes routes Bingo ne sont pas modifiés. Le bridge session tardif reste présent dans les APIs historiques mais n’est plus le parcours normal d’un ancien QR Player Hub.

Validation locale : `php web/tests/hub_player_entry_test.php` exécute la branche de production avant preload, le resolver de destination et la traduction EP avec dépendances DB/runtime simulées. Cas S2→S3 pour les trois jeux, absence de focus, fin, retour canonique, paramètres insuffisants, EP valide/invalide, temporalité EP, papier, autonomie et démo. Aucune DB ni recette navigateur ; absence de doublons réels non certifiée par ce test. Les assertions Player de `hub_legacy_entry_routes_test.php` sont remplacées par cette couverture dédiée.
...
### Podium session mobile — 20/09/2026

Sur Hub Master mobile, les résultats/podium d’une session terminée sont présentés directement sous sa carte. Le podium agrégé Hub reste séparé. DOM existant déplacé selon identité session, restauré à sa position desktop dès992px ; refresh conservé ; toucher la carte terminée affiche le podium, sans CTA Résultats redondant. [Détail et tests](../../../notes/hub-mobile-session-podium-2026-09-20.md).


### Départ volontaire Player — 19/09/2026, patch local

Le marqueur navigateur est désormais `leftVoluntarily:<jeu>:<session>:<player_id canonique>`. Le bootstrap privilégie l’identité Hub validée ; le départ utilise l’identité reçue par `player/ready` avant le cache partagé. Rejoin explicite et finalisation effacent uniquement le marqueur concerné. Les anciens marqueurs anonymes `leftVoluntarily_<session>` sont supprimés sans attribution à un joueur ; les contrôles Hub serveur restent inchangés. Aucun changement DB/WS. Sept tests ciblés verts (`node --test web/tests/player_voluntary_leave_test.mjs`). Recette navigateur départ A → quitter Hub → inscription B puis retour explicite A à effectuer.


## Hub Master mobile canonique — 18/09/2026, patch local non déployé

Complément UI local du 19/09 : retour « Retour à l’espace organisateur », Quick Add centré, compteur compact unique sous QR ; masqué en découverte. Données/refresh/QR/démos inchangés. [Tests et recette](../../../notes/hub-mobile-ui-2026-09-19.md).

Hub Master est désormais la surface organisateur Hub sur desktop et mobile. Sous `<992px` : composition verticale, actions par carte issues du resolver serveur et lancement papier direct autorisé. À `>=992px` : carrousel/CTA partagé et papier Remote-only conservés. Le viewport détermine la surface courante ; aucun mode mobile n’est persisté. Le POST transmet `master_surface=mobile`, simple indication de surface, avec tous les contrôles existants du service inchangé. Prospects, démos, compteur, lifecycle/expiration/grâce, moteurs et polling conservent leurs contrats. [Implémentation, commandes et recette restante](../../../notes/hub-master-responsive-2026-09-18.md).
...
[Correctifs, preuves et limites de validation](../../../notes/remote-bootstrap-regressions-2026-09-18.md).
...
## POC expérimental Cast DEV — 17/09/2026, local non déployé

> **Branche `cast` — NON DÉPLOYÉ EN PROD au 22/09/2026.** Exclu du déploiement `hub_soiree`. [État de livraison](../../deployment-status.md).

Nouveau `/cast/dev/receiver.php`, endpoint assets gardé par `$conf['server'] === 'dev'`, sources hors webroot dans `tools/cast-dev/`. CAF persistant, vrai player Cotton inchangé via importmap ; adaptateur A/B/C avec progression/son logiciel/seek/stop et pause/reprise B, une activation TV maximum, accueil sans destruction avant C. API métier interdite dans cette importmap ; aucun bootstrap Hub/session/Remote, aucune DB. Médias `CAST_TEST_VIDEO_A/B/C` dans `web/config.cast-dev.php`, template fourni. Diagnostics JSON transmis au sender Pro ; READY ne redirige pas. **POC expérimental Cast DEV — non disponible en production.** [Détails, limites matérielles, tests, configuration et recette](../../../notes/cast-dev-poc-2026-09-17.md).


### QR joueurs serveur — 17/09/2026, patch local non déployé

Le Master officiel utilise maintenant `play_url → QRcode PHP existant → PNG mémoire → data URI → deux <img data-hub-player-qr-render> identiques`. Cette section remplace le contrat historique SVG navigateur pour ce QR uniquement. Adaptateur local dans `app_hub_view_helpers.php` : ECC M, facteur12, marge4 modules ; `php://output` évite le Content-Type image, buffers propres restaurés et signature/dimensions validées, erreur isolée. Une génération par rendu HTML Master, aucune dans Play/polls/découverte. Aucun cache applicatif ni PNG temporaire ajouté ; le cache interne historique de PHP QR Code reste inchangé.

Le prospect conserve le cadenas sans QR ni URL joueurs. En erreur serveur, lien HTML « Accès joueurs » et URL canonique complète visibles sans JS. Image cliquable seulement en DEV selon le helper existant ; le lien de secours reste disponible en officiel. Petit QR fourni dès le HTML, CSS block/largeur/max-width/height:auto avec disposition défilable de base ; géométrie moderne appliquée si grid/min/clamp sont supportés.

Le contrôleur pilote uniquement l’état : image marquée IMG, complete, dimensions naturelles et affichées positives ; écoute load/error en capture couvrant les imports de refresh. Grand révélé et validé avant masquage du petit ; échec = fermeture/restauration des classes, inert, aria-hidden et focus, sans confirmation expanded. Refresh importe l’image ou le secours serveur ; asset versionné par filemtime (`?v=`). Priorités, persistance, Remote et règles commerciales/temporaires inchangées.

CDN qrcode-generator retiré du Master uniquement. SweetAlert2 et launch_confirmation passent en defer dans cet ordre ; leurs appels sont dans les interactions. Polices et bootstrap-icons deviennent décoratifs non bloquants via media=print/onload. JS désactivé ou non parsable : HTML/titre/QR restent fournis, police système et éventuelle absence d’icônes ; aucune promesse ES5 pour navigation, lancement/test, refresh, plein écran ou Remote. Sans CDN SweetAlert, l’amélioration interactive peut attendre l’échec du chargement, pas le QR initial.

[Preuves, commandes, échecs préexistants et recette restante](../../../notes/hub-player-qr-server-2026-09-17.md).


### Masques YouTube permanents — 17/09/2026, patch local non déployé

Les masques supérieur et inférieur des supports vidéo YouTube restent visibles pendant toute la lecture afin d’éviter l’exposition d’informations YouTube susceptibles de révéler la réponse. Leur visibilité suit le wrapper média ; démarrage, pause/reprise et replay ne les rendent plus transparents. Hauteur58px, géométrie, empilement, playerVars, fond audio, visualizer et CTA manuel conservés. Aucun changement spécifique Hub ; aucune causalité Hub démontrée. Test : `node web/tests/youtube_masks_test.mjs` depuis Games. Couverture réelle des informations YT à valider visuellement en DEV.


### Ajout anticipé des joueurs papier — 17/09/2026, patch local non déployé

Hub Remote propose « Participants » avec une icône joueurs `bi-people` décorative, au même gabarit13px que celle du QR, à côté du QR lorsque le serveur trouve une officielle papier encore préparable via le sas canonique : Hub actif, hors découverte, fenêtre `before`/`open`, membre actif/configuré et lancement papier nécessitant un sas. Les terminées, supprimées et reprises d’exécution existante (dont suspendues) ne suffisent pas. Une session `running` dont le contrat autorise une recréation peut rester éligible. Le serveur choisit une référence technique déterministe sans changer focus, exécution ou routage.

La modale « Ajouter des participants » utilise le roster Hub et les actions existantes. Présentation compacte commune desktop/mobile : « N inscrits », « Inscrits aux parties de la soirée. » / « Inscrits aux parties de l’événement. » via le resolver canonique, toggle texte « Voir la liste ▾ » / « Masquer la liste ▴ ». Le sas est titré « Vérifier les participants », puis affiche « N participants » et « Ils seront intégrés à cette partie. ». Liste repliée à chaque ouverture, sans filtre ni requête au dépliage ; scroll interne borné à min(28svh,220px). Une séparation légère précède « Ajouter un participant », le champ contextuel et l’aide courte « 3 caractères minimum. Si aucun résultat n’est trouvé, tu pourras ajouter un nouvel invité. ». Titres/labels explicatifs redondants supprimés, aria-label du champ conservé. À360px, largeur336px (marges12px), champ à100% avec min-width:0, noms longs sécables. Dans les deux modales : sous « Ajouter un participant », avant le champ, texte exact « Ajoute les participants non inscrits avec le QR code pour gérer la correction et le classement des parties papier. ». Cette phrase précède le champ ; l’aide « 3 caractères minimum… » reste sous le champ dans les deux accès. Le compteur reste contextuel à la partie. Footer global : seul Fermer ; sas : Annuler et lancement existant. Limite existante de120 entrées du payload conservée. Sans Quiz papier : « Joueur » / « Rechercher un joueur ». Dès qu’un Quiz papier est présent : « Joueur / Équipe » / « Rechercher un joueur ou une équipe », lookups réunis avec badges, sans sélecteur. Clés existantes conservées ; la nature reste une présentation UI. « Nouvel invité » crée uniquement un invité Hub. Undo limité aux ajouts/réactivations de l’ouverture courante ; le sas au lancement et sa confirmation restent en place.

Les deux parcours refusent avant toute mutation une saisie libre réservée par l’historique du compte organisateur, avec le helper et la normalisation runtime : « Ce pseudo est déjà utilisé sur ton compte organisateur. Choisis un autre pseudo. » Une erreur de vérification interdit l’ajout. Sélection canonique existante préservée ; aucune fusion par pseudo. [Audit, contrat exact, tests et limites](../../../notes/hub-paper-early-roster-audit-2026-09-17.md).


### Cycle de vie des démos Hub — 17/09/2026, local non déployé

Les UX restent distinctes : prospect « Faire la démo » sur chaque carte éligible, avec ses gardes et sans confirmation client actif ; client actif test ciblé depuis la première confirmation ou les réglages, aucun CTA global. Une nouvelle action crée une nouvelle intention et copie. Un retry de préparation reprend la copie de la même intention ; une intention fermée est refusée, sans reprise de démo.

Le routage choisit exclusivement la source/exécution correspondant à la publication courante (intention et génération), jamais la première démo ouverte. Le journal `game_events/hub_demo_lifecycle` ajoute un état préparé/actif/fermé, séparé de `hub_suspend`. Sortie, pagehide, signal de déconnexion, fin ou nouvelle intention ferment le droit de routage. Les anciennes réponses ne rouvrent pas cet état. Les surfaces Master/Remote envoient une présence toutes les3s ; silence d’une surface déjà arrivée >12s ferme l’intention au prochain contrôle ; arrivée Master absente après45s expire la préparation. Ce délai borne la détection d’une perte sans signal, et ne constitue pas une reprise autorisée.

La fermeture explicite utilise le POST `demo_lifecycle` sur les routes Master/Remote existantes, accès organisateur/Remote, CSRF de session navigateur et identité source/runtime/exécution. Le navigateur pose aussi un marqueur local par exécution avant de revenir au Hub pour éviter une course avec le dernier poll ; le serveur conserve l’autorité durable. Reconnexion WS automatique désactivée uniquement pour cette surface démo fermée. Contrat officiel de suspension conservé.

Bingo : copie isolée déjà initialisée (playlist/phase, morceaux et grilles neufs) ; aucun `resetdemo` supplémentaire lors de sa préparation. Le reset historique versionné de Bingo reste intact. Les erreurs de préparation après copie conservent son identifiant diagnostique.

[Audit, fichiers, tests et limites](../../../notes/hub-demo-lifecycle-patch-2026-09-17.md).


### Test Hub ciblé — 17/09/2026, patch local non déployé

Le test cible explicitement la session officielle choisie, depuis la confirmation du premier lancement ou « Paramétrer la partie ». Les CTA permanents globaux sont supprimés. Dans les réglages, disponible tant que la session ciblée n’a pas démarré, même si une autre officielle a joué, y compris avant ouverture sous réserve des droits et gardes canoniques. La confirmation de lancement dépend uniquement des officielles démarrées encore membres actifs : supprimer la dernière de ces sessions peut réactiver la confirmation. La trace historique QR n’entre pas dans ces deux décisions. Une session papier peut être testée sur **Master et Remote** : la démo reste numérique, limitée à deux joueurs. Le lancement officiel papier conserve ses surfaces habituelles.

La source et son contenu sont relus côté serveur : playlist Blind Test/Bingo et séries Quiz courantes, sans proposition indépendante pour une source compatible. Seul le Quiz papier incompatible conserve la substitution existante. Readiness, routage, reset et branding suivent le contrat démo existant. Une modale stale ne peut contourner la revalidation serveur. Master accepte les états de présentation `before`, `today` et `open` pour afficher le test ; `today` n’est pas un refus métier.

Réglages : contexte jeu/format/contenu et aperçu des séries, test secondaire, remplacement tertiaire, options, suppression danger ; footer Annuler/Enregistrer uniquement. Cette section remplace les descriptions historiques du test global ou du clic indirect sur son bouton.

[Audit, fichiers, tests et réserves](../../../notes/hub-session-targeted-test-2026-09-17.md).


## Contrat de surface individuelle Pro — clarification du16/09/2026

Le garde des favoris nus dans `app_orga_ajax.php` (commit `71eb3c71`, 08/09) exclut déjà `master_view=individual` pour une session non terminée. Pro utilise désormais explicitement cette exception pour son CTA officiel mobile ; l'intention terminale continue d'être validée séparément pour les résultats. Aucun changement de code Games, membership, exécution, guards ou routage Hub desktop. Ceci précise les anciennes formulations réservant ce paramètre aux seuls résultats. [Audit et correctif Pro local](../../../notes/dashboard-mobile-individual-master-2026-09-16.md).


### Quick Add et remplacement de contenu — 16/09/2026, local non déployé

Les réglages Quiz Master/Remote affichent les noms canoniques des séries sous l’identité, avant le CTA, sur deux lignes maximum (titre complet conservé). Réponses de remplacement et refresh/poll existants actualisent cet aperçu sans effacer les options non enregistrées. Remote respecte le `hidden` canonique de Pilotage manuel en Quiz numérique malgré son layout grid ; papier visible, autres jeux inchangés.

Quick Add : le succès d'affichage dépend de la présence de l'identifiant créé dans le Programme, même si la carte n'est pas sélectionnée. Un poll concurrent n'est pas un échec de création ; relectures bornées et observation DOM précèdent l'erreur. Après commit confirmé, les choix de création restent désactivés et « Réessayer l’affichage » ne recrée rien.

« ↻ Proposer une autre playlist » (Blind Test/Bingo) ou « ↻ Proposer d’autres séries » (Quiz), identiques aux libellés Pro, apparaissent dans le contenu de « Paramétrer la partie », avant les réglages, séparé de la suppression/footer. Éligibilité canonique Global sans critère d'origine : propriétaire, configuration/contenu compatibles, état pending sans suspension ni exécution officielle, fenêtre before/open. Préparation prospect existante conservée ; aucun changement d'offre au lancement. Les handlers HTTP existants appellent plan/apply Global ; erreurs dans la modale, anti-double-clic, options indépendantes conservées, convergence par polls existants. Global revalide sous le verrou partagé lancement/suppression. Ce contrat remplace les anciennes notes de juillet limitant le bouton carte à `quick_hub_create`.

Le view-model Master transmet explicitement le Hub au calcul d'éligibilité de cette action (`theme_renewal_hub`), comme Remote. Le payload de réglages partagé fournit `theme_renewal_label` aux deux surfaces ; gardes stale et règle métier inchangées.

[Audit, fichiers, tests et réserves](../../../notes/hub-quick-add-renewal-2026-09-16.md).

### Supprimer une partie Hub — 16/09/2026, local non déployé

Dans « Paramétrer la partie », Master et Remote proposent « Supprimer la partie » aux non-jouées, suspendues libérées et terminées. En cours : aucune action de suppression, réglages existants conservés. Confirmation applicative dédiée, Annuler/Supprimer, texte programme avant démarrage et soirée/événement via resolver existant après démarrage. Aucun CTA de suppression ajouté sur les cartes. L’action est placée en bas du contenu des réglages après un séparateur, avec corbeille et style tertiaire danger ; le footer contient uniquement Annuler/Enregistrer. La première confirmation officielle indique les joueurs actuellement inscrits à la soirée/l’événement et propose de lancer un test pour vérifier le fonctionnement ; CTA « ✅ Oui, démarrer » conservé. Le CTA secondaire « Lancer un test » est proposé seulement si le bouton Hub correspondant est disponible ; il déclenche ce bouton, sans nouveau flux de démo. Le choix test ne lance jamais l’officielle et ferme le lobby Remote papier via son cleanup existant. Les couleurs danger, titre, textes et labels de lancement sont adaptées à la surface réelle via les helpers de contraste Hub ; accent/bordure et fond du CTA branding conservés, y compris lors du refresh branding.

Master : `session_remove`, organisateur + instance courante + CSRF, service Global. Remote : même action avec accès/instance/CSRF et Master présent ; commande `remove_session`, exécution autoritaire lors du poll Master, ACK avant fermeture réussie. État revalidé au moment de la mutation. Les polls/révisions existants continuent après cutoff ; ACK ou nouvelle révision reconstruisent le programme et le classement, y compris vide. Une modale devenue stale est fermée et sa confirmation annulée. Play relit les membres actifs et le fallback de présentation existant ; aucune extension du lifecycle WS.

Après suppression du dernier membre, un Hub vide conserve son statut commercial issu de l'offre du propriétaire (`own_offer`/`network` ou `inactive`), ses joueurs et l'ajout de partie. Master/Remote ne déduisent jamais le mode découverte du nombre de sessions. Le QR réel réduit reste soumis à la politique existante (Hub/offre actifs, fenêtre `before`/`open`) ; sans partie disponible, `can_expand=false`. Les inscriptions attendent la fenêtre ouverte et Play reste en attente sans session à rejoindre. Correctif local du 16/09, non déployé.

[Validation et réserves](../../../notes/hub-session-removal-audit-2026-09-16.md).
...
Modales soirée/événement via resolver existant ; `quitGame intent=hub_suspend`, ACK métier puis retour ; endpoint interne `hub_lifecycle` réservé au service token et garde des writes HTTP. Play conserve sa navigation par perte de focus.

Correctif local du 11/09/2026 : Le Master reconstruit le payload de suspension avec les seuls champs métier ; les champs `type`/`action` de la trame Remote ne peuvent plus remplacer `quitGame`, y compris lors d’une nouvelle tentative depuis le Master.
...
<!-- AUTO-UPDATE:END id="hub-suspend-20260910-games-readme" -->

<!-- AUTO-UPDATE:BEGIN id="hub-remote-limited-20260910-canon-repos-games-readme" owner="codex" -->
...
<!-- AUTO-UPDATE:END id="hub-remote-limited-20260910-canon-repos-games-readme" -->

## Reprise numérique active — 09/09/2026, patch local Hub

`En cours` / focus actif : Reprendre rattache le Master à l’exécution officielle existante via `launch_session` et l’intention canonique `resume_existing_runtime`. L’ID est conservé, seule une nouvelle génération de routage est publiée. Exécution absente/périmée : refus sans création de secours. Le Master restaure la session commencée en Pause, sans auto-start ni validation Remote. La Remote revenue au Hub peut suivre cette nouvelle génération ; son propre Reprendre utilise déjà le service partagé. Suspendue/Relancer, papier Remote-only, démo, premiers lancements, terminaux et liens legacy restent distincts. [Audit, limites des tests et recette](../../../notes/hub-active-master-resume-2026-09-09.md). Non déployé.

## Préparation papier Hub Remote — confirmation finale et équipes Quiz (2026-09-09, local non déployé)

La carte ouvre directement la préparation lorsqu’elle porte paper-mode=1, paper-lobby-required=1 et hub-confirm-official=1, hors démo commerciale. Le dernier CTA « Lancer la partie » appelle la confirmation partagée existante, puis seulement submitLaunchSession et son intention canonique. Le dialog natif est fermé temporairement avec launching verrouillé ; annuler rouvre la préparation sans réinitialiser les inscriptions ni publier focus/exécution/routing. Les doubles clics sont bloqués pendant la confirmation. Une Remote remplacée ne lance pas après validation. Les opérations de préparation existantes restent possibles avant confirmation ; aucun changement de leur contrat, d’injection ou d’hydratation.

Uniquement dans cette modale, `cachedProgramSession(...).paper_mode === true` et `settings.game === 'quiz'` sélectionnent « équipes » : titre, aide, placeholder, compteur, états vides et messages de recherche/création/erreur. Les pseudos, IDs, payloads et tables restent inchangés. Le contexte soirée/événement vient du resolver hub_wording existant. Blind Test, Bingo et numérique gardent leurs libellés. Les autres surfaces et les reprises sans sas restent inchangées.

## Remote Hub — récupération papier et officielle numérique (2026-09-09, local non déployé)

Le front partagé Quiz/Blind Test/Bingo propose une sortie volontaire uniquement avec `HUB_REMOTE_RETURN.enabled === true` et (`SESSION_PAPER` ou `hubRemoteReturn.runtimeMode === 'official'`). `remote_canvas.php` projette `contextType` depuis le Hub déjà chargé (`event` ou `soiree`), sans requête supplémentaire.

- `PRIMARY_DISCONNECTED` papier : **Écran de diffusion déconnecté** — « Pour reprendre la partie, reprends la diffusion depuis l’ordinateur et retourne à la soirée [ou à l’événement]. »
- `remoteQuitUnavailable` papier : **Impossible de quitter** — « L’écran de diffusion n’est pas connecté. » suivi de la même aide.
- Bouton **Retour à la soirée / Retour à l’événement** ; **OK** conservé sur le refus de quit. Sans SweetAlert, confirmation native explicite. Sans tuple de retour valide, aucune sortie non gardée n’est proposée.
- Numérique officiel Hub : même bouton volontaire dans les deux notices ; même aide que le papier : « Pour reprendre la partie, reprends la diffusion depuis l’ordinateur et retourne à la soirée [ou à l’événement]. » Numérique démo ou mode absent/inconnu : aucune extension. Hors Hub : messages et comportements historiques conservés.
- Une reconnexion `GAME_RESUMED` ferme toujours la notice et réactive les commandes. Si l’utilisateur est déjà retourné au Hub, la reprise canonique peut réutiliser la même exécution avec une nouvelle génération de routage ; le retour seul ne termine pas la session. Recette réelle de cette course restante.

`HubTransition.remoteRecoveryUrl()` réutilise la destination Hub validée et `withRemoteReturnMarker`, sans transition terminale. Le tuple `execution_id + routing_generation` est porté dans l’URL (`hub_remote_returning=1`, `returned_execution_id`, `returned_routing_generation` et alias existants). Le Hub conserve sa garde au reload ; aucun stockage durable ajouté. Le poll canonique bloque uniquement le tuple quitté ; une nouvelle intention validée publie une nouvelle génération et permet la reprise, une autre exécution reste autorisée. Aucune commande quit, fin, clear focus, mutation runtime/grâce ou modification des reconnects.

## Contrat 2026-09-08 — Compatibilité de la programmation legacy (local, non déployé)

Le rattachement actif `games_hubs_sessions` fait autorité. Les consultations ne créent pas de Hub, ne réparent pas les memberships et ne normalisent aucun champ métier de `championnats_sessions`. Un programme partiellement rattaché n'est pas remplacé par son seul sous-ensemble Hub : la liste historique complète reste disponible, avec diagnostic de résolution. Sans membership, le retour legacy est conservé ; plusieurs candidats plausibles donnent une ambiguïté, jamais un choix par poids ou ID. La réparation explicite compare toutes les sessions attendues, complète les absentes sans déplacer les relations étrangères, et reste idempotente.

Les lots propres à une session restent prioritaires pour cette session et son résultat. Sans lots Hub explicites, une compatibilité commune n'est exposée que si le programme canonique complet est uniforme ; divergence ou personnalisation d'une seule session reste `session_scoped`, sans import à la lecture. Le Dashboard expose les lots divergents par partie. Le branding effectif se complète champ par champ : session, Hub explicite, opération événement, réseau, compte, fallback Cotton ; aucune copie entre sessions. Un Hub sans opération reste pris en charge.

Publication Hub et flags historiques restent distincts. Les flags privé/non publié de l'événement filtrent les listes publiques legacy, pas l'accès historique par URL directe ; aucune nouvelle ACL n'est déduite d'un flag de session. Les probables legacy des sessions membres restent lisibles et dédoublonnés, avec identité équipe distincte : aucune présence/joueur Hub n'est créée à la consultation. Une confirmation réelle dans la fenêtre existante peut matérialiser le probable Hub sans supprimer sa source legacy.

Home, Agenda, préparation Dashboard et résolution des anciennes entrées ne dépendent plus de l'ancien `id_offre_client` de la session. Les guards commerciaux des actions restent en place. Master/Play legacy rejoignent le Hub canonique hors exceptions d'exécution explicite et de consultation terminale ; Remote ne réutilise qu'un accès existant pour le propriétaire ou une session Remote déjà liée, sinon état contrôlé 409.

La stratégie reste `PRECHECK -> DDL -> backfill déterministe -> contrôles -> activation`. Les 58 Hubs / 68 memberships sont une cible future, pas une migration réalisée. Les helpers de schéma existants peuvent encore assurer leur DDL ; le contrat de non-mutation concerne les données métier historiques, pas une promesse de zéro écriture pour tout le runtime. Audit, matrice, commandes et risques : [rapport de compatibilité](../../../notes/hub-legacy-runtime-compatibility-2026-09-08.md).

## Update 2026-09-08 — Confirmation locale et reprise officielle Hub

Confirmation uniquement au premier lancement officiel du Hub, sur la seule surface initiatrice Master/Remote. Pour le papier Hub Remote, elle est désormais demandée sur « Lancer la partie » après la préparation, juste avant submitLaunchSession ; numérique et Master conservent leur emplacement précédent. `games_hub_has_official_launch()` lit la preuve persistante `games_hubs.player_qr_official_started_at`, puis les `hub_has_started` canoniques des sessions officielles déjà chargées en repli pour les anciens Hubs. Aucun calcul JS, lecture DB supplémentaire ou write QR. Dès qu’une officielle a été lancée, même terminée/suspendue, les suivantes ne confirment plus ; les démos seules ne comptent pas. Projection Remote `action.confirm_official` inchangée de forme ; reprise/relance consommée exclue ; officiel papier toujours Remote-only.

Wording Hub unique : « Démarrer la soirée maintenant ? », « ⚠️ Une fois une session officielle lancée, elle ne peut plus être redémarrée. », « Si tu souhaites d’abord vérifier le fonctionnement, tu peux lancer un test. », CTA « ✅ Oui, démarrer ». Aucune variante papier. Croix/Échap sans effet ; guards serveur et confirmation historique autonome inchangés.

`isOfficialHubOrchestrated()` consomme `hubOrganizer.enabled`, runtimeMode officiel, executionId et identité source/runtime concordante. Il exclut le boot diagnostic automatique et le QR Remote papier automatique, ainsi que le blocage prelaunch de reprise. Diagnostic manuel conservé. Dashboard Pro mobile sans exécution Hub et historique autonome conservent leurs comportements ; le masquage papier mobile historique reste intact. Aucun nouveau marqueur, poll/timer, routage ou write métier. [Audit, fichiers et recette](../../../notes/hub-launch-resume-ux-2026-09-08.md). Tests locaux verts ; réserve préexistante grand QR du contrat Remote et recette DEV restante.

## Update 2026-09-08 — QR Hub : suite jouable et réactivation

Complément UI après confirmation DEV : l'aperçu historique de la session sélectionnée (27718) ne bloque plus Agrandir si le serveur confirme une suite jouable (27731). Games marque les panneaux statiques de résultats `data-hub-podium-preview="1"` ; seuls les podiums visibles non marqués restent prioritaires. Le mode `hub_idle` masque les aperçus de session sans réécrire `presentation_session_id`. Résultats consultables, podium global explicite/runtime/transition, queue et confirmation conservés ; aucune expansion automatique. Renderer PHP et contrôleur JS Games doivent être livrés ensemble. Test avec vrais renderer carrousel + adaptateur QR, contre-épreuve ancien JS en échec ; correctif local, recette navigateur après livraison restante.

Patch local : la politique QR utilise `app_games_hub_player_qr_has_continuation()` au lieu de verrouiller l'agrandissement sur le podium historique de `presentation_session_id`. Sessions officielles canoniques `pending` ou `running` non terminées, token/jeu supportés et garde métier accepté ; exception de reprise d'exécution officielle identique au lancement. Une session suspendue reste `running` avec focus vide. Toutes terminées ou sans candidat jouable : `can_expand=false` ; ajout autorisé : réévaluation au poll existant, sans toucher au choix réduit ni à l'historique.

Les gardes commerciales/temporelles, le runtime, le mode explicite `hub_podium`, le podium prioritaire côté Master, ses réductions confirmées et la concurrence sont conservés. Master/Remote partagent la décision serveur ; la Remote conserve son attente de confirmation et ne peut couvrir un podium prioritaire affiché par le Master. Contrat détaillé : [Pilotage du QR joueurs Hub — Éligibilité réversible](../../interfaces/canvas-bridge.md#2026-09-08--éligibilité-qr-réversible-selon-le-programme-courant). Tests locaux uniquement ; recette DEV 296 et ajout après fin à effectuer, aucun accès serveur/DB existante, déploiement ou redémarrage.


## Update 2026-09-08 — Blind Test individuel, équipes en stand-by

Le Player conserve le markup équipe caché (`hidden` + `display:none`), y compris hors Hub ; seul `teamState.enabled === true` du WS autorise l’affichage et l’émission des actions équipe. La capacité reste fausse avant ACK/état explicite ; l’inscription ne demande plus spontanément `teamList`. Le booléen autoritaire se trouve dans `blindtest/web/server/features.js` et vaut false. Les rendus Master/Remote/Player d’anciens résultats équipe restent compatibles.

`blindtest_adapter_glue.php` conserve les lignes de `blindtest_session_teams` lorsqu’un snapshot final individuel n’apporte aucune équipe (`changed=false`, `skipped=true`, `reason=NO_TEAM_RANKINGS`). Les writes ordinaires `update_score`/`session_update` et les lectures historiques sont conservés. Aucun changement Bingo/Quiz ou contrat équipe Hub. [Audit/tests/limites](../../../notes/blindtest-teams-standby-2026-09-08.md).

## Update 2026-09-08 — Fin historique individuelle après démo Hub

`canvas_api_hub_session_natural_ended` vérifie désormais le start brut via `app_games_hub_execution_terminal_provenance_matches` avant toute autorité de fin : démo, focus actif, recovery après clear et replay `already_completed`. Une démo annexe ne peut pas justifier la fin officielle de sa source, même si une completion contaminée existe déjà ; `hub_execution=false` conserve l'émission/rendu historique `endGame`. QR, dashboard, inscription, lobby et WS/Play inchangés. Helper Global et bridge Games doivent être livrés ensemble (Global d'abord si séquentiel), sans redémarrage WS requis par ce patch PHP.

Les starts incomplets/anciens ne sont pas promus à officiel par les valeurs normalisées du resolver : fin historique, sans clear/completion/statistiques Hub. Une vraie exécution legacy sans métadonnées perd donc son retour Hub par ce bridge. Une ancienne officielle exacte encore ouverte, sans nouveau start ni preuve d'une autre incarnation, reste ambiguë et acceptée en recovery : ce correctif ne la distingue pas d'un nouveau passage historique sur le même token. Un nouveau start rend l'ancienne non autoritaire ; un nouveau terminal invalide l'ancienne completion. Ces comportements sont couverts par les tests de provenance/fin ; voir [audit et recette](../../../notes/hub-historical-player-end-audit-2026-09-07.md). Huit suites ciblées vertes. Recette navigateur DEV du 08/09 validée par l'utilisateur sur les trois parcours ; logs cohérents (historique 27717, officiel Hub 27718, démo Hub 27722). Réserve SQL sur le lien start/completed et la démo préalable historique 27720, détaillée dans l'audit. Aucun commit ni déploiement par l'agent ; livraison PROD du correctif à effectuer Global avant Games.

## Update 2026-09-07 — Complément UX QR prospect non scannable

Le seul QR d'inscription joueurs du Hub Master commercial découverte est remplacé par une illustration SVG statique (cadre de scan et cadenas), sans matrice, URL encodée, cible de génération, clic ni agrandissement. La carte unique réunit le badge `Mode découverte`, l'illustration, `Tes joueurs s’inscriront à la soirée en scannant ce QR code.` ou `Tes joueurs s’inscriront à l’événement en scannant ce QR code.`, puis `Inscription indisponible en mode découverte.` Le contexte provient de `games_hub_master_wording_labels_get(...).noun_with_article` : usage client prioritaire, contexte Hub en fallback, jamais le titre libre.

Le refresh retire l'ancien dialogue QR actif après fermeture accessible ; un poll retardé ne peut réactiver les contrôles ni afficher un QR en découverte. Le retour actif recrée uniquement un dialogue manquant depuis le HTML canonique ; un refresh actif conserve celui déjà ouvert. QR actif, QR télécommande découverte, QR démos historiques et guards serveur inchangés. Aucun changement de routage, DB ou API. Ce complément remplace l'ancien attendu visuel et wording prospect ci-dessous ; la clôture des trois assertions reste historique. Patch local, recette visuelle/scan DEV restante ; [preuves et validation](../../../notes/hub-launch-test-audit-2026-09-07.md#complément-ux--qr-joueurs-prospect-réellement-neutralisé--2026-09-07).

## Update 2026-09-07 - Branding Hub synchronisé avant et après inscription

Patch local, sans déploiement. Le formulaire Hub Play réutilise le poll compteur à 15 s avec `include_branding=1` ; cette option charge client/sessions/branding et expose le payload `hub_branding` existant, sans agrégats joueur. Le compte seul reste léger pour les autres appelants. Le poll inscrit `active_launched_session` conserve sa synchronisation. Aucune réécriture de saisie/identité ; réponse compteur antérieure à l’inscription ignorée pour le branding.

Le logo est un footer unique hors du panneau ready, disponible avant/après inscription, caché sans espace réservé si l’URL effective est vide. Dimensions/opacité/responsive conservés. La révision d’image hache le branding seul ; les src ne changent qu’avec le média/version ou cette révision. Master synchronise aussi le lien de police sans réassigner un visuel inchangé ; Play/Remote retirent le src supprimé et un stylesheet devenu vide. Remote garde son snapshot, ses commandes et son routage existants. Voir [audit, tests et recette](../../../notes/hub-branding-sync-2026-09-07.md).

## Update 2026-09-07 — Remote UX, Master découverte, panneau droit et grand QR
...
Patch local, sans déploiement. Remote conserve le branding partagé et ajoute une localisation non répétée au titre ; hero sans couverture, textes multiligne, vignettes canoniques de parties via `sessions[].visual_url`, rendu PHP/JS cohérent et signature de projection enrichie. Sélection, actions, horaires globaux et cadences conservés. Incompatibilité démo initiale alignée au rerendu.

Master prospect : carte `Mode découverte`, illustration cadre/cadenas non scannable et deux paragraphes contextuels selon le complément UX ci-dessus ; aucun CTA commercial. QR actif réduit : `SCANNE POUR JOUER`, avec la même classe de titre que les lots (`hub-master-panel__head` : uppercase, graisse 800, taille et contraste partagés), et liens DEV-only. La mention pédagogique prospect suit le complément UX ci-dessus ; cette règle typographique est partagée avec le grand QR central. Master conserve la colonne droite et sa largeur, avec deux zones permanentes : `Joueurs inscrits` (zéro confirmé discret dans l’en-tête ; grand compteur uniquement hors zéro) et `Classement général` (sans données : `Le classement apparaîtra ici après les premières parties.`). Le classement réel n’est jamais remplacé par un retour du compteur à zéro. Le rafraîchissement partiel existant intègre un nouveau classement, sans effacer un classement connu si la projection suivante est absente ; des payloads identiques conservent les nœuds de liste/classement. `players_count.active_count_confirmed` provient de l’argument optionnel Global `&$confirmed` ; réponse invalide/erreur conserve le dernier état, SSR inconnu affiche `—`. Compteurs Remote/Play et définition SQL inchangés. Livraison coordonnée Games/Global nécessaire.
...
Grand QR B implémenté localement après autorisation applicative : choix dans les quatre colonnes dédiées, commande `player_qr_display_set` à cible/révision explicites et clé idempotente, queue et polls existants. La confirmation Master est bornée à trois champs (`revision`, `instance_id`, `mode`), 256 octets maximum ; le serveur revalide révision, instance courante et éligibilité, sans réécrire un JSON identique. Un terminal de commande ne prouve pas l’affichage. Attente bornée à 20 secondes et terminaux existants ; anciennes réponses ignorées, commandes concurrentes protégées par comparaison atomique de révision.

Reload, réseau et remplacement d’instance conservent le choix durable et demandent une nouvelle confirmation. Initialisation toujours réduite ; restauration agrandie uniquement avec preuve d’un choix explicite à la révision courante. Lancement officiel réussi : réduction, incrément de révision et première date conservée ; refus/échec : choix conservé. Une démo réussie réduit sans dater le premier officiel. Podium effectivement engagé : réduction confirmée et validée côté serveur, même s’il disparaît entre deux polls. Au retour, aucun agrandissement automatique ; action manuelle possible selon l’affichage. Une écriture QR de lancement échouée dépose un reçu de réduction dans la queue existante, lié à la révision ; les polls la reprennent sans écraser un choix ultérieur, y compris après une deuxième partie ou une démo. Les reçus de lancement antérieurs restent des preuves de secours pour la première date officielle. Focus, présentation et routage restent intacts.

Stockage absent, partiel ou illisible : QR réduit et commandes indisponibles, sans bloquer Hub, inscriptions ou lancement ; aucun ALTER automatique. Schéma DEV déjà importé par l’utilisateur (quatre CONFORME, OK_SCHEMA_QR), migration non rejouée pendant B. Livraison coordonnée : Global avec son nouveau helper, puis Games avec son nouveau JS ; PROD reste réservé à sa migration globale. [Contrat](../../interfaces/canvas-bridge.md#pilotage-du-qr-joueurs-hub--b-local) et [runbook](../../runbooks/hub-player-qr-migration.md).
...
Sur Master, le clic applique immédiatement une intention visuelle temporaire, puis conserve le POST/queue et les révisions existants. Un clic suivant remplace cette intention sans lancer un deuxième POST concurrent ; le dernier choix est envoyé après le terminal de la commande précédente à sa révision appliquée exacte. Refus, conflit ou expiration de la demande courante réconcilient l’état serveur avec une erreur sobre. La confirmation envoyée décrit seulement le SVG appliqué à la révision persistée correspondante, jamais une intention locale sous une autre révision. Une priorité métier annule l’agrandissement temporaire afin qu’un retour rapide ne le réactive pas. Aucun nouveau poll/timer ni changement Global/SQL.

Recette visuelle et scan téléphone restent à effectuer. [Audit et preuves raw](../../../notes/hub-remote-master-ux-audit-2026-09-07.md).

## Réconciliation du retour Remote — 2026-09-06

Sur un retour valide X/G, un signal d’accès serveur doit porter sa propre exécution, une session positive et une génération canonique strictement supérieure à G pour afficher une nouvelle attente d’accès. Une autre exécution Y ne suffit pas sans génération postérieure. Le filtre s’applique même lorsque le routing vise déjà le Hub. L’attente serveur est recalculée à chaque contrôle et s’éteint quand sa cause disparaît ou que le Master revient ; l’attente d’une commande locale conserve ses terminaux existants. Ce traitement UI ne modifie ni navigation ni anti-rebond.

## Robustesse bootstrap Remote — 2026-09-06

Avant l’émission du premier `control_poll`, une erreur de bootstrap non liée au remplacement d’instance affiche un message de resynchronisation et `Recharger` (`window.location.reload()`). Les terminaux existants restent prioritaires ; une fois la première requête émise, la récupération du polling reste inchangée. Aucun timeout supplémentaire n’est ajouté. La console navigateur trace une seule fois `remote_bootstrap_start`, `takeover_resolved`, `poll_loop_started` et, en cas d’échec, `bootstrap_error` avec étape et raison. Ces traces ne sont pas envoyées aux logs serveur.

## Update 2026-09-04 - Hub Remote: routage canonique générationnel

<!-- AUTO-UPDATE:BEGIN id="routing-0409-limited-revision-20260910" owner="codex" -->
Révision 10/09 — voir le contrat courant en tête de page : présence requise pour tous les runtimes, aucune nouvelle readiness pour existing, retour automatique au TTL et réentrée même X/G. Les paragraphes historiques ci-dessous sur le blocage returned et existing sans heartbeat sont remplacés.
<!-- AUTO-UPDATE:END id="routing-0409-limited-revision-20260910" -->
...
Passe UX/UI implémentée localement le 07/09, sans déploiement : visuel intégral seul puis `Bienvenue`, h1 en flux et métadonnées ; liste compacte avec toutes les identités de rang 1 à 3 selon la sélection Master, puis joueur/voisins sans doublon et dans l’ordre canonique. `photo_src`/`photo_source` proviennent des lignes déjà enrichies, sans lecture média par ligne ; image cassée masquée sans perdre rang/pseudo. Un payload identique conserve les nœuds existants. Les lots utilisent `rank_value` et gardent #3 seul ou #1+#3. Sans lot personnalisé, les fallbacks canoniques backend sont affichés comme dans Master, y compris après suppression totale ; aucun libellé n’est recréé côté Play. Le classement vide est masqué, les lots suivent les valeurs backend même marquées `is_default`. À partir de 992px, classement et lots partagent une grille 920px majoritaire/secondaire ; une seule section occupe la largeur disponible. [Audit et livraison](../../../notes/hub-play-ux-end-to-end-audit-2026-09-06.md).

La décision pure Games `games_hub_display_presentation_resolve` compose les helpers Master de mode et de sélection. Master la consomme avec son contexte de retour naturel ; `current_player` et `active_launched_session` exposent sa projection `display_presentation`, distincte de `presentation` et des accès runtime. Les groupes complets du podium ne sont pas dupliqués dans cette projection Play. Le podium intermédiaire ne laisse plus surgir une session fallback ; idle suit la sélection Master, y compris une session terminée, et l’auto-overlay peut masquer cette sélection. Les conditions de fin, les priorités temporelles/d’accès/résultat pending et l’acquittement local Master restent inchangés. Aucun changement Global/Remote et aucune participation créée pour obtenir la présentation initiale.

L’arrivée affiche `Vérification de ton inscription…`, sans identité présumée. Après confirmation, identité/photo et sections valides apparaissent ; la carte personnelle reste masquée sous `Chargement en cours…` jusqu’au premier poll d’accès, sans ancienne carte ni fausse attente. Une erreur de revalidation conserve l’identité mémorisée non validée et affiche `Connexion interrompue. Nouvelle vérification…` : un essai initial + deux reprises, séparés de 3500ms, timeout 8000ms par essai, une requête en vol, annulation/génération pour les réponses obsolètes. Après épuisement : `Vérification indisponible pour le moment.`, sans spinner ni bouton. Seule l’absence/inactivité confirmée ouvre l’inscription. Une erreur de poll conserve les sections valides avec `Connexion interrompue. Nouvelle vérification en cours.` ; le premier contexte inconnu arrête aussi son spinner après 8s sans ajouter de poll. Les messages identiques ne sont pas réannoncés ; réduction des animations respectée. Quitter la page annule les requêtes obsolètes, le retour bfcache revalide l’identité ; cadence du watcher inchangée.

Complément du 07/09 : le bouton et sa confirmation utilisent `Quitter la soirée` / `Quitter l’événement` depuis le contexte de wording existant. [Audit quitter/réinscrire](../../../notes/hub-play-quit-rejoin-audit-2026-09-07.md) : traces Hub 287, identité 415 avant puis 416 après ; le départ Hub historique efface la preuve locale et les cookies, à la différence du départ de session. Le garde limité aux propriétaires actifs préexistait à l’UX/UI. Décision utilisateur du 07/09 appliquée : l’inscription invitée publique appelle `app_games_hub_player_register_guest_from_play`, qui réserve dans ce Hub tout pseudo normalisé déjà présent, y compris `left`. Après départ, même avec l’ancienne preuve, le pseudo est refusé (`USERNAME_TAKEN`) ; un autre pseudo libre crée une identité neuve sans historique/photo. Une identité encore active peut rejouer sa propre inscription sans mutation. Les réparations internes EP/papier/runtime gardent leur réactivation par identité ; aucune ligne existante n’est fusionnée ni supprimée.
...
Complément UX du 07/09 : Sans carte de session visible et avec classement disponible, le conteneur d’attente générique est masqué entièrement : identité → classement → lots. Carte présentée, chargement/erreur, préparation de résultat, fin et absence de classement restent inchangés. Le joueur absent du classement dispose d’une `personal_row` UI distincte des lignes canoniques : rang « — », pseudo exact et repère « Toi », style personnel existant. « 0 part. » exige `stats_parties_count=0`, `stats_computed_at` valide, champs dirty/erreur présents et vides, et dernier résultat confirmé indisponible ; pending affiche « Résultat en préparation », sinon « Non classé ». Aucun score/photo/droit ajouté, aucune association par pseudo ; la première ligne canonique remplace la ligne personnelle. Classement global vide toujours masqué, aucune lecture supplémentaire ni changement de polling.

Correction locale après audit du 06/09 : la branche agrégée `historical_fallback` enrichit désormais les photos actives via une lecture groupée des identités Hub exactes, avant dérivation des podiums. Aucun score, rang, départage ou critère Top 3 n’est recalculé par cet enrichissement. Les équipes, identités étrangères ou contradictoires ne sont pas associées par pseudo. Le rebuild ne considère plus comme manquante une contribution absente pour un mapping actif dans une session non terminée ; les résultats manquants des sessions terminées restent bloquants. La projection déjà dirty doit être recalculée via le rebuild canonique existant, jamais effacée par une simple lecture. Les règles de scores restent inchangées. Une ligne runtime retrouvée mais non classable (Quiz/Blindtest sans score positif, Bingo sans résultat de phase après lecture réussie) ne déclenche pas dirty. Décision métier confirmée : session papier sans résultat saisi hors classement, sans dirty. Un classement vide est valide après preuve que toutes les projections Hub sont calculées et sans dirty/erreur ; il ne déclenche pas de fallback répété. Un mapping finalisé dont la référence runtime est introuvable reste une anomalie, sans prétendre prouver une réponse joueur ou une saisie papier. Zéro individuel avec un autre score positif reste classé ; la non-saisie individuelle sur papier partiellement renseigné n’est pas distinguée par les champs lus. Voir [audit et état de correction](../../../notes/hub-photo-latency-audit-2026-09-06.md).

La route `/hub/{token}/play` présente désormais le Hub comme une expérience joueur cohérente avec le Hub Master: le visuel devient un hero intégral seul, suivi de `Bienvenue` et du titre dans le flux. Le bandeau Play neutralise le plafond historique `max-height: 170px`; l'image complète est rendue en `contain` et son chargement transmet son ratio intrinsèque au wrapper, avec `5/2` comme repli cohérent avec le fallback `600×240`. Un plafond raisonnable de `420px`/`52svh` borne les formats extrêmes; tout espace résiduel est intégré au dégradé du branding plutôt qu'affiché en noir. Les métadonnées sortent du visuel et composent deux groupes robustes: date/heure, puis `parties · joueurs`. Les mots ne sont plus coupés arbitrairement; la recomposition est réappliquée après les refresh branding. Le compteur joueurs reste live avec `joueur|joueurs`; la barre intermédiaire et son statut visuel redondant sont supprimés.
...
Le renderer conserve `register_guest`, les règles de validation/unicité, l'identité Hub et ses cookies/localStorage, `current_player`, le polling, l'état joueur déjà inscrit, les lots et les états post-inscription. L'erreur pseudo reste issue du même contrat serveur; elle est maintenant rattachée au champ visible et annoncée comme alerte. Aucun endpoint, write DB, runtime, focus ou session n’est modifié par la composition ; les projections de lecture Play sont enrichies selon le complément du 07/09 ci-dessus.

Une fois le joueur inscrit, Hub Play consomme la décision d’affichage Games partagée avec Master. Le poll historique `active_launched_session` conserve le resolver Global et sa projection `presentation`, et ajoute `display_presentation` ; `games_hub_play_personal_state_resolve(...)` utilise cette dernière pour le seul contexte affiché. La surface Play reste strictement en lecture seule: elle ne sélectionne aucune session, ne poste jamais `presentation_session_select` ou `presentation_mode_set` et conserve la cadence de 3,5 secondes.

Lorsque la présentation cible une session non terminée, l'identité légère précède une carte principale portant uniquement un statut joueur (`Prochaine partie`, `Partie en cours` ou `Partie quittée` selon l'état déjà existant), le jeu, la thématique et le visuel résolu par le helper de session commun. Aucun horaire individuel, contrôle organisateur, réglage ou CTA de lancement n'est projeté. Le changement A → B remplace la carte au poll suivant; sans session effectivement présentée, le podium agrégé garde son contexte intermédiaire ou final canonique ; les fallbacks personnels restent disponibles hors de cet overlay. La reprise volontaire conserve son CTA et son contrat existants hors de la carte. Lots, classement, sortie, runtime et retour de session gardent leurs contrats.
...
Correctif remplacement du 06/09 : chaque tentative crée un fichier distinct pour que la suppression de l'ancien média n'efface jamais le nouveau. L'échec conserve l'ancienne photo ; les erreurs HTTP lisent le code JSON (ou le statut si non JSON), et le succès requiert une confirmation serveur avec média/source. Un poll commencé avant la confirmation ne réinjecte plus son ancienne projection photo ; ses autres projections sont traitées normalement. Sélection, prévisualisation et consentement survivent aux polls ordinaires ; une lecture FileReader tardive après annulation est ignorée. Aucune cadence n'est modifiée. Preuve et limites : `notes/hub-play-photo-replacement-audit-2026-09-06.md`.
...
Complément UX local du 07/09 : le Hub Master remplace le vrai QR joueurs par une illustration non scannable dans une carte unique avec `Mode découverte`, explication d’inscription à la soirée/à l’événement et indisponibilité actuelle. Voir le contrat détaillé en tête de page. La notice du 01/09 et la phrase intermédiaire sont remplacées. Aucun lien ni URL e-commerce ne sont rendus par Games. La décision commerciale et le CTA restent dans l’interface Pro; Remote, le runtime démo et la limite de deux joueurs ne changent pas.

## Update 2026-08-31 - Diagnostic Hub : démo de contrôle

Complément DEV du 09/09/2026, non déployé : les deux CTA globaux conservent le resolver commun et la première officielle. Seul un Quiz papier incompatible chez un client actif utilise désormais le contenu Cotton numérique de substitution décrit dans README Global, avec nombre de séries exact si disponible et best effort Quick sinon. Le parcours compatible et le prospect sont inchangés. Master affiche les erreurs résiduelles sous le CTA via une zone `role=status` / `aria-live=polite`, effacée au prochain clic. Remote conserve le message métier dans `result_json.message`, le projette dans `command.message` et le rend seulement pour `launch_hub_demo` ; les autres commandes gardent leurs messages. Tests en mémoire et DOM locaux ; recette DEV authentifiée restante.

Correctif du 07/09, recette navigateur DEV validée après 14h40 sur H292 (CTA Master et Remote, démos Bingo 27702/27703, présentation officielle différente, maintien Player et retours). Aucun déploiement effectué par Codex. Comportement : la commande `launch_hub_demo` persiste le `transition_type` réellement fourni par l’exécution ; le CTA Master transmet également `runtime_session_id` au relais Global. Les deux origines attendent la readiness canonique de la démo ouverte, jamais un simple heartbeat. Le callback commandé Remote rejoint la même validation puis confirme la présence du marqueur enregistré ; un échec d’écriture n’est pas acquitté comme ready.

Le Player historique avec contexte démo validé source/runtime/exécution reste indépendant de la présentation officielle (A, B ou Hub). `play-ws.js` ignore ce poll avant son départ et revalide le contexte après les réponses asynchrones. Les retours terminaux fin naturelle/sortie via HubTransition et le routage des joueurs officiels sont conservés. Tests dynamiques : `hub_demo_readiness_flow_test.php` et `hub_demo_player_presentation_test.mjs`. Voir [audit et recette](../../../notes/hub-launch-test-audit-2026-09-07.md), section « Correctif local autorisé ».

Le `hub-preflight-dialog` Master reste réservé au diagnostic. Pour une offre active, le CTA global `Lancer un test` est disponible avant le premier démarrage officiel depuis Master et, sous forme compacte, dans le header Remote. Son éligibilité calculée par le `business_snapshot` est aussi projetée dans le payload SSR initial, puis réconciliée à chaque refresh : le CTA ne reste donc pas masqué par un état de premier rendu incomplet. Les deux entrées convergent vers `app_games_hub_demo_representative_create(...)`: Master appelle `launch_hub_demo`; Remote crée la commande Master `launch_hub_demo` soumise au TTL de présence, claim et annulation existants, puis son poll redirige le Master vers l’Organizer démo comme pour un lancement de session. Le resolver `client_routing` retrouve l’exécution `runtime_mode=demo` par sa session source lorsque `active_session_id` est vide, attend le marqueur canonique de readiness du Master historique puis reconstruit la Remote depuis le même `execution_id` et `runtime_session_id`. Au retour terminal vers le Hub, le marqueur conserve le couple `execution_id`/génération ; la Remote bloque ce couple exact, même si la lease Master est encore fraîche. Une nouvelle intention publiée avec génération postérieure reste routable selon le contrat du 04/09. Ni focus, ni présentation, ni membership, ni runtime officiel ne sont modifiés. `Faire la démo` est réservé au mode commercial prospect, y compris lorsqu’une démo annexe active est ouverte : les cartes d’un client actif conservent leurs libellés officiels. Les trois actions Master sont contenues dans le panneau latéral (spinner pendant le test) et la Remote conserve son spinner jusqu’au routage réel. Complément UX du 07/09 remplaçant la copie initiale : en mode commercial, la carte Master contient une illustration non scannable, le badge `Mode découverte`, une explication d’inscription au Hub contextualisée et `Inscription indisponible en mode découverte.`, sans CTA commercial. Aucun vrai QR joueurs n’est généré ; le contrat détaillé figure en tête de page. La limite runtime de deux joueurs est conservée, sans mention imposée dans cette notice. Les comptes sans offre active ne voient pas le CTA annexe et le garde Global refuse l’appel forgé.
...
L'instrumentation navigateur temporaire de reprise QR Hub Player est retirée des sources runtime. Les correctifs fonctionnels restent en place: masquage anti-flash `data-hub-transition-pending="player_auto_restore"`, release/fallback `HubTransition`, priorité de l'auto-restore Hub sur la reprise locale, retour Hub Play canonique (entrée QR et reprise réconciliées dans la section du 20/09/2026), inscription Bingo avant restauration de grille et départ volontaire confirmé par `leave_session`.
...
Contrat réconcilié le 20/09/2026 : un QR session Hub nu retourne d’abord à Hub Play. La reprise directe est réservée à son retour canonique validé, décrit dans « Entrée Player session vers Hub Play — 20/09/2026 ». Le PHP actuel conserve l’ensure runtime de ce retour ; le fast path serveur qui le sautait n’est plus le contrat courant. Bingo passe par le submit `player_register` puis affectation/hydratation de grille ; la description historique d’une restauration Bingo avant register est obsolète. Quiz/Blind Test conservent leur auto-restore depuis le mapping serveur.
...
Le rattachement Canvas est lu depuis les memberships persistés. Les anciennes règles opération/client/date sont réservées à la réparation explicite ; elles ne créent aucun rattachement à la consultation.
...
- Actualisation 2026-09-08 : les lots Hub sont lus sans bootstrap ; un consensus complet peut être projeté en lecture seule, toute divergence reste propre à chaque session. Les valeurs explicitement enregistrées dans le Hub sont conservées. Le Dashboard présente les lots divergents par partie.
...
- Actualisation 2026-09-08 : les lots Hub sont lus sans bootstrap ; un consensus complet peut être projeté en lecture seule, toute divergence reste propre à chaque session. Les valeurs explicitement enregistrées dans le Hub sont conservées. Le Dashboard présente les lots divergents par partie.
- Hub Master/Play exposent les lots communs ; les canvases et résultats de session conservent leurs lots propres, prioritaires sur les lots Hub, sans copie des lots d’une autre session.
...
Contrat remplacé pour l’entrée Player le 20/09/2026 : une URL session Hub nue rejoint Hub Play avant toute participation runtime. L’identité et le focus sont résolus au niveau Hub ; seul son retour canonique validé ouvre le Player. Voir « Entrée Player session vers Hub Play — 20/09/2026 ».

Le bridge historique reste dans les endpoints `player_register` Quiz/Blind Test/Bingo pour leurs appelants existants : retour `hub_bridge`, cookies `cotton_hub_token` / `cotton_hub_player_token` et mapping session. Il ne constitue plus le chemin public normal d’un ancien QR Hub. Les appels internes de l’ensure Hub conservent `skip_hub_register_bridge=true`. Aucune API n’est supprimée dans ce lot.
...
- Depuis le 20/09/2026, un QR session sans contexte canonique validé passe d’abord par Hub Play ; la reprise de ses cookies/identité y précède le choix du focus.
- Une identité Hub absente conduit au parcours Hub Play, pas au formulaire de la session source ; le Player classique reste le parcours des sessions autonomes.
