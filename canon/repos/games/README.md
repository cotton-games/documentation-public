...
- player supports (quiz/blindtest) : timeout de démarrage durci avec adaptation réseau (image/drive 15-20s), retry unique image/drive, annulation de timers au `load/error`, et logs `SUPPORT_START_FAIL_DETAIL` enrichis avec `stale_token` pour distinguer erreur active vs timeout obsolète.
