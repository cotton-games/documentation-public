...
- [x] Patch local : sélection par publication, fermeture durable par intention, présence/événements des deux surfaces, Bingo sans reset redondant, diagnostics de copie préservés ; UX prospect/client séparées.
...
- [x] Patch local : Modales soirée/événement via resolver existant ; `quitGame intent=hub_suspend`, ACK métier puis retour ; endpoint interne `hub_lifecycle` réservé au service token et garde des writes HTTP. Play conserve sa navigation par perte de focus.
