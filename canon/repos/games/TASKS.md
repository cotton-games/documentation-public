...
- `../games/web/organizer_canvas.php`
  - expose maintenant `AppConfig.sessionDetailUrl` vers `/extranet/start/game/view/<id_securite_session>`.
...
- `../games/web/remote_canvas.php`
  - ajout du CTA termine `Ajouter les photos des gagnants !`;
  - exposition de `AppConfig.sessionDetailUrl` cote remote;
  - correction de l'initialisation `CONF_SITE_ROOT` pour eviter le warning PHP sur les URLs absolues de branding.
- `../games/web/includes/canvas/remote/remote-ui.js`
  - sur un quit volontaire en `Partie terminee`, la remote redirige maintenant vers la fiche detail session au lieu de tenter `close/about:blank`.
- `../games/web/includes/canvas/php/boot_lib.php`
...
### Effet attendu
- fermer volontairement `master` ou `remote` en fin de session ramene l'organisateur sur la fiche session `pro`;
