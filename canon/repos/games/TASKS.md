<!-- AUTO-UPDATE:BEGIN id="hub-suspend-20260910-games-tasks" owner="codex" -->
...
**25/09 — Audit PROD Hub298 du24/09, sans patch.** Snapshots Git `hub_soiree` Games/Bingo/Global, journal AI Studio raw et documentation privée main/develop consultés. Trois premiers lancements : documents Organizer200 et Players bindés, auth Organizer non établie ;15 AUTH_FAIL Client mismatch tardifs fortement corrélés à30996/30993, sans clé de jointure certaine. Le garde compare E mémoire WS et E officielle relue ; les protections de reprise running ne couvrent pas nécessairement une initialisation Player avant gameplay. Cause initiale non concluable ; SQL de lecture fourni, conclusion correspondante en attente. Aucun applicatif/marker/commit/restart/deploy. [Rapport et preuves, section M](../../../notes/hub-soiree-hotfix-2026-09-23.md#m-incident-prod-hub-298-du-2409--audit-sans-patch-du-25092026).
...
**25/09 — Diagnostic régression suspension BT27984/Hub353** : publish puis En cours08:11:30, sans mainPlayerStarted observé ; trois hub_lifecycle409 HUB_PREGAME_FOCUS_LOCKED08:11:41/49/58. Nouveau wrapper pregame sur focus_clear refuse le slot starting, alors que suspend durable et ACK précèdent release. Reproduit localement : ACK puis ERROR, suspended=true/released=false avec transport simulé. Distinct des hub_busy Bingo. Scénario START accepté/média non parti/suspension manquant aux tests ; correctif à cibler sur intégration suspension/slot/focus, sans faux started ni ouverture générale du guard. Section X de la note V2. Diagnostic historique ; correctif local A réalisé ensuite, section Z. Aucun deploy/restart.


**Nouvelle recette Bingo stabilisée1018/Hub353/session27983** : reset réussi19:06:53.834,5,623s après13e bind ; started accepté19:06:54, morceau19:07:06.254, late joins ensuite. Aucun restart WS dans la fenêtre. Trois hub_busy et des NOT_STARTED persistent même après started : contention/guard à isoler, pas défaut du timer démontré. Section W de la note V2 ; audit documentaire uniquement, aucun patch/deploy/restart.


**Complément recette Hub353/BT27982** : publish19:02:53,29e bind19:03:11.787, premier En cours19:03:17.009 (+5,222s), trois binds tardifs, started accepté et countdown19:03:30, puis pause19:03:31. Aucun hub_busy retrouvé côté BT sur cette fenêtre. PHP rechargé atteste aussi started Bingo27981 à19:02:34 : récupération après les erreurs initiales, sans cause exacte établie du reset refusé. Sections U/V de la note V2 ; audit seulement, aucun patch/deploy/restart.


**Recette Bingo1018, Hub353/session27981 (audit local des logs)** : publish accepté18:59:33 ; premier hub_busy18:59:36.741 ; reset refusé hub_pregame_not_started18:59:39.363 ; premier bind18:59:40.516.28 binds/22 identités sur toute la fenêtre, pas un effectif simultané. Restart WS observé19:00:28.592 ; gameplay réussi non attesté. Le guard local traduit aussi un verrou indisponible en NOT_STARTED : contention étayée, raison exacte du refus reset encore indéterminée. Nouveaux délais non qualifiés. Aucun correctif/déploiement/restart effectué ; section U de la note V2.
...
**Recette après correctif EP (déployé et retesté selon opérateur) :** Hub353/BT27977, Player1854 atteint Player Canvas à17:16:45, mapping2379 actif, bind WS participation221319 à17:16:46.624 ; compteur session2. Ancien blocage avant redirection levé sur BT. Compteur visuel prégame/START/abandon et nouveau test Quiz restent à confirmer. Audit sans nouveau patch ; preuves et limites en section R de la note V2.
...
- [x] Première recette Blind Test DEV signalée positive côté Master ; correctif local du suivi Remote et de la préparation sur reprise.
- [ ] Refaire la recette DEV ciblée premier lancement/reprise/reload Remote ; Quiz/Bingo et collecte instrumentée ensuite, sans changement du moteur START.
...
Audit client1018 (Hub353/Quiz27976) : publish accepté, Organizer/Remote WS présents, aucun Player WS et compteur0 ; auto-admission1854 répétée mais logs PHP tronqués. HUB_PREGAME_NOT_STARTED signalé côté UI, commande rejetée non identifiée dans les logs ; ne pas attribuer sans preuve au signal audio de la recette précédente. Détail/preuves/limites et capture ciblée restante : section P de la note Open Players. Audit seulement ; note/TASKS/HANDOFF puis index/sitemap actualisés, aucun changement applicatif ni déploiement.

- [x] Audit nouvelle recette DEV client10 Hub351/Quiz27974 : publish et cleanup acceptés, Remo lié au WS (compteur1), focus actif libéré après abandon. CTA inactif : conflit probable du renderer historique attendant un prelaunch désactivé sous Hub, reproduit sur fragment réel ; DOM DEV non capturé. [Preuves et correction ciblée restante](../../../notes/hub-open-players-foundations-2026-09-24.md#n-recette-dev-client10--hub351--quiz27974-2409-à1410). Correctif CTA ensuite autorisé : renderer aligné sur l’autorité Hub, guards opening/closing et chargement préservés, classe CSS synchronisée ; deux modules JS Games, test CTA et runner modifiés. Test dédié4/4 ; aucun déploiement ni qualification navigateur. Note/README/TASKS/HANDOFF/CHANGELOG et sitemap actualisés.
...
- [x] Slot/bridge/writers/admission, Organizer sans auto-start, preuves serveur, compteur, Lancer durable et abandon raccordés ; reset Bingo stable au premier Lancer.
- [x] Tests du parcours avec DB/transport simulés et matrice historique ; [commandes et résultats](../../../notes/hub-open-players-foundations-2026-09-24.md).
- [x] Correction locale du transport proof et tests JSON.stringify → formulaire → PHP → bridge ; reprise du cleanup corrélé testée.
- [ ] Livrer le correctif bridge, vérifier le slot abandonné et refaire la recette DEV minimale ; UI finale/fullscreen ensuite.


## 23/09/2026 — Hotfix minimal hub_soiree

- [x] Base propre `hub_soiree` vérifiée ; extraction minimale, sans merge/cherry-pick. `hub_capacity_get(stage=admit)` réutilise le transport service-only, sans calcul commercial/stock/roster. Attente persistante du runtime officiel numérique ; fence Master fraîche et garde sessionStorage Hub/session/E1 avant continuation unique. Papier/démo gardent leur attente historique.
...
- [x] Préflight propre, branche opérateur `hub_session_readiness` depuis `hub_soiree`, reprise sélective sans merge. Lobby Master historique, continuation automatique, preuve Player après état appliqué, API service-only hub_readiness, Remote sans abandon terminal 15 s, harnais Browser Quiz/BT/Bingo froid.
...
## 22/09/2026 — Podium Hub Master et scrollbar programme

- [x] Audit RAW/journal AI Studio, asset complet et scène HTML ; mélange viewport/cadre central identifié. Logs locaux : piste Hub185/session30455, Safari17.4 le17/09, lien indirect client16.
- [x] Révision CSS immersive : overlay ancré à l’écran Master, ancêtres intermédiaires neutralisés uniquement à l’ouverture desktop, contain maximal16:9/plafond natif1664px. Confinement à la feature980px supprimé ; scrollbar masquée conservée. Classements, programme et mobile préservés.
- [x] Lint, tests mobile/podium/agrégat/QR relancés OK ; contrôles conceptuels G1–G9 et frontière991/992, diff strictement CSS. Échec settings antérieurement reproduit sur HEAD, hors périmètre. Rapport existant/contrat corrigés, sitemap/index régénérés.
- [ ] Recette immersive G1–G9, fermeture, overlays concurrents et scrollbar sur navigateurs et écrans réels ; aucun déploiement effectué.

[Audit, commandes, limites et rollback](../../../notes/hub-master-podium-scrollbar-2026-09-22.md).


## 22/09/2026 — Hub vide et navigation Pro

- [x] Audit retrait/commandes et état vide ; preuve de suppression complète ultérieure Hub343 dans les logs Pro. Aucun changement de handler, moteur, WS, score ou suspension.
- [x] Fixture `hub_empty_commercial_test.php` complétée avec configuration Pro :105 contrôles verts ; test DOM suppression vert. Correctif fonctionnel dans Global/Pro seulement.
- [ ] Recette réelle retrait dernier membre puis Quick Add et navigation ; aucun déploiement/restart.

[Rapport et limites](../../../notes/hub-empty-pro-navigation-2026-09-22.md).


## 22/09/2026 — Audit numérique Hub343/client10

- [x] Captures Master et complément08:47 : six lignes stables, compteur20→6 également en pause stable ; pas une simple différence pause/lecture. WS : roster6 persistant, aucune publication20 ;466 HTTP500 `test_bots.php` dus au helper `canvas_live_participant_rows` non chargé par le glue autonome. Défaut de dépendance confirmé localement sans DB.
- [x] Correctif bots local : `games/web/test_bots.php` utilise le bootstrap Canvas commun ; trois résolveurs chargés, lecteurs Quiz/BT et filtres testés avec PDO simulé. Régression avant/après confirmée, suite PATCH1 14/14 verte. Aucun WS/marker/restart.
- [ ] Après copie de ce seul fichier sur base PATCH1/PATCH2, relancer les bots et vérifier les20 admissions ; tracer séparément le20 intermittent (origine non établie). [Preuves et limites](../../../notes/hub343-digital-blindtest-roster-audit-2026-09-22.md).


## Audit sas initial numérique Hub — 22/09/2026
...
- [x] Migration unifiée, socle Browser Quiz puis Bingo local : [contrat](../../interfaces/hub-bot-test-runner.md), mêmes sujets/polling/timeline/résumé/stop/cleanup. [Bingo prepared/off](../../../notes/hub-bingo-browser-baseline-2026-09-22.md) avec vraies grilles et ready après post-reset ; 66 tests Games,27 Bingo,4 AVANT verts. Bundle AVANT/APRÈS et rollback vérifiés SHA ; restent livraison Bingo AVANT et recettes50/200, sans sas/performance dans AVANT.
...
Complément pipeline : [audit charge/convergence](../../../notes/hub-player-registration-pipeline-audit-2026-09-22.md). Roster capacité répété, publication BT quadratique, auth Bingo sérialisée (5 HTTP nominalement, snapshot complet). Signal agrégé nécessitant évolution WS ; pas de durée/cache/batch décidé. Probe local avec I/O simulées et tests bots verts, pas de qualification 5 000. Dépendance ecommerce signalée au journal : rechargement requis avant analyse de son coût interne. Aucun patch applicatif.

Audit sans patch applicatif : séparation runtime/inscription/Play faisable dans le boot Master commun sans changer le pipeline WS. Pas de signal robuste de fin de vague identifié ; appels capacité par inscription à mesurer avant toute garantie 5 000 joueurs. Course auto-sync Bingo déjà protégée localement ; reprise Pause/no-autostart vérifiée par test local. Journal lu, aucun rechargement ciblé identifié, parité PROD non établie. [Rapport, preuves, périmètre futur et tests](../../../notes/hub-initial-session-sas-audit-2026-09-22.md). Aucun marker, DB réelle, déploiement ou restart ; mécanisme de convergence à décider.
...
## PATCH1B — réconciliation à la reprise papier (21/09/2026)

- [x] Correction recette suspension : grant avant ensure, exception serveur scoped dans Games ; vrai dispatcher/garde/grant testés, aucune levée générale du gel. Livraison coordonnée Global + Games `php/hub_lifecycle.php`.


- [x] Réutilisation de l’injection canonique avant retour de reprise papier ; compteurs, exclusions/échecs et retry idempotent.
- [x] 876 contrôles PHP sur vrai launch/resolver/ensure avec I/O simulées ; transmission des fixtures aux vraies maintenances Quiz/BT/Bingo :18→20 sans socket. Suite6/6 incluant PATCH2 8/8 et PATCH1 14/14, lint/diff-check.
- [ ] Recette réelle Hub341 après livraison séparée ; aucune DB réelle/SSH/service/restart exécuté ici.

[Rapport, fichiers et limites](../../../notes/paper-resume-patch1b-2026-09-21.md).
...
Complément recette22/09, client3/Hub342 : audit des logs terminé. Quiz utilise le reçu PATCH2 ; Blind Test passe encore dans le chemin PHP historique (24 retries du même score300). Le module Remote refuse cette réponse sans confirmation structurée, bloque les corrections suivantes et n’envoie aucun `admin_set_score`. Reproduction locale avec réponses legacy/PATCH2 simulées verte ; version Games/PHP réellement exécutée à aligner/vérifier avant nouvelle recette. Aucun patch applicatif, DB réelle ou restart. [Preuves et diagnostic](../../../notes/hub342-blindtest-paper-score-audit-2026-09-22.md).
...
- [x] Audit Hub341 sur logs rechargés :20 actifs Hub, échecs d’injection tronqués, reprise existante sans injection complète identifiée ; complément SQL utilisateur : les2 absents Quiz sont mustach1132 et Jiji1133, sans participation ni mapping ; traces d’inscription dans les jeux suivants retrouvées. Parcours clients confirmé : polling Hub Play/bots → ensure individuel papier ou numérique ; protection conditionnée à un client actif et à une inscription réussie. [Preuves et suite](../../../notes/hub341-paper-resume-roster-audit-2026-09-21.md). Aucun changement applicatif dans cet audit.


- [x] Provenance validée, recherche/saisie libre non exclusive sauf nom exact réservé, preload/live actif commun, bind demandé même sur `already_active`, autorité explicite des projections papier Remote/Master et clôture des HTTP anciens. Les compteurs suivent les listes acceptées ; protections équipes BT et fin Bingo conservées.
...
## 2026-09-21 — Audit transversal des identités papier, archivé

Rapport A–U Quiz/Blind Test/Bingo et annexes conservés dans [notes](../../../notes/paper-session-participant-identity-audit-2026-09-21.md). Audit local sur sources figées : défaut score Quiz/BT, fallback Bingo numérique incomplet, hypothèse organisateur non exclusive. Archivage documentaire terminé ; PATCH1 inscription/roster validé en recette DEV selon l’utilisateur ; PATCH2 scores/finalisation et présentation des podiums implémenté localement ensuite, voir le suivi ci-dessus. Bingo D-only traité localement par PATCH3 : roster canonique scoped, association non bloquante, ACK HTTP respecté ; tests serveur/HTML Master et Remote verts. [Rapport PATCH3](../../../notes/bingo-paper-association-patch3-2026-09-21.md). Les résultats ne constituent pas une validation de production.



Audit 21/09 : joueurs réels non garantis prêts au premier extrait ; Bingo session 27915 commence au compteur 27, atteint 100 environ 49 s après. Compteur diffusé avant envoi de l’état ; 373 erreurs de coches bots reset_stale. Aucun patch fonctionnel. [Preuves et recommandation](../../../notes/hub-real-player-readiness-2026-09-21.md).
...
## Entrée Player session vers Hub Play — 20/09/2026

- [x] Ancienne URL Player Hub traitée avant preload/HTML ; source sans autorité de destination, y compris terminée/non focus.
- [x] Retour canonique validé sur joueur/token/ID, focus rejoignable, exécution officielle et mapping actif ; papier et Hub inactif restent côté Hub.
- [x] Traduction du jeton EP par les helpers existants ; aucune participation source, temporalité probable/fermée conservée.
- [x] Tests dédiés de branche et résolution S2→S3, trois jeux/EP/démo ; ancienne assertion de bypass par paramètres remplacée. Lint et tests ciblés exécutés, documentation réconciliée.
- [ ] Recette manuelle DEV après copie autorisée : ancien S2→S3, identité EP, reload canonique sans boucle, attente/final, autonome, démo et papier. Aucun déploiement effectué ; legacy Bingo hors exploitation dans ce lot.
...
- [x] 20/09 — Podium session mobile déplacé sous sa carte terminée sélectionnée ; desktop/global inchangés, resize/refresh couverts. Complément demandé : CTA Résultats retiré, sélection de carte seule et footer vide masqué en mobile. Suites podium/mobile/QR/settings/confirmation vertes ; mock DOM compteur réparé. [Rapport](../../../notes/hub-mobile-session-podium-2026-09-20.md).
- [ ] Recette visuelle du nouveau placement, sans reload aux seuils991/992 et après refresh.


- [x] 19/09 — Blind Test27894 : marqueur anonyme confirmé par console puis déblocage utilisateur. Patch local départ par jeu/session/player_id ; sept tests ciblés verts, syntaxe/diff-check verts. Suite générale bloquée par assertion DOM compteur Master préexistante (ligne33), reproduite sans patch Player.
- [ ] Recette DEV : A quitte session puis Hub, B s’inscrit dans le même profil sans blocage, A revient explicitement ; vérifier aussi fin serveur et papier. Aucun déploiement/restart.

## Hub Master responsive — 18/09/2026

- [x] Complément UI du 19/09 : desktop sans libellé mobile parasite, sticky/wording mobile, retour/Quick Add/compteur unique ; suites locales vertes. [Détail et recette visuelle restante](../../../notes/hub-mobile-ui-2026-09-19.md).

- [x] Composition verticale, CTA par carte, QR/overlay, compteur, podium et retour Dashboard ; politique papier mobile/desktop sans état persistant.
- [x] Tests resolver/rendu, 21 cas POST réel avec service simulé, handler/confirmation/resize, QR, refresh et non-régression verts.
- [ ] Recette visuelle multi-largeurs et intégration papier réelle sur les trois moteurs : environnement non accessible.

[Rapport et commandes](../../../notes/hub-master-responsive-2026-09-18.md).


## POC expérimental Cast DEV — 17/09/2026

> **Branche `cast` — NON DÉPLOYÉ EN PROD au 22/09/2026.** Exclu du déploiement `hub_soiree`. [État de livraison](../../deployment-status.md).

- [x] Précondition AI Studio consultée, périmètre isolé sans métier/DB/Remote ; code local uniquement.
- [x] Sender Pro mobile opt-in, guards endpoint DEV, receiver Games CAF persistant et vrai player Cotton A/B/C, une activation, diagnostics/export.
- [x] 30 tests POC, 59 contrôles historiques Pro, lint PHP/JS ; docs et index/sitemap mis à jour.
- [ ] Livraison DEV manuelle, Application ID, trois médias, appareils enregistrés et recette physique Chrome Android/TV. Aucun déploiement réalisé ; compatibilité sonore non prouvée par mocks.

[Rapport A–N, fichiers, commandes et rollback](../../../notes/cast-dev-poc-2026-09-17.md).


## QR joueurs serveur — 17/09/2026

- [x] Précondition AI Studio levée avant patch : `global_librairies.php` resynchronisé le09/09, hash local identique au rapport d’application archivé ; Global non modifié.
- [x] PNG mémoire unique, deux images SSR, secours HTML officiel, aucun QR/URL découverte, aucun encodeur sur poll/Play.
- [x] Master sans génération JS/QR CDN ; chargements décoratifs non bloquants, contrôleur IMG/readiness/load/refresh, transition et rollback accessibles, filemtime, CSS de base.
- [x] Tests encodeur réel/décodage indépendant/marge/HTTP/buffers/échecs/DEV-only et DOM/Remote ; régressions métier mémoire. Deux échecs larges reproduits sur HEAD avant patch : modales papier et CTA démo ; voir rapport.
- [x] README Games, bridge-contract, HANDOFF, CHANGELOG et note actualisés ; sitemap/index régénérés.
- [ ] Recette physique Smart TV et navigateur desktop : scan petit/grand, JS coupé, CDN bloqué/lent, ancien moteur CSS, refresh, Remote, focus. Aucun déploiement ni DB réalisés.

[Rapport et commandes](../../../notes/hub-player-qr-server-2026-09-17.md).


## Masques YouTube permanents — 17/09/2026

- [x] Relire START/sitemap/manifest et Markdown brut embarqué du journal AI Studio : aucun fichier ciblé signalé à recharger.
- [x] Retirer timers/fonctions/événements de disparition et règle `.overlay-hidden` ; conserver DOM,58px, hiérarchie, audio, CTA, paramètres YT et Hub.
- [x] `node web/tests/youtube_masks_test.mjs` : démarrage auto/manuel, pause/reprise, replay Quiz suivi de pause, reprise Bingo après jingle, support suivant et audio. Contre-épreuve `--baseline` en échec attendu sur HEAD avant patch.
- [x] Régressions `hub_launch_confirmation_test.mjs`, `hub_active_resume_test.mjs`, `hub_launch_demo_shortcut_test.mjs` et `hub_launch_confirmation_contract_test.php` vertes.
- [ ] Recette DEV réelle : desktop/mobile, resize, buffering, replay, Hub/historique, audio et CTA. Aucun déploiement/restart/DB/SSH.


## Préparation anticipée papier Hub — patch local du17/09/2026

- [x] Journal RAW et audit sas/roster/lookup/undo ; aucun reload ciblé requis. Arbitrages : recherche unifiée typée, aucun sélecteur, clé stable conservée sans nouvelle sémantique équipe ; fallback invité.
- [x] Fil du10/09 retrouvé : REMO historique du compte10, refus USERNAME_REFERENCED ; guard commun Games avant ensure avec le helper et la normalisation runtime, refus sans écritures et wording exact.
- [x] CTA global Remote + éligibilité serveur + même sas sans lancement ; Global et runtimes inchangés. Tests Games/Global verts, dont75 nouveaux contrôles PHP et parcours DOM global soirée/événement. [Rapport, fichiers et commandes](../../../notes/hub-paper-early-roster-audit-2026-09-17.md).
- [x] Correctif UX mobile : compteurs courts, toggle texte Voir/Masquer la liste sans style CTA, section Ajouter un participant et aide courte, suppression des titres redondants, largeur utile360px et scroll interne. Quatre suites UI/DOM vertes ; aucun changement métier. Recette visuelle appareil restante. Complément wording : CTA Participants avec icône joueurs au style QR / titre Ajouter des participants, inscrits soirée/événement sans mention papier, phrase exacte d’ajout QR avant le champ dans les deux accès ; aide de recherche courte conservée sous le champ. Compteur du sas inchangé ; titre « Vérifier les participants ».
- [ ] Recette DEV navigateur/DB réelle après diffusion autorisée ; concurrence QR/papier non garantie par le contrôle existant. Aucun déploiement/restart/migration.


## Cycle démo/test Hub — 17/09/2026

- [x] Auditer logs et contrats : première démo sélectionnée avant matching, retour sans révocation, reset Bingo sans identité.
- [x] Patch local : sélection par publication, fermeture durable par intention, présence/événements des deux surfaces, Bingo sans reset redondant, diagnostics de copie préservés ; UX prospect/client séparées. Complément Remote18/09 repris après régressions DEV : séparation candidate/propriétaire/révoquée, réinscription standard, cache ESM versionné et bootstrap de génération absente depuis la publication exacte. Validateurs Global et serveur Bingo inchangés ; takeover/rattachement tardif validés en DEV par l’utilisateur. Complément UX local commun Games : ancienne Remote remplacée vers fermeture/about:blank, sans retour Hub ni clôture de démo ; quit volontaire inchangé. [Correctif de sortie takeover](../../../notes/remote-takeover-exit-2026-09-18.md). [Preuves, fichiers et recette restante](../../../notes/remote-bootstrap-regressions-2026-09-18.md). Complément18/09, expiration involontaire confirmée : identité moteur figée, événement `hub_runtime_expired` idempotent, focus CAS, refus `HUB_RUNTIME_EXPIRED`, aucun CTA/recréation officielle ; tests grâce valide, suspension >1 h et papier/numérique. [Rapport](../../../notes/hub-runtime-expired-2026-09-18.md).
- [x] Contre-épreuves routage/cycle/idempotence/UX, régressions suspension officielle, Pro historique et reset Bingo. [Rapport et commandes](../../../notes/hub-demo-lifecycle-patch-2026-09-17.md).
- [ ] Recette navigateur multi-écrans et concurrence MySQL autorisée ; aucun déploiement/restart/DB réelle.


## Test lié à une session — 17/09/2026

- [x] Audit puis patch local : source explicite, contenu courant, deux entrées UI, papier Master/Remote, revalidation et reprise de la même intention.
- [x] Complément utilisateur : test des réglages conditionné uniquement au non-démarrage de la cible ; confirmation fondée sur les membres actuels après suppression. Correction Master `today` et régressions dédiées.
- [x] Validation hors connexion : 33 suites réussies sur 34 ; échec Quick Add préexistant documenté, fichiers concernés identiques à HEAD.
- [ ] Recette navigateur authentifiée et concurrence SQL en environnement autorisé ; aucun déploiement/restart/accès DB réel. [Rapport](../../../notes/hub-session-targeted-test-2026-09-17.md).


## AUDIT VALIDÉ — Routage mobile Pro — 16/09/2026

- [x] Identifier le garde302 Games du08/09 (`71eb3c71`), reproduire son interception du vrai lien Pro et valider l'exception existante `master_view=individual` ; aucun code Games modifié. Contrats legacy, reprise et Remote verts ; README précisé. [Audit et correctif Pro](../../../notes/dashboard-mobile-individual-master-2026-09-16.md).
- [ ] Recette navigateur après diffusion autorisée ; aucun déploiement/restart.


## Quick Add / remplacement de contenu — 16/09/2026

- [x] Corriger séparément la fausse erreur Quick Add : carte canonique courante, poll concurrent/latence, retry borné sans recréation, double clic.
- [x] Déplacer le remplacement des cartes vers les réglages Master/Remote ; déléguer l'éligibilité sans origine au helper Global, conserver handlers/options, ajouter feedback et gardes modales stale.
- [x] Micro-UI Quiz : noms de séries compacts et rafraîchis Master/Remote ; priorité CSS `hidden` du pilotage manuel Remote numérique restaurée. Tests PHP/JS/CSS et polling existant verts.
- [x] Complément affichage/wording : corriger le Hub absent du view-model Master, aligner les libellés playlist/séries avec Pro via le payload commun. 60 contrôles PHP et branches JS réelles pour trois jeux/quatre états ; aucune modification de l'éligibilité ou du service.
- [x] Tests JS réels, empreintes Master/Remote/Pro à timestamp identique, non-régression Hub vide/suppression/QR ; [preuves et commandes](../../../notes/hub-quick-add-renewal-2026-09-16.md).
- [ ] Recette navigateur authentifiée et concurrence DB réelle. Aucun déploiement/restart/DB réelle ; cette recette remplace l'ancienne attente d'icône réservée aux Quick Add.

## Suspension officielle Hub — 10/09/2026

- [x] Patch local : Modales soirée/événement via resolver existant ; `quitGame intent=hub_suspend`, ACK métier puis retour ; endpoint interne `hub_lifecycle` réservé au service token et garde des writes HTTP. Play conserve sa navigation par perte de focus. Complément18/09 : génération publiée exacte, continuité Remote distincte du takeover et diagnostic corrélé selon le périmètre du repo ; [rapport et tests](../../../notes/bingo-hub-resume-robustness-2026-09-18.md).
- [x] Contrat et tests hors connexion consignés dans le [rapport de validation](../../../notes/hub-suspend-patch-2026-09-10.md).
- [ ] Recette sur services réels après livraison autorisée : matrice papier/numérique, perte involontaire puis reprise active2→3, double suspension/reprise, socket zombie, deux onglets/appareils, diagnostics, cutoff. Validation locale18/09 verte ; aucun déploiement ni redémarrage exécuté.
...
- [x] 11/09/2026 — Correction du timeout observé en DEV Blind Test : Le Master reconstruit le payload de suspension avec les seuls champs métier ; les champs `type`/`action` de la trame Remote ne peuvent plus remplacer `quitGame`, y compris lors d’une nouvelle tentative depuis le Master. Tests et preuves dans le rapport lié ci-dessus. Recette DEV après diffusion du correctif restante.

- [x] 11/09/2026 — Retour Play après suspension : `active_launched_session` peut conserver `presentation.target=session` sur la session suspendue tout en renvoyant `session:null`. Le Player utilise désormais cette absence explicite de session active pour revenir au Hub, indépendamment de la sélection de présentation. `player_unresolved` et un champ absent ne prouvent pas une perte de focus. Isolation démo et garde visuelle terminale conservées ; polling existant de15s et contrôles de premier plan inchangés.
- [x] Tests : scénario avant correction rouge, après correction vert ; suites suspension/démo/transition JS (35 résultats TAP) et séparation présentation/runtime PHP vertes.
- [ ] Recette DEV Player après diffusion de `includes/canvas/play/play-ws.js` et rechargement du navigateur ; aucun restart WS nécessaire pour ce correctif.
...
- [x] 11/09/2026 — Reprise Quiz papier : suppression du filtre `flag_controle_numerique != 0` dans le garde `$HUB_RESUME_EXISTING` de Games `organizer_canvas.php`. Une reprise officielle papier autorisée suit désormais le même chemin que le numérique ; contexte activé, exécution exacte, focus actif, runtime running et génération exacte restent obligatoires. Le dégel WS exige toujours son autorisation de reprise.
- [x] Régression reproduite avant patch puis corrigée : 56 contrôles PHP reprise, 35 résultats TAP JS suspension/reprise, anciennes routes PHP verts.
- [ ] Recette DEV après diffusion du seul `organizer_canvas.php` : reprendre Quiz papier, vérifier arrivée Master en Pause avec scores/progression conservés. Aucun restart WS nécessaire.
...
- [x] 16/09/2026 — Incident DEV du même patch : TDZ `bingoResetInFlight` reproduite avant toute API reset ; initialisation déplacée avant le boot, deux régressions boot/confirmation et boot/retry rouges puis vertes. 49 résultats TAP Games, 15 Bingo et quatre suites PHP verts. Preuves et timeline ajoutées au rapport lié ci-dessus ; contrats inchangés.
- [ ] Recette DEV du reset historique à reprendre après correction TDZ et rechargement Master, avec Player absent, concurrents et nouveaux joueurs ; mesurer les lectures WS et verrous MySQL. Aucun déploiement, restart ou accès DB réel. La conception du futur `Recommencer` Hub reste distincte.
...
### Suppression Hub — périmètre réduit, 16/09/2026, patch local non déployé

- [x] Master/Remote : action danger dans le paramétrage pour non-jouée, suspendue libérée et terminée ; confirmation applicative contextualisée. Aucun retrait en cours ; revalidation serveur sous verrou partagé avec lancement et rebuild.
- [x] Non-jouée : délégation à la suppression canonique historique. Démarrée : intention durable, membership inactif, mappings `left`, historique conservé, rebuild synchrone, présentation nettoyée. Une suspension reste gelée jusqu’au cleanup canonique de cutoff ; reprise interdite.
- [x] Polls existants maintenus après cutoff ; refresh partiel Master/Remote/Pro, invalidation des modales Master/Remote, programme vide Pro sans réintroduction de l’historique. Pro initie toujours uniquement les non-jouées.
- [x] Tests isolés PHP/DOM et non-régressions ; [audit, matrice des 24 cas, commandes et limites](../../../notes/hub-session-removal-audit-2026-09-16.md).
- [x] Micro-correctif UX : action de suppression tertiaire danger avec corbeille dans le contenu après séparateur, footer Annuler/Enregistrer ; textes du premier lancement contextualisés soirée/événement. Handlers/permissions/confirmation de suppression inchangés ; quatre suites JS, deux suites PHP, syntaxe et diff-check OK.
- [x] Complément contraste/test : tokens de présentation partagés Master/Remote et ratio ≥4,5 sur palettes violet, claire, vert vif, Cotton et gris intermédiaire ; raccourci de démo via le bouton existant avec revalidation, anti-double-clic, erreurs/retry et nettoyage du lobby papier. Tests PHP de contraste et JS des handlers réels ajoutés ; aucun endpoint ou wording modifié. Recette visuelle réelle desktop/tablette/mobile restante.
- [x] 16/09/2026 — Hub vide : corriger le fallback commercial Master/Remote en résolvant l'offre du propriétaire Hub, sans dépendre des sessions. 105 contrôles de projection/rendu et suites QR/suppression passent ; joueurs, compteur, ajout et attente Play conservés. QR réduit autorisé selon fenêtre/offre, agrandissement indisponible sans continuation ; aucun changement Global/Play. Local non déployé, recette navigateur restante (preuves dans HANDOFF).
- [ ] Recette navigateur multi-surfaces et validation des verrous sur environnement de test autorisé. Aucun accès DB réel, déploiement ou redémarrage exécuté.

<!-- AUTO-UPDATE:END id="hub-suspend-20260910-games-tasks" -->

<!-- AUTO-UPDATE:BEGIN id="hub-remote-limited-20260910-canon-repos-games-tasks" owner="codex" -->
...
Validation locale : 18 suites ciblées vertes, dont 44 cas de grâce Quiz et 44 Blind Test, livraison terminale Bingo, matrice des 3 jeux et des modes, lecture du getter réel avec frontière TTL (12 s valide, 13 s expirée), erreurs SQL/HTTP, courses et cycle complet X/G sans clic ni mutation. Syntaxes PHP/JS et diff checks vérifiés. Recette navigateur/réseau réelle restante ; aucune DB, connexion serveur, migration, publication ou restart.

Baseline avant patch : polling rouge sur le sas papier (mock DOM sans `querySelector`, puis confirmation attendue à l’ancien emplacement) ; contrat Games rouge sur six assertions. Transition/readiness : obsolètes après 10/09 ; extraction de fonction trop large : test incorrect ; signature de lancement à quatre arguments et QR piloté par le flag serveur : tests obsolètes hors évolution. Les attentes sont réalignées sur le comportement courant et couvertes dynamiquement ; aucun code métier papier/QR modifié.

Archéologie : Games `4dc215a` contient le routage canonique initial ; `247b1f9` ajoute le filtrage au retour, `eb41a75` un contournement par commande, `6aab58c` le tuple générationnel et le routage unique ; `aa80a6b` borne l’overlay post-retour ; `caee494`/`299ed1b` ajoutent/étendent les CTA ; `ac743fc` ajoute l’attente persistante du 10/09. Global `bc44e2c` restaure la garde de présence. Aucun revert massif.

Journal AI Studio RAW relu avant patch : aucun chemin runtime ciblé identifié comme modifié hors workspace ; alignement serveur non vérifié. START/SITEMAP/manifest develop relus.

Fichiers applicatifs : Games `remote_canvas.php`, `modules/app_hub_remote_ajax.php`, `includes/canvas/remote/remote-ui.js`, `includes/canvas/core/hub_transition.js` ; Global `app_games_hubs_functions.php` ajoute seulement `available` au getter de présence pour distinguer l’échec SQL. Tests Games mis à jour/ajoutés. Documentation : README/TASKS Games et Global, HANDOFF, contrat Canvas et CHANGELOG ; index/sitemap régénérés.

<!-- AUTO-UPDATE:END id="hub-remote-limited-20260910-canon-repos-games-tasks" -->

## Hub Remote papier — confirmation finale / équipes Quiz — 2026-09-09

- [x] Confirmation déplacée de la carte vers le dernier CTA du sas, uniquement premier lancement officiel papier. Confirmation partagée et submitLaunchSession conservés ; annulation sans intention, doubles clics protégés.
- [x] Vocabulaire équipes Quiz papier limité à la préparation, y compris recherche/création et erreurs ; autres jeux, noms et données conservés.
- [x] Tests : 14 assertions de parcours, 12 matrices de rendu jeu/format/contexte ; suites confirmation JS/PHP existantes vertes, syntaxe PHP et diff checks verts.
- [ ] Recette visuelle DEV du dialog/confirmation, annulation, double clic et inscription ; aucun déploiement/restart.

## Remote Hub — récupération papier et officielle numérique sans rebond — 2026-09-09

- [x] Recette DEV 09/09 : diagnostic logs Hub307/session27758 (Blind Test numérique). À17:16:28,17:16:39,17:17:04, clic Master Reprendre → GET `/master/<session>` sans paramètre →302→ Hub Master200. `games_hub_master_card_action_resolve` renvoie un lien direct pour focus actif, construit sans contexte d’exécution ; `app_orga_ajax.php` redirige cette entrée legacy vers le Hub. Aucun patch de ce défaut dans la passe de diagnostic.
- [x] Reprise active numérique corrigée localement sur Hub : service canonique, exécution conservée, nouvelle génération, Pause sans auto-start, refus contexte périmé. Master autonome ; Remote utilise le même contrat. [Audit et tests](../../../notes/hub-active-master-resume-2026-09-09.md).
- [x] Validation après interruption : 42 contrôles PHP actifs, bootstrap/registration JS, confirmation avec double clic/refus/retry et test démo verts ; fixture DOM complétée (setAttribute/removeAttribute). Aucun changement supplémentaire du contrat runtime.
- [ ] Recette DEV intégrée Master/Remote après livraison opérateur ; tests isolés verts, ancien échec UX démo reproduit avant patch. Aucun déploiement/restart.

- [x] Wording papier ajusté après retour utilisateur : reprendre la diffusion depuis l’ordinateur, puis retourner à la soirée/événement ; helper commun aux deux messages, comportement inchangé.
- [x] Audit : garde exécution/génération déjà présente ; reprise canonique publie une nouvelle génération. Aucun changement du routing Hub nécessaire.
- [x] Bouton explicite Hub papier étendu le 09/09 aux sessions numériques `runtimeMode=official`, sur absence primary/refus quit. Démo numérique/mode absent exclus. Wording identique au papier, sur demande utilisateur : « Pour reprendre la partie, reprends la diffusion depuis l’ordinateur et retourne à la soirée [ou à l’événement]. » Marqueurs, reprise et absence de commande métier conservés.
- [x] Extension couverte par 46 scénarios locaux papier/numérique, trois jeux, deux contextes/notices, démo/métadonnées absentes, reconnexion, fallback natif et tuple/génération. Suites anti-rebond et transition vertes ; syntaxe ES module et diff checks verts. Tests isolés, aucune recette serveur revendiquée.
- [ ] Recette DEV papier et numérique officiel : perte Master, retour volontaire, Hub stable/reload, Master revenu puis Reprendre ; vérifier même exécution, nouvelle génération et programme mixte avec papier jouable. Aucun déploiement ni restart.

## Remote Hub — wording absence Master — 2026-09-09

- [x] Patch présentationnel commun des deux messages, variantes papier/numérique avec les états existants ; hors Hub inchangé. Aucun appel serveur ni comportement modifié.
- [x] 18 contrôles locaux : 16 variantes Hub/hors Hub × papier/numérique × SweetAlert/fallback × deux événements, comparaison des effets avec HEAD ; handlers GAME_RESUMED et quit normal identiques. Syntaxe JS validée.
- [ ] Recette visuelle DEV après livraison ; patch local uniquement, aucun déploiement ni redémarrage.

## Migration du code Hub vers main — 2026-09-09

- [x] [Audit branches/code et manifeste candidat](../../../notes/hub-prod-code-migration-2026-09-09.md) : Fast-forward simulé ; 32/34 suites vertes, deux assertions QR/fixture datée diagnostiquées (contre-sonde datée verte), whitespace signalé. 46 fichiers applicatifs dont 2 images source optionnelles ; dépendance Global et livraison du bridge avant WS.
...
## Sécurisation lancements/reprises officiels Hub — 2026-09-08

- [x] Audit documentaire raw, journal AI Studio et chemins Master/Remote/Organizer ; aucun rechargement ciblé annoncé, aucune DB nécessaire.
- [x] Confirmation partagée, locale avant mutation, uniquement au premier lancement officiel du Hub : trace persistante Hub et repli sur preuves canoniques chargées. Wording Hub unique papier/numérique ; tests/démos, sessions suivantes et reprises exclus, papier Remote-only.
- [x] Garde d’autorité officielle Hub sur métadonnées existantes pour diagnostic automatique et QR papier ; historique/Dashboard mobile préservés.
- [x] Tests de projections Hub (première officielle, suivante après lancement/fin/suspension, démos seules, trace durable), wording unique, handlers réels VM et non-régressions ciblées ; échec grand QR préexistant reproduit sur ancien renderer.
- [ ] Recette DEV authentifiée de la matrice et vérification visuelle Master/Remote. Voir [audit](../../../notes/hub-launch-resume-ux-2026-09-08.md).

## QR Hub 296 — éligibilité réversible — 2026-09-08

- [x] Complément UI confirmé par l'utilisateur : serveur éligible, seul podium27718 visible, session27731 restante. Aperçus historiques marqués explicitement côté Games ; priorité des autres podiums conservée ; hub_idle masque les aperçus. Aucun nouveau patch Global.
- [x] Test Games enrichi avec vrai rendu carrousel + contrôleur : sélection27718, poll favorable même révision, ajout/reload, clic explicite, hub_idle, refresh et podium global actif/fermeture. Ancien JS échoue sur le cas27731 ; version corrigée passe. Suites QR, settings Master, séparation présentation/runtime, UX, polling Remote et syntaxe vertes. Assertion Remote HTML découverte déjà en échec conservée hors périmètre.
- [ ] Recette du complément UI : livrer ensemble PHP/JS Games puis vérifier le cas confirmé 296 ; aucune livraison ni redémarrage effectués ici.

- [x] START/sitemap/index/manifest R11 et journal AI Studio raw relus ; aucun fichier QR ciblé signalé hors workspace. État déployé non vérifié.
- [x] Helper de suite officielle jouable/reprenable, retrait du veto podium historique ; priorités effectivement affichées et protections de commandes conservées. Aucun changement DB/présentation/historique.
- [x] Tests ajoutés : Global `hub_player_qr_continuation_test.php` (vraie policy/state + statuts canoniques) ; adaptation des suites commandes/stockage QR et Games `hub_player_qr_test.mjs` (réactivation Master/Remote sans commande automatique).
- [x] Tests QR mémoire et SQL isolé, présentation/séparation runtime, contrôle Remote, settings Master, UX Master/Remote et polling JS validés ; syntaxe et diff checks validés. Le contrat Remote conserve un échec préexistant à la ligne 330 : assertion textuelle attend l'ancien ternaire découverte dans le QR, désormais rendu dans une branche active séparée. Test et renderer concernés inchangés dans cette passe ; aucune correction hors périmètre.
- [ ] Recette DEV : Hub 296 A terminée/B prête, sortie de podium, reload, Remote, fin de programme puis ajout, reprise suspendue. Pas d'accès DEV/PROD ni navigateur intégré ; aucune recette réelle revendiquée.
- Docs : README Games/Global, contrat Canvas QR, TASKS Games/Global, Handoff, Changelog, routing du nouveau test et index générés. Aucun déploiement ni redémarrage.
...
## Audit continuité joueurs Hub / Pro mobile — 2026-09-07

- [x] Audit fonctionnel local consigné dans [le rapport](../../../notes/hub-mobile-player-continuity-audit-2026-09-07.md) : route individuelle sous 992 px, absence d'orchestration Hub au clic historique, sélection papier active et numérique par parcours/focus, mappings et limites de rendu.
- [x] START/sitemap/manifest/index/README/TASKS main et develop et journal AI Studio consultés ; aucun fichier ciblé signalé hors workspace. Écart documentaire main/develop, état serveur non prouvé. Quatre contrats locaux verts ; aucun patch fonctionnel, commit ou déploiement.
- [ ] Évolution mobile reportée par l’utilisateur : aucun correctif de continuité à exécuter. Réinscriptions historiques acceptées, compteur Hub 2 confirmé ; voir audit distinct des résultats Play Hub 291.

## Complément UX QR prospect — 2026-09-07

- [x] Véritable QR joueurs remplacé par illustration statique non scannable, carte unifiée et wording canonique soirée/événement. Guards serveur et QR actif/télécommande/historiques conservés.
- [x] Rafraîchissement et réponse retardée couverts : ancien dialogue fermé puis retiré, fond/focus restaurés, commandes prospect bloquées, dialogue actif conservé.
- [x] Vrai renderer PHP prospect/actif et tests JS adaptés ; onze suites ciblées, lints et diff check OK. [Rapport du complément](../../../notes/hub-launch-test-audit-2026-09-07.md#complément-ux--qr-joueurs-prospect-réellement-neutralisé--2026-09-07). Correctifs démo/branding DEV préservés ; audit précédent non rouvert.
- [ ] Recette DEV du complément : scan, clic/agrandissement/refresh, soirée/événement, desktop/étroit, QR actif et télécommande/démo. Aucun déploiement.

## Clôture assertions QR/notice Master — 2026-09-07

- [x] Trois assertions obsolètes confrontées aux décisions UX raw du 07/09 : QR visible verrouillé, Mode découverte, phrase pédagogique exacte sous QR. Aucun défaut fonctionnel établi ; [preuves et assertions exactes](../../../notes/hub-launch-test-audit-2026-09-07.md), section « Clôture des trois assertions QR/notice ».
- [x] Games hub_demo_runtime_surfaces_test.php teste désormais le vrai SSR prospect/actif, visibilité/lock/accessibilité/actions, copie exacte et placement. Suite, test UX associé, lint et diff check verts. Aucun code applicatif modifié.
- [x] Ancien paragraphe README Games harmonisé avec la décision UX actuelle ; Handoff/TASKS et rapport actualisés, sitemap/index régénérés et relus. Démo et branding validés DEV préservés ; aucun déploiement. Écarts clos.

## Branding Hub — synchronisation et footer Play — 2026-09-07

- [x] Audit ciblé Master/Play/Remote et source Pro/cascade Global, contrats raw et journal AI Studio relus ; aucun écart concret exigeant rechargement. [Rapport](../../../notes/hub-branding-sync-2026-09-07.md).
- [x] Patch local : empreinte branding dans préparation Remote ; formulaire Play réutilise le compteur avec branding optionnel, footer unique avant/après inscription ; révision média limitée au branding, suppression et liens de police synchronisés. Commandes, saisie, identité et correctifs démo préservés.
- [x] Deux tests branding PHP/JS verts, suites UX/routage/anti-rebond/démo vertes ; mêmes trois assertions QR/notice préexistantes en échec, inchangées. Lints et diff check OK.
- [ ] Recette DEV : Pro et trois surfaces ouvertes, Play avant/après inscription, ajout/remplacement/suppression, même chemin avec version média changée, pseudo en cours, footer mobile/desktop, commande Remote. Aucun déploiement ; validation locale seulement.

## « Lancer un test » — 2026-09-07 — correctif local après audit Hub 292

- [x] Audit et complément Master/Remote conservés dans [le rapport](../../../notes/hub-launch-test-audit-2026-09-07.md) ; contrats raw et journal AI Studio relus avant patch, aucun écart concret de fraîcheur identifié.
- [x] Readiness preflight : provenance, exécution ouverte, Hub/source/runtime/intention/génération exacts ; type réel du résultat Remote et runtime persistant du relais Master ; callback supplémentaire articulé avec la voie canonique et vérification du marqueur, sans acquittement artificiel.
- [x] Player démo indépendant de la présentation officielle, garde après await et retours terminaux conservés ; focus, présentation, roster et Programme inchangés.
- [x] Deux nouveaux tests dynamiques : producteurs réels et écritures SQL en mémoire jusqu’au routage, poll réel retardé et handlers terminaux avec HubTransition ; rouge avant/vert après. Suites officielles, prospect, reprise et anti-rebond vertes ; lints OK.
- [x] README, Handoff, Changelog et rapport actualisés ; sitemap/index régénérés et relus. Patch local uniquement, aucune DB ni déploiement.
- [x] Recette navigateur DEV confirmée après 14h40 : H292, source27697/présentation27698/focus0 ; Master→27702 génération2, Remote commande511→27703 génération3 ; historiques Remote chargés, Player maintenu 45/73 s, fin naturelle 27703 puis retours sans rebond observé. Logs corrélés dans le rapport ; marqueur DB/JSON routing non lus directement. Autres jeux et témoins navigateur officiels/prospects non détaillés par ce lot, tests locaux conservés.
- [x] Trois assertions QR/notice préexistantes réauditées lors de la clôture : obsolètes selon les décisions UX du 07/09 ; remplacées par une couverture SSR précise, suite désormais verte. Aucun patch fonctionnel.

## RECETTE RESTANTE 2026-09-07 — UX Remote / Master et grand QR
...
- Correctif du faux podium QR (07/09, Hub 288) : `session_priority` ne déduit plus une présentation prioritaire du seul événement `hub_execution_completed`, qui peut concerner une démo. Il utilise la session canonique appartenant au Hub, son état terminé et le contexte de résultats avec podium utilisable (has_podium, rang 1–3, nom non vide), comme le rendu Master. Une source officielle non terminée ou sans podium laisse Agrandir disponible ; runtime actif et vrai podium restent prioritaires. Aucun write de réparation du choix, de la trace officielle ou de la fenêtre ; pas de réagrandissement forcé d’un choix reduced. Tests mémoire Global enrichis et verts ; livraison du helper Global à prévoir, aucun SQL exécuté ni déploiement.

- Correctif QR Master du 07/09 après réponse utilisateur `QR_REVISION_STALE` sur revision 23 : le Master applique le clic sans attendre une confirmation serveur ; celle-ci reste requise pour la Remote. Les clics locaux rapprochés remplacent la cible visible, avec une seule commande en vol ; la suivante attend le terminal de sa propre commande et sa révision appliquée exacte. Aucun rebasing au-delà d’une révision métier/concurrente, aucune relance automatique après conflit réel ; snapshot frais renvoyé sur refus Master et ancienne confirmation invalidée. Délai global de 20 s, queue/polls, instance/CSRF, CAS et priorités conservés. Tests mémoire Global et JS verts, aucun SQL ni déploiement. Livraison coordonnée helper QR Global puis asset JS Games ; recette DEV clics rapides et lancement concurrent restante.
- [x] Audit raw et code ciblé, journal AI Studio relu HTTP 200 (28/08, aucun chemin Hub récent trouvé ; pas de preuve de synchronisation serveur).
- [x] Remote : textes multiligne, lieu sans répétition, vignettes de parties, cohérence PHP/JS ; éligibilités et routage conservés.
- [x] Master prospect : Mode découverte, phrase exacte, QR indisponible et aucune ancienne notice/CTA commercial.
- [x] Master panneau droit révisé selon la dernière consigne : colonne/largeur conservées, zones Joueurs inscrits et Classement général permanentes, zéro discret dans l’en-tête et phrase de classement vide. Le grand compteur est masqué uniquement à zéro confirmé ; erreurs/inconnus préservent le dernier état. Classement connu conservé et nouveau classement intégré au refresh existant, sans nouvelle lecture/poll ; DOM identique conservé.
- [x] QR actif réduit : SCANNE POUR JOUER, typographie commune avec les titres Lots, y compris après refresh ; QR prospect inchangé et règle appliquée aussi au grand QR.
- [x] Tests PHP/DOM/Remote ciblés complétés et exécutés ; documentation de la proposition QR.
- [x] B autorisé et implémenté localement : grand QR, commandes Master/Remote idempotentes via queue, confirmation révision/instance, délais, priorités et repli sans stockage ; aucun ALTER automatique ni migration rejouée. A/C/D conservé. Voir [audit](../../../notes/hub-remote-master-ux-audit-2026-09-07.md).
- [x] Tests QR PHP/SQL réels sur MariaDB local isolé (absent/partiel/complet, conflits/retries, instances, succès/refus, retour/podium et pannes) et contrôleur/DOM JS ; contrats Play/Remote/démo/papier/instances, lints et diff check verts.
- [x] Complément UX QR : superposition indépendante sans redimensionner le Hub, SVG à 72 % de hauteur utile borné par largeur/espace des commandes, QR latéral sans double padding, CTA sous le QR et barre utilitaire libérée. Focus/Tab/Échap accessibles.
- [x] Master : effet local immédiat, dernier clic protégé des anciennes réponses, réconciliation sur refus/conflit/expiration ; priorité métier annule l’intention temporaire. Remote : pastille fixe avec indicateur, commandes regroupées, erreurs hors hero. Queue/contrats Global/polls inchangés, aucun SQL exécuté.
- [x] Tests QR contrôleur/DOM et SSR actif/prospect enrichis ; suites Master/Play/Remote et lints PHP/JS, diff check verts.
- [x] Composition finale : carte QR opaque arrondie, centrée sur tous les écrans ; un seul QR visible avec emplacement latéral réservé, inert/aria-hidden et restauration du focus après refresh. Hero Remote titre/connexion, lieu, planning, compteurs, boutons secondaires égaux et conditionnels ; logo unique effectif.
- [x] QR manuel uniquement et carte centrée : arrivée réduite quelle que soit la fenêtre ; suppression des helpers d’accueil automatique et de leur lecture historique. Restauration d’un choix explicite par reçu de queue réussi à révision courante, y compris après remplacement avec nouvelle confirmation obligatoire ; ancien expanded automatique exclu. Centrage permanent du viewport, carte/style/masquage latéral/effet immédiat et A/C/D conservés. Tests mémoire initialisation/reload/retour, CSS centrage et suites ciblées ; aucune migration, aucun SQL distant, aucun déploiement.
- [ ] Recette visuelle Master large/plein écran/tablette paysage, scan téléphone/Échap, Remote 360/768/large et zéro/une/deux commandes, attente stable ; recette avant → ouvert, réduction/reload, après cutoff/terminé/inconnu. Aucun navigateur intégré ni déploiement.

- [x] Préparer les lectures phpMyAdmin et le mécanisme de migration manuelle QR, autorisés localement ; [runbook](../../runbooks/hub-player-qr-migration.md).
- [x] Retrouver le DDL documenté et compléter avec les relevés DEV ; préparer migrations/hub_player_qr_dev.sql, ajout conditionnel contrôlé et test local jetable.
- [x] Import phpMyAdmin DEV réalisé par l’utilisateur sur MariaDB 10.3.39/MyISAM : quatre colonnes ABSENTE → CONFORME, statut final OK_SCHEMA_QR.
- [ ] Recette applicative Hub et contrôle des données après import ; réimport cible non constaté. Livraison B Global → Games puis recette QR et scan téléphone ; PROD reporté à une migration globale distincte.

# Repo `games` — Tasks (journal bref)

## PATCH 2026-09-04 - Hub Remote: routage canonique générationnel

- [x] 2026-09-06 — Retour explicite prioritaire sur un signal serveur ancien : identité propre du signal filtrée par génération strictement postérieure, attente serveur recalculée séparément de la commande locale ; reproduction rouge/vert, expiration et retour Master couverts, navigation inchangée.


- [x] 2026-09-06 — Micro-robustesse bootstrap : terminal de resynchronisation + bouton reload sur erreur avant premier poll, logs console uniques par étape, remplacement/terminaux préservés ; test rouge puis vert, routing/anti-rebond inchangés.
...
- [x] Correctif remplacement 06/09 : exploiter les archives du 04/09, reproduire le fichier nouveau supprimé par l'ancien média, préserver erreurs JSON 400/403 et 413 non JSON ; ne confirmer qu'un média/source serveur non vide, ignorer les polls photo antérieurs à la confirmation et les FileReader annulés.
- [x] Ajouter `hub_photo_replace_test.mjs` (draft/consentement/polls, annulation, A→B→C, erreurs), adapter les deux assertions de signature DOM ; contrats Games/Global et test writer réel rename/unlink verts.
- [x] Remplacement confirmé fonctionnel par l’utilisateur le 06/09 ; auditer sans patch la lenteur avec les deux rechargements courants jusqu’à 22:12:26. Hub 286 : douze rendus Master 3,680–4,614 s, fallback dominant ; snapshots Remote partagent le fallback, dernière sélection lue à +7/+11/+16 s serveur. Retour B→A à 21:51 : véritable écriture canonique, source du second POST inconnue. Voir `notes/hub-photo-latency-audit-2026-09-06.md`. Complément photo : sonde PHP locale fraîche/dirty/session confirme que le fallback omet la photo active, alors que les deux autres chemins la renvoient. Historique : omission antérieure au changement Top 3 du 04/09 ; aucune causalité de déploiement présumée. Correctif appliqué dans la passe autorisée suivante : enrichissement par identité Hub exacte avant dérivation des podiums, sans recalcul du classement.
- [x] Correctifs photo/consolidation confirmés en recette par l’utilisateur, nette amélioration de vitesse ; rebond Master/Play non reproduit. Aucun nouveau diagnostic de rebond requis dans la passe UX/UI.
...
- [x] Auditer la fin de passe UX/UI sans patch : matrice Master→Play (podium/idle/suspension/focus/fin/réouverture), revalidation, Classement et Lots. Capture utilisateur 360×740 inspectée : concurrence texte image/hero ; proposition image intégrale puis Bienvenue/titre sous image, métadonnées conservées. Note `notes/hub-play-ux-end-to-end-audit-2026-09-06.md` ; sondes PHP et contrats PHP/DOM verts.
- [x] Direction UX/UI validée ; complément d’audit du 07/09 terminé sans patch : chaîne Global/PHP/DOM Master, décision partagée Games proposée, ex æquo tous rangs 1–3, initial/poll et récupération réseau bornée.
- [x] Implémenter la passe validée le 07/09 : décision pure Games Master/current_player/poll, hero séparé, transitions et revalidation bornée 3 essais/3,5s/8s, classement illustré tous rangs 1–3, lots backend à rang exact, fallbacks canoniques conservés. Helper et tests PHP/DOM adaptés ; garde du formulaire photo préservée. Tests de handler sans création de participation, absence de lecture média par ligne, réseau/timers/réponses tardives et cycle de page verts. Aucun changement Remote ni cadence, aucun déploiement. Complément : copies Quitter contextuelles / Chargement en cours, masquage des lots par défaut annulé, suppression totale restaure les fallbacks Master. Audit Hub 287 : 415→416 prouvé par logs, mécanisme antérieur d’effacement de preuve. Décision finale appliquée côté Games/Global : pseudo déjà utilisé dans le Hub refusé après départ, nouvelle identité sans historique pour un autre nom libre ; replay actif préservé, aucune fusion. Tests invité/public/handler verts. Recette quitter/réinscrire validée par l’utilisateur le 07/09 : le pseudo utilisé avant départ est bien refusé. Correctif conservé ; cette validation ne vaut pas recette des nouveaux ajustements de classement. Ajustements suivants implémentés : attente générique masquée avec classement sans carte ; ligne personnelle UI exacte, rang absent, zéro seulement prouvé, préparation et homonymes préservés. Tests PHP/DOM ciblés verts ; recette visuelle de ces deux ajustements restante.
- [ ] Recette visuelle interactive 360×740, 768×1024, 992 et large : transitions réseau lent, titres/pseudos/lots longs, photos A→B→C et refresh branding ; vérifier la matrice Master/Play sur DEV après livraison séparée. Navigateur pilotable indisponible dans cette passe.
...
- [x] Complément DEV 09/09 : fallback Global commun Master/Remote, message résiduel Master visible et annoncé, message métier Remote persisté/projeté pour `launch_hub_demo` uniquement. Nouveau `hub_demo_error_feedback_test.mjs` vert, tests settings/DOM/polling/readiness verts.
- [x] `hub_remote_contract_test.php` garde son seul échec QR cliquable, reproduit avec le helper HEAD antérieur au patch ; aucun changement QR dans cette passe.
- [x] Logs DEV du09/09 : créations Quiz Master27749/Remote27751, signaux ready12:39:28/12:43:44, ouvertures Remote200 à12:39:29/12:43:46, retours Hub200 à12:40:04 et12:44:00–01. Bingo Remote27750 et lancement officiel Quiz27741 également observés. Aucun nouvel échec de conversion après12:39.
- [ ] Recette visuelle des erreurs résiduelles et vérification deux joueurs ; non prouvées par les logs serveur.
...
- [x] Audit de parité 09/09 : différence toujours active sur `hub_soiree`, retour local PROD à 800 ms contre attente terminale DEV. [Preuve et portée](../../../notes/hub-dev-prod-parity-2026-09-09.md). Aucun patch Games effectué ; harmonisation à reprendre après validation du flux terminal WS.
- [x] Décision utilisateur 09/09 : **conserver le comportement actuel pour le moment**. Recette PROD sans différence visible avec DEV ; cela est compatible avec un signal serveur rapide et ne prouve pas le recours au secours. [Implications et plan de reprise](../../../notes/hub-dev-prod-parity-2026-09-09.md#remote-bingo--décision-et-reprise-lors-dun-prochain-patch).
- [ ] Prochain patch uniquement : corréler demande/confirmation terminale/navigation ; tester latence, perte et double signal en recette avant de décider du sort du secours. Adapter le test de contrat si harmonisation. Aucun défaut urgent établi ni changement Remote demandé à ce stade.
...
- [x] Reprise 2026-09-08 (locale, non déployée) : memberships complets/partiels/absents, résolution de lecture séparée des writers et garde d'ambiguïté ; aucune normalisation de session historique.
- [x] Préserver lots/branding/publication/probables au bon niveau ; anciennes entrées canonisées et préparation visible quelle que soit l'ancienne offre. Voir [rapport et fichiers](../../../notes/hub-legacy-runtime-compatibility-2026-09-08.md).
- [x] Validation commune : 30 suites PHP vertes sur 31 ; l'échec HTML QR Master du contrat Remote est reproduit avec le renderer HEAD. Lint des 27 fichiers PHP modifiés/nouveaux et diff checks verts.
- [ ] Recette intégrée après livraison autorisée ; préparer séparément PRECHECK, DDL et backfill des 58 Hubs / 68 memberships. Rien n'est déployé ni migré par cette passe.
...
- [x] Actualisé 2026-09-08 : getter de lots sans bootstrap ; consensus complet readonly, divergence propre aux sessions, initialisation Hub vide respectée.
...
### Statut actualisé 2026-09-08 — fonctionnalité conservée en stand-by
- [x] Carte cachée dès le HTML, capacité WS explicite nécessaire au rendu et aux actions sortantes ; flag central OFF côté Blind Test. Anciennes données/podiums toujours lisibles.
- [x] Snapshot individuel Canvas sans purge de `blindtest_session_teams` ; tests PHP stockage simulé et UI JS ajoutés, contrat lobby historique adapté.
- [x] Tests UI/historique, paramètres Hub, papier historique, polling Remote et fin/statistiques verts. Contrat Remote : une assertion QR préexistante en échec, reproduite sur HEAD, hors correctif.
- [x] README, interfaces, Handoff, changelog et génération documentaire actualisés. [Rapport/commandes](../../../notes/blindtest-teams-standby-2026-09-08.md).
- [ ] Recette navigateur après livraison autorisée ; aucun déploiement. Réactivation via chantier dédié uniquement.

La carte décrite ci-dessous est désormais dormante.
