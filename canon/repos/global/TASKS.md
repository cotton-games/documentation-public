<!-- AUTO-UPDATE:BEGIN id="hub-suspend-20260910-global-tasks" owner="codex" -->

**25/09 — Audit autorité suivi d’un patch local Games/WS ; Global inchangé.** Les lectures E/admission existantes suffisent au garde retry ; aucun changement de création E, focus, DB ou politique de réutilisation. Comparaison main/develop/pregame réalisée ; aucun prérequis Global à importer pour le backport proposé. [Rapport et limites](../../../notes/hub-execution-robustness-backport-2026-09-25.md).
...
**25/09 — Reprise Master après suspension : correctif local, non déployé.** Recette Blindtest Hub354/session27995 : la Remote reprend, le Master refuse. Le garde expected_execution_id exigeait à tort le focus actif, pourtant libéré par suspension. Une suspension canonique terminée du même E officiel numérique réutilisable autorise maintenant ce paramètre ; le chemin historique restaure le focus et le grant, sans nouvel E ni Open Players/START. Exécution discordante/absente et suspension inachevée restent refusées. Régression reproduite avant patch puis corrigée (288 vérifications PHP, effets externes simulés). Recette DEV à refaire ; moteur de décision et instrumentation inchangés. [Diagnostic et recette](../../../notes/hub-open-players-foundations-2026-09-24.md#ai-reprise-master-après-suspension--25092026).
...
- [ ] Qualification navigateur DEV après déploiement opérateur du lot cohérent, puis UI finale/fullscreen.


## 23/09/2026 — Audit recette hotfix compte442

- [x] Copies des logs Global corrélées avec Games et les trois WS : client442/Hub349, environnement DEV,30 réponses HTTP200 sur17:17–17:22, sans fatal PHP/SQLSTATE/timeout upstream observé. [Rapport et limites de validation](../../../notes/hub-soiree-hotfix-2026-09-23.md#i-recette-dev-compte-442--hub-349--audit-des-logs-du-2309). Aucun changement Global ni accès DB/serveur.

## 23/09/2026 — Hub session readiness

- [x] Préflight propre, branche opérateur `hub_session_readiness` depuis `hub_soiree`, reprise sélective sans merge. Journal durable E1, réutilisation pending/running, fenêtre configurable, projection Remote de préparation. Événement game_events existant, sans migration.
...
## 22/09/2026 — Navigation Pro des Hubs vides

- [x] Audit préalable RAW/AI Studio, comparaison main/develop, preuve logs + sorties phpMyAdmin utilisateur : retraits27934–27937 puis suppression complète Hub343 à09:29:57 ; pas de réactivation démontrée.
- [x] Prédicat partagé memberships/provenance durable ; Home/Agenda/réseau/first-party/Dashboard, réconciliation sans résurrection ; Quick Add source vide testé, resolver standard aligné sur les retraits achevés, même Hub vide réutilisé.
- [x] Tests locaux de navigation49 contrôles/148 SELECT de fixtures, SchedulePlan68, Quick Add46, suppression162, Hub vide Master/Remote105, régressions et lint ; note/contrats/HANDOFF actualisés, index générés.
- [ ] Recette réelle après livraison manuelle, concurrence HTTP et plans MySQL ; réutilisation Quick Schedule après retrait achevé à vérifier, aucun déploiement ni DB réelle exécutée par l’agent.

[Preuves, fichiers, commandes, échec préexistant séparé et rollback](../../../notes/hub-empty-pro-navigation-2026-09-22.md).
...
- [x] Équipes historiques sélectionnées conservées comme guest typés (`auth_identity_key=team:ID`, K inchangée), source réinjectée jusqu’au moteur. Validation organisateur/source/K avant enregistrement ; adoption legacy uniquement sur la même K validée. Aucun schéma modifié ; lecteurs compatibles nécessaires en cas de rollback après usage.
...
### Retour compte depuis Hub Play et partage — complément 21/09/2026

Signin/signup respectent désormais `hub_account_action=join` même si le joueur EP est déjà connecté : le clic compte depuis Hub Play reprend le helper canonique d’admission puis revient à Hub Play avec `ep_connect_token`, pendant la fenêtre ouverte et sous garde de capacité existante. Le GET `probable` continue vers la fiche EP sans admission ; les POST auth et liens croisés conservent l’intention explicite. Le partage après probable Hub réutilise le renderer historique, vers WWW uniquement. [Correctif, tests et fichiers](../../../notes/play-hub-account-return-share-2026-09-21.md).
...
Résolveur de capacité depuis l’offre effective, garde atomique du roster sous verrou Hub, refus HUB_CAPACITY_REACHED, départ sérialisé, snapshots session ascendants et complément de grilles Bingo. Aucun snapshot commercial Hub ni migration.
...
- [x] Patch local : sélection par publication, fermeture durable par intention, présence/événements des deux surfaces, Bingo sans reset redondant, diagnostics de copie préservés ; UX prospect/client séparées. Complément18/09, expiration involontaire confirmée : identité moteur figée, événement `hub_runtime_expired` idempotent, focus CAS, refus `HUB_RUNTIME_EXPIRED`, aucun CTA/recréation officielle ; tests grâce valide, suspension >1 h et papier/numérique. [Rapport](../../../notes/hub-runtime-expired-2026-09-18.md).
...
- [x] Audit officielle → démo (16/09) : source relue au lancement, copie SQL courante puis proposition indépendante ; ancien runtime réutilisé uniquement pour le même intent. Logs rechargés27831→27838/27839, deux intents distincts, pas de sélection historique traçable. Deux contrats démo verts ; aucun code modifié. Détails/limites dans HANDOFF.

## Remplacement Hub sans critère d'origine — 16/09/2026

- [x] Éligibilité canonique partagée, revalidation sous verrou Hub avant application, moteur plan/apply conservé et contrat de préparation prospect inchangé.
- [x] 70 contrôles mémoire origines/états/concurrence/paramètres ; suites planner/apply, suppression et révisions validées. Assertion de commandes Remote actualisée avec `remove_session` préexistant.
- [ ] Recette concurrence MySQL/runtime réelle ; [audit local non déployé](../../../notes/hub-quick-add-renewal-2026-09-16.md).

## Suspension officielle Hub — 10/09/2026

- [x] Patch local : Journal léger `game_events`, provenance d’exécution ouverte, garde temporelle avant `open_execution`, commande Remote hors fenêtre refusée, reprise conditionnelle du focus et génération causale. Complément18/09 : génération publiée exacte, continuité Remote distincte du takeover et diagnostic corrélé selon le périmètre du repo ; [rapport et tests](../../../notes/bingo-hub-resume-robustness-2026-09-18.md).
...
- [ ] Définir l’autorité des tentatives et son application aux résultats/fallbacks avant tout patch de restart.


### Fallback historique Hub borné aux memberships actifs — 16/09/2026 — patch local

Le blocage de lecture identifié par l’audit suppression est corrigé localement : `app_games_hub_players_stats_legacy_context_get` relit le programme canonique, transmet `allowed_session_ids` aux lecteurs période/événement ; le SELECT initial du contexte historique applique cette borne avant toutes les agrégations. Liste fournie vide = zéro session (`id IN (0)`), option absente = contrat historique inchangé. Le rebuild nominal, les scores Bingo et le stockage photo restent inchangés. La suppression tous états et son runtime/UI ne sont pas implémentés par ce patch préparatoire.

Deux fichiers applicatifs Global et deux tests ciblés modifiés/ajoutés. Six suites PHP passent ; contre-épreuve avant/après hors Hub : 18 contextes complets identiques (compute/période/événement, Blind Test/Bingo, équipes table/snapshot). Ancienne reproduction 101/102 conservée comme régression ; aucune contribution retirée ne revient. Journal AI Studio raw rechargé, identique à l’audit précédent ; aucune resynchronisation serveur ciblée nécessaire. Aucun accès DB réel, déploiement, restart, commit ou push. Docs : README/TASKS Global, HANDOFF, note d’audit résolue, bridge, CHANGELOG et index générés. Recette DB réelle et protocole de concurrence de la future suppression restent à faire.

[Preuves, fichiers, commandes et limites](../../../notes/hub-session-removal-audit-2026-09-16.md).
...
<!-- AUTO-UPDATE:END id="hub-suspend-20260910-global-tasks" -->

<!-- AUTO-UPDATE:BEGIN id="audit-activation-684-20260910-global" owner="codex" -->

## Audit des logs — activation annoncée client684 — 10/09/2026

Contrôle des copies locales `pro/logs/{access_log,error_log}` et `global/logs/{access_log,error_log}` pour le test annoncé client684. Aucune occurrence de684, même après décodage des URL. Séquence PRO du10/09/2026 à16:51–16:52 : step1/step2, webhooks `invoice.paid` puis `checkout.session.completed` pour offre2719, tous deux HTTP200 et `livemode=false`, retour étape3 HTTP200. Les messages Email Suppressed sont attendus en test ; aucun échec checkout/PHP sur cette séquence. Global ne contient aucune trace684/2719 ; dernières entrées à15:21. Le lien684→2719, l’état actif enregistré, les montants/remise et la commande ne sont pas certifiés par ces logs. Aucun code applicatif, log, DB ou Stripe modifié.

<!-- AUTO-UPDATE:END id="audit-activation-684-20260910-global" -->

<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-global-tasks" owner="codex" -->

## CORRIGÉ LOCAL / À RECETTER — Remises2026 affiliés — 10/09/2026

- [x] Appliquer le contrat d’acquisition propre aux surfaces global, conformément aux décisions métier ; source locale déclarée à jour par l’opérateur pour DEV.
...
<!-- AUTO-UPDATE:END id="remises-affilies-20260910-global-tasks" -->

<!-- AUTO-UPDATE:BEGIN id="hub-remote-limited-20260910-canon-repos-global-tasks" owner="codex" -->
...
<!-- AUTO-UPDATE:END id="hub-remote-limited-20260910-canon-repos-global-tasks" -->

<!-- AUTO-UPDATE:BEGIN id="hub-empty-805-canon-repos-global-tasks" owner="codex" -->

## Cas805 / Hub148 — patch local réconciliation, 09/09/2026

- [x] Centraliser la cible vide admissible, conserver racine/annexes/historique et refus réels ; événement conservé avant prepare.
- [x] Verrouiller SchedulePlan et suppression individuelle, relire sous verrou Hub ; compensation fixed non destructive après configuration.
- [x] Reproduction 805 et 52 contrôles verts, tests existants comportement/Quick Hub/temporal verts. Deux assertions statiques Dashboard obsolètes reproduites sur HEAD.
- [ ] Recette DEV avec DB/navigateur et concurrence réelle ; mesurer lecture game_events. [Rapport](../../../notes/hub-empty-reconciliation-2026-09-09.md).
- [ ] Chantier distinct : commandes Remote orphelines après purge manuelle ; aucune cascade/purge dans ce patch.

<!-- AUTO-UPDATE:END id="hub-empty-805-canon-repos-global-tasks" -->

## Reprise numérique active — 09/09/2026

- [x] Service canonique renforcé pour rattacher le Master à une exécution active numérique existante : validation de l’exécution attendue, aucune création/focus/injection dans cette branche, génération idempotente existante, reprise Remote symétrique. [Audit, preuves et suites](../../../notes/hub-active-master-resume-2026-09-09.md).
- [ ] Recette DEV intégrée avec Games ; patch Hub local, aucune DB réelle, aucun déploiement/restart.

## Migration du code Hub vers main — 2026-09-09

- [x] [Audit branches/code et manifeste candidat](../../../notes/hub-prod-code-migration-2026-09-09.md) : Merge simulé sans conflit, ecommerce main=Hub exclu ; 43/44 suites vertes + stockage QR SQL isolé vert. Contrat SchedulePlan contient deux assertions Dashboard obsolètes. 17 fichiers applicatifs candidats ; schéma/backfill séparés inchangés.
...
## Phase 1 schéma Hub PROD — 2026-09-08

- [x] [Audit repris et PRECHECK](../../../tmp/hub-prod-initial-schema-2026-09-08/README.md) : inventaire Hub/programmation et dépendances partagées, colonnes QR/stats intégrées au contrat, historique comparé.
- [x] Correctif status autorisé : définition exacte lue avant ALTER ; schéma conforme 0 DDL au premier et second appel. Dix scénarios mémoire, contre-épreuve ancien code, quatre contrats et lint verts. Format SHOW COLUMNS vérifié sur 10.11 isolé, aucune certification 10.3.
- [x] PRECHECK PROD analysés ; paquet final prêt : 13 CREATE InnoDB (11 Hub + 2 Quick), 210 colonnes/52 index, aucun ALTER legacy. blindtest_session_teams exclue, mode OFF et gardes vérifiés.
- [x] POSTCHECK mécanique et vacuité exacte ; installation/relance/interruption/reprise, vrais helpers zéro DDL et 15 contre-tests verts sur 10.11 isolé (pas certification 10.3).
- [ ] Import opérateur PROD puis retour POSTCHECK ; STOP avant backfill/code. Les émissions des ensures Bibliothèque restent une réserve distincte de livraison code, aucun patch Pro.
- [x] Préparer l'actualisation des sources du backfill dans `/tmp/cotton-hub-audit/10-prod-refresh-readonly.sql` : audits retrouvés dans `tmp/audit-hub-pre-prod/` et `tmp/audit-hub-prod-initial/`, lectures legacy uniquement, partitions candidates et annexes. Les 68 sessions / 58 contextes sont une photographie antérieure, pas un décompte courant.
- [x] Analyser les retours PROD du 08/09 15:45–15:51 : 70 sessions J/futur, 59 Hubs candidats (57 soirées + 2 événements), 19 clients ; +2 sessions 30139/30140 du client595. E05 contient 51 lots dont 12 pour la veille ; faux export SQL en erreur, données disponibles dans le texte. E03 reçu, F01 absent. Analyse locale `/tmp/cotton-hub-audit/RETOUR-REFRESH.md`.
- [x] R01 reçu (795 IDs uniques,70 officiels J/futur), plans R02 ALL/BNL analysés. Préparer le [pack backfill59/70](../../../tmp/hub-prod-backfill-2026-09-08/README.md) : matrice34 colonnes, manifeste figé, SQL20/21/22/23, empreintes toutes colonnes des sources ciblées, rollback avant usage avec reçu. Publication portée par le compte selon clarification utilisateur ; aucun état runtime artificiel ni lot Hub. Les détails R13 ne bloquent plus la préparation structurelle.
- [x] Backfill PROD09/09 appliqué par opérateur et validé par POST22 à06:22:42 UTC :59 Hubs/70 memberships,21 captures complètes identiques au PRE (comparaison locale/digest vérifiés), rattachements et attributs exacts. Création06:21:44 UTC observée via POST ; premier essai1100 annulé0/0 puis corrigé. [Preuves et pack](../../../tmp/hub-prod-backfill-2026-09-09/README.md) archivés, mapping70 retourné/25 visibles et export05 complet facultatif. Veille exclue, aucun déploiement ni SQL exécuté par l’agent ; ne pas relancer21.
- Aucun backfill exécuté, DB existante contactée, commit/push ou déploiement.
...
## RECETTE RESTANTE 2026-09-07 — Compteur confirmé et stockage QR applicatif
...
- [x] Vérifier le COUNT Hub actif et ajouter une sortie optionnelle de lecture confirmée, sans changer l’entier ni la requête des autres appelants.
- [x] Couvrir succès 0/1/plusieurs et échec SQL dans le test isolé Games `hub_remote_master_ux_test.php` ; aucune DB contactée.
- [x] Auditer schéma, présence, queue et historique officiel ; documenter la proposition QR indépendante.
...
- [ ] Recette DEV temporelle et livraison helper QR Global → vues/JS Games ; ancien expanded hors fenêtre, réduction explicite puis reload, état inconnu et Hub terminé.
...
- [x] Exception DEV du 09/09 limitée au test annexe actif : garde du contenu numérique substitué ; le prospect et les duplications ordinaires conservent le refus de la source incompatible.
...
- [x] Complément DEV 09/09 : fallback uniquement Quiz papier incompatible / offre active / test annexe ; garde du contenu substitué avant copie et à l'application, source officielle préservée, catalogue Cotton et planificateur Quick réutilisés, nombre exact puis best effort, zéro ID en dur.
- [x] Nouveau `hub_quiz_demo_substitution_test.php` : vrais resolver/préparateur, planificateur, garde Quiz, matérialiseur et handler Remote avec frontières DB/runtime simulées. Cas compatibles, refus prospect/ordinaire, cardinalité, retry et message Remote couverts ; contrats Quick/démo/readiness existants verts.
- [x] Logs DEV rechargés le 09/09 à12:44–12:45 vérifiés : Hub303/source Quiz papier27748 (création T489,T490,T491 à12:32:16), refus numérique à12:32:44, puis démos27749 Master à12:39:24 et27751 Remote commande558 à12:43:39, ready et ouvertures Remote200. Branding validé par runtime_execution_demo. Aucun fatal PHP/HTTP5xx sur12:30–12:45.
- [ ] Confirmer le nombre de séries/contenu effectif et la recette deux joueurs ; ces champs ne sont pas exposés par les logs examinés. Aucune DB réelle interrogée par Codex.
...
- [x] Prendre la première session officielle configurée selon date/heure/position/id ; copier ses champs puis forcer le numérique et appliquer le contenu de test. Source et contenu de substitution exceptionnel restent distincts.
- [x] Réutiliser le producteur Hub par intention `app_session_demo_create_from_source` (copie bas niveau commune au resolver historique de `session_duplicate`) pour créer la session démo isolée, puis la suggestion quick-add numérique pour remplacer uniquement son contenu/thématique.
...
- [x] Faire exposer par `app_games_hub_player_upsert(...)` / `app_games_hub_player_ensure(...)` les transitions roster `created`, `reactivated` et `already_active`, sans assimiler un mapping session `left` à une désinscription Hub. Complément audit 07/09 : `web/tests/hub_guest_rejoin_test.php` caractérise preuve conservée/perdue, autre navigateur, casse/espaces, garde actif, photo/classement/compteur sans fusion. Logs Hub 287 : 415 avant, 416 après ; décision finale appliquée via `app_games_hub_player_register_guest_from_play` : réservation tous statuts dans le Hub, sans récupération après départ ; replay actif préservé et autres appels internes inchangés. Tests publics/handler, erreurs de lecture et verrou verts. Recette quitter/réinscrire validée par l’utilisateur le 07/09 : le pseudo utilisé avant départ est bien refusé. Correctif conservé ; cette validation ne vaut pas recette des nouveaux ajustements de classement. Aucun changement applicatif Global supplémentaire pour la ligne personnelle UI Games. Voir `notes/hub-play-quit-rejoin-audit-2026-09-07.md`.
...
- [x] Corriger le remplacement (06/09) : nom unique par tentative pour éviter écrasement puis unlink du nouveau fichier ; candidat image vérifié avant bascule ; écriture conditionnelle de l'ID actif contrôlée, refus explicite en cas d'échec/conflit en conservant l'ancien média.
- [x] Reproduire rouge/vert le premier ajout réussi puis A→B cassé, via `hub_photo_replace_test.php` avec vrai `upload_traiter`/delete et fichiers temporaires ; couvrir A→B→C, refus/consentement/échec SQL/exception/conflit/fichier absent et rang 4. Logs du 04/09 corrélés dans la note d'audit photo.
- [x] Audit lenteur du 06/09 sans patch : `hub_players_stats_rebuild_incomplete` à 21:47:01 (`mapping_without_result_count=1`) précède `projection_dirty` durable et 211 fallbacks. Les douze profils Master imputent 96,65–97,78 % au fallback historique ; aucune causalité photo démontrée. Quatorze occurrences Remote établissent le chemin partagé, même sans rendu photo. Note `notes/hub-photo-latency-audit-2026-09-06.md`. Complément photo : sonde PHP locale fraîche/dirty/session confirme que le fallback omet la photo active, alors que les deux autres chemins la renvoient. Historique : omission antérieure au changement Top 3 du 04/09 ; aucune causalité de déploiement présumée. Correctif appliqué dans la passe autorisée suivante : enrichissement par identité Hub exacte avant dérivation des podiums, sans recalcul du classement.
- [x] Données phpMyAdmin Hub 286 reçues : résultat 27677 score 19 consolidé 500/1 ; mapping suivant 27678 actif score 0 causait le faux incomplete. Corriger le diagnostic sans modifier contributions/scores ; enrichir aussi le fallback photo par identité exacte. Tests `hub_stats_pending_session_test.php` et `hub_aggregate_photo_fallback_test.php` rouge/vert, 20→0 fallbacks après vrai rebuild en simulation. Recette DEV après déploiement séparé : récupérer projection via rebuild canonique, mesurer HTTP Master/Remote et propagation DOM. Aucun déploiement dans cette passe. Complément règle résultat : exclusion prouvée tous-zéros/sans phase sans dirty, papier non saisi hors classement confirmé par l’utilisateur ; vide consolidé reconnu sans fallback. Tests quatre cas + Quiz/Blindtest/Bingo complétés et verts ; référence runtime manquante reste une anomalie, aucune preuve de réponse/saisie inventée. Recette photo/consolidation désormais validée par l’utilisateur avec nette amélioration de vitesse. Audit UX/UI suivant sans patch : distinction resolver hub_idle/hub_podium consignée ; complément du 07/09 identifie les consommateurs Global/Remote et propose une composition d’affichage partagée dans Games, aucune modification Global ni routage ; voir `notes/hub-play-ux-end-to-end-audit-2026-09-06.md`.
...
- [x] Actualisé 2026-09-08 : retirer le bootstrap des lectures ; le helper explicite refuse toute divergence et ne copie aucun champ session.
...
- [x] Actualisé 2026-09-08 : ne partager qu’un consensus complet en lecture seule ; conserver les personnalisations divergentes par session.
