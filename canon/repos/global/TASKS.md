# Repo `global` — Tasks
## REPORTÉ SUR MAIN 2026-09-04 - Période courante Stripe
- [x] Confirmer en PROD les deux colonnes `DATETIME NULL` avant tout portage applicatif.
- [x] Reporter uniquement le commit `periode` sur `main` (`2aa944e`), sans chantier Hub/soirée ni hunk DDL parasite.
...
- [x] Valider les lints PHP, les tests Subscription/Invoice/renewal TdR et `git diff --check` avant push.
...
- [x] Retirer l'outil BO transitoire sans modifier les colonnes, le resolver, les webhooks, les affichages ou les fallbacks.

## FUTUR 2026-09-04 - Changement d'offre des clients en propre
...
- [x] Reporter directement depuis `renew_hors_cadre` le moteur contractuel et ses deux tests, sans merge ni fichiers Hub hors périmètre.
- [x] Couvrir snapshot jauge 50 contre Cotton 100, fréquence mensuelle contre Cotton annuelle, montant catalogue indépendant, Price/Product inactifs ou absents, guards propre/cadre/hors-cadre, no-op et snapshot V2.
- [x] Vérifier la parité DEV→main, les lints PHP, les deux tests autonomes et `git diff --check`.
- [ ] Après déploiement code PROD, vérifier l'observabilité avant activation manuelle des événements Stripe manquants.
- [ ] Normaliser séparément et manuellement les trois subscriptions legacy Beer’s Corner.

## DOC 2026-07-27 - Ecommerce: snapshot remises decimales
- [x] Documentation livree:
  - canoniser `ecommerce_offres_to_clients.remise_pourcentage` en `DECIMAL(5,2) UNSIGNED`;
  - documenter que `ecommerce_offres_to_clients` est un snapshot contractuel;
  - rappeler que le moteur facture calcule volontairement le TTC depuis `prix_reference_ht` et la remise exacte;
  - préciser que le moteur runtime ne doit pas etre transforme vers un calcul depuis HT net snapshoté sans analyse metier.
- [x] Validation documentee:
  - cas incident `69,90 HT`, remise `28,61 %`, net `49,90`, TVA `9,98`, TTC `59,88`;
  - cause racine bornee a la perte de decimales dans le snapshot.
- [ ] Suivi hors documentation:
  - verifier que les prochains exports structure-only DB restent alignes sur `DECIMAL(5,2) UNSIGNED`.
