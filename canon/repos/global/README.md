## Update 2026-09-04 - Futur changement d'offre Cotton → Stripe → Cotton
...
La V1 n'autorise qu'une hausse stricte de jauge sans réduction de durée de période. Le Customer Portal n'est pas le parcours moderne de changement commercial. Les offres déléguées hors cadre sont exclues et conservent leur contrat snapshot/repricing au renewal.

## Update 2026-09-04 - Période de facturation Stripe persistée

Les offres adossées à une subscription Stripe disposent de deux champs dédiés, `stripe_current_period_start` et `stripe_current_period_end`. Le résolveur partagé lit en priorité les périodes portées par `subscription.items.data[*]` (forme Basil): une borne partielle est invalide et plusieurs items ne sont acceptés que si leurs deux bornes sont identiques. En l'absence de période sur les items, il retombe sur les propriétés racine legacy. Une forme absente, incomplète, invalide ou ambiguë n'est jamais persistée.
...
La période d'une facture est un contrat distinct. Lors de `invoice.paid`, Global sélectionne uniquement les `invoice.lines.data[*].period.start/end` des lignes récurrentes rattachées à la Subscription facturée. Toutes les périodes pertinentes doivent être identiques; une divergence est refusée comme ambiguë. Un payload de lignes tronqué déclenche un GET ciblé paginé de toutes les Invoice Lines. La période résolue est transmise à la création de commande puis figée dans `ecommerce_commandes_lignes.nom`; elle n'est jamais relue depuis `stripe_current_period_start/end`. En l'absence de période Invoice fiable, le calcul Cotton legacy reste le fallback non cassant, avec logs explicites.

## Update 2026-09-03 - Support et assiette contractuels du renewal hors cadre

Le renewal d'une offre déléguée hors cadre lit exclusivement le snapshot Stripe `cotton_contract_*` courant. L'assiette `cotton_contract_base_ht_cents` reste autoritaire et seule la remise réseau courante varie automatiquement. L'offre, la jauge, la fréquence et le tarif Cotton courants ne fournissent aucun fallback.

Le snapshot offre/jauge/fréquence construit un lookup `ABN{jauge}M|A`. Le Price catalogue doit être actif, unique et compatible en devise/récurrence, et son Product doit être explicitement actif. Ce Price sert uniquement de support technique: son `unit_amount` est ignoré pour calculer le Price net. Toute metadata ou résolution incomplète échoue fermée avant write.

Le snapshot représente le dernier état contractuel explicitement validé, stable entre intentions commerciales. Une future intention Stripe pourra le remplacer atomiquement; les renewals suivants consommeront alors cette nouvelle version. Le workflow de remplacement, les SubscriptionSchedules et la migration des subscriptions legacy ne font pas partie de ce patch.

## Etat 2026-07-27 - Ecommerce: remises decimales preservees dans les snapshots

Le snapshot contractuel `ecommerce_offres_to_clients` conserve maintenant les remises applicables avec leur precision decimale.

Contexte incident:
- `ecommerce_offres_to_clients.remise_pourcentage` etait historiquement en `tinyint(3) unsigned`;
- une remise source `28,61 %` pouvait donc etre stockee en `29`;
- le moteur de facturation recalculait alors le TTC depuis `prix_reference_ht` et ce taux degrade, ce qui produisait un TTC Cotton inferieur au paiement Stripe.

Decision de schema:
- `ecommerce_offres_to_clients.remise_pourcentage` est canonise en `DECIMAL(5,2) UNSIGNED`;
- ce choix aligne le snapshot offre client avec `ecommerce_remises_to_offres.remise_pourcentage` et `ecommerce_commandes_lignes.remise_pourcentage`;
- les snapshots actifs corriges doivent conserver le taux exact applique au contrat, pas seulement une valeur arrondie pour affichage.

Invariant de facturation:
- le moteur runtime existant reste volontairement base sur le prix de reference HT et la remise exacte pour calculer le TTC;
- avec `prix_reference_ht=69,90`, `prix_ht=49,90`, `remise_pourcentage=28,61` et `TVA=20`, le runtime produit `TTC=59,88` et `TVA affichee=9,98`;
- ne pas remplacer cette logique par un recalcul systematique du TTC depuis le HT snapshoté sans analyse metier, car elle sert aussi a rester alignee avec les arrondis Stripe quand le taux exact est conserve.

Fichiers de reference:
- schema canon: `documentation/dev_cotton_global_0.sql`, `canon/data/schema/DDL.sql`;
- moteur prix/facture: `global/web/app/modules/ecommerce/app_ecommerce_functions.php`.
