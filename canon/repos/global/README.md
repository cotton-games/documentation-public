<!-- AUTO-UPDATE:BEGIN id="hub-suspend-20260910-global-readme" owner="codex" -->
...
[Contrat, fichiers et vérification](../../../notes/hub-open-players-foundations-2026-09-24.md). Aucun déploiement ni validation navigateur DEV effectués.


## 23/09/2026 — Préparation Hub E1

**Branche `hub_session_readiness` : NON DÉPLOYÉE EN PROD, exclue du hotfix `hub_soiree`.** Les recettes DEV et tests locaux ci-dessous ne valent pas livraison PROD. Journal durable E1, réutilisation pending/running, fenêtre configurable, projection Remote de préparation. Événement game_events existant, sans migration. [Contrat, reprise sélective, fichiers, validations et limites](../../../notes/hub-session-readiness-2026-09-23.md). Les descriptions sas_players antérieures ne constituent pas le contrat de lancement de cette branche.


## 22/09/2026 — Sessions retirées d’un Hub : surfaces publiques

**Déploiement PROD prévu par l’opérateur ; confirmation de réalisation attendue.** Correctif postérieur au lot `hub_soiree` livré. WWW/Play appliquent le critère de navigation Pro : membership active valide, ou véritable session autonome sans membership ni trace de retrait. Listes et anciennes URL filtrées ; historique personnel et données conservés. [Périmètre, fichiers, tests et livraison Global avant WWW/Play](../../../notes/hub-public-removed-sessions-2026-09-22.md).
...
## Hub vide : navigation Pro et provenance de retrait — 22/09/2026, local non déployé
...
La présence historique d'une session dans `championnats_sessions` ne suffit pas à rendre cette session membre du programme Hub courant. Les surfaces Pro générales respectent l'état canonique des memberships Hub pour déterminer le programme visible. Sans membership, une trace `hub_execution_removed` interdit aussi le fallback autonome et la réconciliation legacy ; cette preuve survit à la suppression complète du Hub.

`app_games_hub_pro_navigation.php` produit les prédicats SQL sans DDL/écriture/cache : membre actif dans Hub actif/non deleting du même compte, ou vraie session autonome sans relation ni retrait journalisé. `app_games_hub_sessions_reconcile` exclut aussi ces retraits durables ; les règles d’identité du resolver restent inchangées.

Quick Schedule réutilise l’unique Hub vide compatible : les sessions retirées conservées historiquement ne bloquent plus la date. Une membership active, même mal configurée, impose le Dashboard. Une exécution retirée ne cesse de bloquer qu’avec son intent et son accusé de retrait achevé correspondants ; retraits incomplets, exécutions ouvertes, commandes en attente et gardes temporels restent bloquants. Identité/événement et annexes conservés, aucune ancienne membership réactivée. Un Hub physiquement supprimé n’est pas recréé par un GET. [Audit et tests](../../../notes/hub-empty-pro-navigation-2026-09-22.md).
...
### CTA de participation Hub — révision du 21/09/2026 (local, non déployé)

Pour une session/soirée Hub, l’EP permet de déclarer ou retirer une participation probable, y compris pendant la fenêtre d’exécution. Il ne constitue plus une porte d’admission au Hub.

- Fiche session, agenda et page EP Hub partagent `app_games_hub_participation_cta` : « Participer à la soirée/à l’événement », puis « Participation annoncée » et « Annuler ma participation », avant comme pendant la fenêtre. Les POST revalident contexte, identité EP et CSRF. Une S2 terminée ne bloque pas les actions du Hub encore pertinent. Hub fermé/inactif : aucune action.
- Lecture individuelle exclusivement dans `games_hubs_participations_probables` avec `app_games_hub_probable_get_by_ep(..., false)` ; le fallback legacy reste disponible pour les autres appelants. Aucune probable ne crée/réactive `games_hubs_players`, ne consomme de place ou ne garantit l’accès au runtime. Déclaration possible à jauge 50/50.
- WWW session réutilise le CTA existant vers `/signin/public/{session_token}` : connexion/création de compte puis fiche EP source. Les boutons Hub supplémentaires des cartes/pages de consultation classiques sont retirés ; leurs liens de détail restent présents. Le contexte Hub sans session utilise `hub_account_action=probable`, qui revient à la page EP Hub sans admission même si la fenêtre s’est ouverte entre-temps. Intention absente/invalide : `probable`.
- L’accès direct Hub Play depuis WWW est réservé au contexte QR permanent `/place/{code_client}` et uniquement pendant la fenêtre canonique ouverte (J 00 h → J+1 midi exclu). `qr_place=1` propage une intention de navigation, sans preuve de présence. Accès invité possible. Filtrage J/J-1, publication et dédoublonnage conservés ; plusieurs Hubs ouverts ne sont pas départagés arbitrairement.
- Les sessions autonomes, y compris accès EP historique le jour J, restent inchangées. QR Hub/session et anciennes URLs Player restent des admissions terrain. L’intention explicite `join` du parcours compte issu de Hub Play conserve son helper d’admission ; aucune page WWW classique/EP ne la génère comme action de participation.

**Réconciliation du 31/07 :** les descriptions historiques « EP Hub → join pendant fenêtre → Hub Play » dans les sections du 31/07 ci-dessous sont remplacées par ce contrat. La page EP Hub ne propose plus `hub_ep_join` ; l’intention probable ne devient plus une présence runtime. Seul le retour d’authentification d’une admission explicite depuis Hub Play reste distinct. Les sections historiques hors blocs de maintenance sont conservées comme historique, pas comme contrat actuel.

Les probables équipe Quiz legacy restent distinctes ; aucune conversion en probable individuelle. Le préchargement/compteur legacy et le masquage des branches équipe par le CTA individuel Hub préexistaient au patch du 20/09 et restent inchangés. [Comparatif, tests et limites](../../../notes/hub-access-probable-only-2026-09-21.md).
...
Service commun `app_games_hub_session_test_launch(hub, session_id, intent)` sous verrou Hub : membership, propriétaire, source officielle prête, type et état de démarrage de la source revalidés. Aucun resolver « première officielle » dans ce chemin. Le ledger `game_events/hub_demo_launch_intent` réserve un token stable avant copie ; un retry retrouve et répare la même copie après panne partielle. Aucune migration. Les contrats prospect et Pro historiques sont conservés.

[Audit et validation](../../../notes/hub-session-targeted-test-2026-09-17.md).


### Remplacement de contenu Hub — 16/09/2026, local non déployé

`app_programming_theme_renewal_hub_session_is_eligible` centralise l'éligibilité Master/Remote sans origine Quick Add : propriétaire concordant, Hub actif, configuration complète, contenu compatible Quiz5/Blind Test4/Bingo3-6, pending canonique, aucun focus/suspension/exécution officielle, fenêtre before/open. Le catalogue/planner revalide les alternatives ; la préparation sans offre active existante est conservée, le lancement conserve sa garde commerciale.

Pour un scope Hub, `app_programming_theme_renewal_apply_for_session` prend le verrou existant partagé avec lancement/retrait, relit Hub/membership/éligibilité et refuse un retrait journalisé. Le moteur historique est conservé sans modification sous `_unlocked`, avec CAS, paramètres indépendants et matérialisation Bingo sans attribution de grilles. Hors scope Hub, chemin historique direct. Aucune modification du commit Quick Add ni des producteurs de révision. [Audit et tests](../../../notes/hub-quick-add-renewal-2026-09-16.md).

### Retrait individuel Hub — 16/09/2026, local non déployé

`app_games_hub_session_remove` revalide propriétaire, membership et état sous le verrou Hub partagé avec lancement/reprise, suppression historique et rebuild persistant. Non-jouée : `app_session_delete_canonical` et ses nettoyages. Suspendue libérée/terminée : journal idempotent `hub-remove-{hub}-{session}` / `hub_execution_removed`, membership inactif, mappings `left` sans purge de participation/photo, présence et commandes de lancement/sélection ciblées invalidées. Le journal de suspension reste intact pour conserver le gel jusqu’au cleanup de fin de fenêtre. Le contexte runtime exige le membership actif ; le lancement et le rattachement explicite refusent le retrait journalisé.

Rebuild réel synchrone, puis révision `date_maj` et ACK durable `hub_removal_completed`. Échec partiel : réponse retryable, intention conservée, nouvelle demande reprend les étapes. Les tables MyISAM excluent une promesse de transaction globale ; les autres writers de rebuild prennent le même verrou. Le fallback historique borné reste inchangé. `presentation_session_id` est effacé conditionnellement ; le resolver existant choisit un autre membre ou le Hub neutre sans lancement. La résolution du programme Pro reconnaît exclusivement les memberships inactifs dotés de cette intention, pour ne pas confondre retrait et membership manquant.
...
Journal léger `game_events`, provenance d’exécution ouverte, garde temporelle avant `open_execution`, commande Remote hors fenêtre refusée, reprise conditionnelle du focus et génération causale.

## Fallback historique Hub — contrat local du 16/09/2026

**Fallback historique Hub = historique borné aux memberships actifs du Hub.** `app_games_hub_players_stats_legacy_context_get` relit `app_games_hub_sessions_get`, même si le rendu lui fournit une ancienne liste, puis transmet `allowed_session_ids` aux lecteurs historiques période et événement. `app_client_joueurs_dashboard_context_compute` applique `AND id IN (...)` au SELECT initial de `championnats_sessions`, avant chargement des mappings, scores, phases et snapshots d’équipes. Liste explicitement vide = `id IN (0)`, donc contexte sans classement, podium ni participation ; sans option, les lecteurs historiques hors Hub conservent leur contrat.

Cette borne s’applique quelle que soit la cause du fallback (dirty, erreur, projection absente/incomplète, schéma indisponible). Aucun filtre après agrégation. Le rebuild nominal et le lecteur persistant sain ne changent pas ; aucune photo supprimée, aucun score Bingo modifié. Les descriptions historiques ci-dessous du fallback client/période se lisent désormais avec cette borne supplémentaire pour le seul appelant Hub.

Préparatoire, non déployé : ne livre ni suppression tous états, ni cleanup runtime, ni UI. La liste est relue au début du fallback ; ce patch ne crée pas de transaction/verrou avec une future suppression concurrente. [Audit résolu et validation](../../../notes/hub-session-removal-audit-2026-09-16.md).

<!-- AUTO-UPDATE:END id="hub-suspend-20260910-global-readme" -->

<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-global-readme" owner="codex" -->

## Remises 2026 — achats propres des affiliés — 10/09/2026

**Patch local pour DEV, non déployé.**

L’affiliation réseau seule n’exclut plus une acquisition propre de Remises 2026. L’offre ABN12 périodique doit appartenir à l’acheteur chargé côté serveur, non-siège, avec délégation explicitement zéro et sans contexte délégué/invalide ni support. Sièges, offres déléguées, supports et auto-délégations legacy restent hors scope standard. Les anciennes descriptions excluant tous les affiliés décrivent le comportement antérieur à ce patch local.

Le helper de scope existant porte cette règle. `app_ecommerce_checkout_acquisition_context_get` partage la lecture de contexte entre widget et tunnel ; un contexte expiré conserve son refus jusqu’à relance explicite du parcours réseau. La preview construit le futur propriétaire depuis ce client chargé. Le signup accepte les affiliés non-sièges. Les résolveurs transmettent la durée illimitée explicitement (`0`) au plan d’exécution, pour éviter que `null` soit réinterprété comme la durée par défaut. Barème, volume, snapshots contractuels délégués, renouvellements et synchronisation Invoice inchangés.

Aucune mutation rétroactive des subscriptions, coupons, schedules, offres actives, commandes ou factures. [Rapport et recette](../../../notes/remises-2026-affilies-patch-2026-09-10.md).

<!-- AUTO-UPDATE:END id="remises-affilies-20260910-global-readme" -->

<!-- AUTO-UPDATE:BEGIN id="hub-remote-limited-20260910-canon-repos-global-readme" owner="codex" -->
...
<!-- AUTO-UPDATE:END id="hub-remote-limited-20260910-canon-repos-global-readme" -->

<!-- AUTO-UPDATE:BEGIN id="hub-empty-805-canon-repos-global-readme" owner="codex" -->

## Hub actif vide : réconciliation transparente — 09/09/2026

Le Hub conservé après suppression individuelle de sa dernière session est réutilisé automatiquement par SchedulePlan sans cible, s’il est l’unique racine du client/date/contexte/événement, sans membership actif, non deleting et admissible dans la fenêtre canonique. Exécution ouverte et commande Remote non QR en attente refusent ; historique terminé, joueurs, lots et branding sont conservés. DATE_OCCUPIED reste réservé aux conflits effectifs. Le writer reçoit le même Hub et son target_hub_id ; l’événement canonique est préservé avant prepare.

Verrou client/date partagé avec la suppression individuelle, puis verrou canonique de suppression Hub et relecture avant writer. Compensation fixed limitée aux brouillons prouvés par Pro ; une session configurée reste conservée en cas d’échec. L’Agenda n’affiche aucun conteneur vide. [Contrat, preuves, 52 contrôles et limites](../../../notes/hub-empty-reconciliation-2026-09-09.md). Local hub_soiree, non déployé.

<!-- AUTO-UPDATE:END id="hub-empty-805-canon-repos-global-readme" -->
...
## Update 2026-09-09 — Réconciliation PROD sur Hub

Le bootstrap `web/global_librairies.php` de Hub conserve le chargement PROD obligatoire de `ai_studio/automatisations/core/ai_studio_core_functions.php`, remplaçant la recherche optionnelle des anciens helpers transactionnels. Les require Hub et programmation sont maintenus ; les deux mécanismes AI Studio ne sont pas cumulés.

Application locale non commitée, non déployée ; [preuves](../../../tmp/hub-prod-code-2026-09-09/reconciliation/RECONCILIATION-PROD.md).

## Update 2026-09-08 — Ensure Hub : contrôle précis de status avant ALTER

`app_games_hub_schema_ensure()` lit la définition `games_hubs_players_sessions.status` par SHOW COLUMNS, puis compare l’ENUM exact (ordre/casse), NOT NULL, défaut created et Extra vide. Conforme : aucun ALTER, et aucun CREATE lorsque toutes les tables existent ; deux appels du même cycle restent à zéro DDL. Définition ancienne/incompatible complète : seul le MODIFY historique est conservé. Métadonnées absentes/incomplètes ou résultat false : aucun ALTER supposé. Le rôle des autres schema_ensure reste inchangé.

Correctif local limité à ce bloc ; [audit PROD et sonde](../../../tmp/hub-prod-initial-schema-2026-09-08/README.md). Dix scénarios mémoire, contre-épreuve ancien code et quatre contrats existants verts. PRECHECK PROD analysés : le paquet initial final comprend les quatorze colonnes tardives QR/stats dans ses CREATE complets, onze tables Hub et deux tables Quick Schedule associées, toutes nouvelles et InnoDB/utf8_general_ci. Aucun ALTER legacy ; blindtest_session_teams absente est exclue (mode équipe OFF et gardes de lecture/persistance). Le POSTCHECK mécanique valide 210 colonnes, 52 index et treize comptages exacts à zéro. Installation/relance/reprise et vrais helpers zéro DDL validés sur 10.11 isolé, pas une certification 10.3 ; import/POSTCHECK PROD opérateur restant. Les DDL opportunistes de Bibliothèque Pro identifiés par l’audit ne sont pas couverts par cette garantie du helper Hub. Aucun déploiement ni backfill.
...
## Etat 2026-09-08 — Provenance des exécutions de fin Canvas

Le helper de lecture seule `app_games_hub_execution_terminal_provenance_matches` vérifie le dernier start du Hub sous le token source : execution_id, runtime_mode, source_session_id, runtime_session_id et session_primary_id explicites, cohérents avec la session terminée. Le bridge Games l'utilise pour l'ouvert et la completion (start retrouvé par execution_id). Le resolver générique et le reader partagé des completions restent inchangés ; aucun filtrage global des démos, aucune réparation DB. Le writer de completion conserve l'INSERT IGNORE avec event_id stable.
...
Patch local, sans déploiement. `app_games_hub_remote_business_revisions_get` intègre le hash du branding effectif `app_games_hub_branding_get` dans `preparation_revision` existante ; un changement general_branding ou d’URL média versionnée déclenche ainsi le snapshot Remote même sans modification des sessions ou du Hub. Aucun nouveau champ de révision ni write, focus, génération de routage ou décision de navigation.

Le resolver partagé conserve ses fallbacks, notamment aucun logo Cotton ajouté ; le versionneur commun `v=filemtime` reste inchangé. La lecture canonique supplémentaire dans le contrôle Remote est à vérifier en recette DEV. Tests inter-repos Games `hub_branding_sync_test.php`/`.mjs` ; [audit et recette](../../../notes/hub-branding-sync-2026-09-07.md). Les interfaces historiques ne sont pas modifiées.

## Update 2026-09-07 — Hub : compteur confirmé et persistance du QR joueurs
...
`app_games_hub_players_count_active($hub_or_id, &$confirmed = null)` conserve son entier et la requête historique `COUNT(*)` des joueurs Hub `active`. La sortie optionnelle vaut true uniquement après lecture d’un champ `nb` numérique ; hub absent, échec SQL ou ligne absente = false. Aucun write, source de compteur ni statistique modifiés. Games expose `active_count_confirmed` et réserve au zéro confirmé le petit zéro d’en-tête du panneau Master permanent (grand compteur masqué à zéro) ; les autres appelants conservent leur comportement. Patch local, livraison Games/Global conjointe à prévoir.
...
`web/app/modules/jeux/hubs/app_games_hub_player_qr_functions.php`, chargé par `app_games_hubs_functions.php`, porte lecture/validation, initialisation, CAS, queue et confirmation. Le succès de `app_games_hub_session_launch_from_master` couvre création et reprise ; `app_games_hub_demo_representative_create` couvre la démo. Les requêtes QR sont comptées dans la mesure existante du contrôle. Contrat QR publié avant ce patch : non trouvé ; le contrat local documente cette nouvelle implémentation autorisée.
...
Complément DEV du 09/09/2026, non déployé : `Lancer un test` conserve la première officielle configurée des memberships actifs, tri SQL `date`, `heure_debut`, `position`, `id`. Aucune sélection du carrousel. Tous les cas compatibles suivent strictement la suggestion et la duplication existantes, y compris le remplacement de thématique déjà prévu.

Exception unique : source Quiz papier refusée par le garde numérique, offre active et transition `preflight_demo_launch`. `app_programming_quick_hub_quiz_demo_substitution_build` reprend le chargeur catalogue Cotton et `app_programming_quick_theme_plan_build`, sans second moteur. Les lots catalogue sont vérifiés avec le garde numérique canonique (au moins une question, aucune question incompatible). L'ordre du catalogue est conservé ; les lectures s'arrêtent dès que le nombre souhaité est atteint. Le nombre demandé vient de `hub_quiz_series_count`, sinon des tokens de la source, sinon du défaut numérique Quick. Si le nombre exact est impossible, le `allow_quiz_best_effort` existant utilise les lots compatibles disponibles ; aucun lot disponible produit un échec métier explicite. Pas d'ID produit codé en dur, pas d'historique consommé par le test, catalogue chargé avec origine Cotton.

La source officielle reste la référence d'exécution et d'intention. Le contenu substitué est contrôlé avant toute copie, puis à son application sur la démo ; les `lot_ids` choisis sont conservés sans ajout d'un pack automatique de taille différente. Le garde prospect/duplication ordinaire reste inchangé. Flags démo/numérique, deux joueurs, branding, reset, isolation et retry restent ceux du producteur existant. La substitution n'est pas une conversion du contenu papier officiel.


Correctif du 07/09, recette navigateur DEV validée après 14h40 sur H292 (CTA Master et Remote, démos Bingo 27702/27703, présentation officielle différente, maintien Player et retours). Aucun déploiement effectué par Codex. Comportement : `preflight_demo_launch` est reconnu par la validation canonique avec provenance de CTA annexe, Hub/source/runtime/exécution/intention/génération exacts et exécution démo ouverte. Le relais Master sérialise `runtime_session_id` ; la commande Remote le porte déjà et publie maintenant son type réel côté Games. Un type absent ou discordant reste refusé. Même avec un ancien marqueur ready, un contexte preflight non correspondant reste non joinable.

`source=ready` peut ainsi enregistrer `hub_remote_runtime_ready` pour les deux CTA. Le callback commandé `remote_launch_readiness` accepte `launch_hub_demo` uniquement via cette même validation et écriture idempotente, puis relit le marqueur ; absence après écriture = `READINESS_WRITE_FAILED`. Ni `completed` de commande, ni présence/heartbeat ne valent readiness. La voie officielle `launch_session`, la readiness new/recreated et la reprise existing restent distinctes. Test de chaîne inter-repos : Games `web/tests/hub_demo_readiness_flow_test.php` ; stockage SQL en mémoire, aucun accès DB. Voir [audit et recette](../../../notes/hub-launch-test-audit-2026-09-07.md), section « Correctif local autorisé ».
...
La primitive lit la première session officielle du Hub dans l’ordre canonique du Programme. Elle passe ensuite par le producteur Hub par intention `app_session_demo_create_from_source(...)`, distinct du resolver historique `app_session_demo_source_resolve_or_create(...)` utilisé par `session_duplicate` ; ils partagent la copie bas niveau `app_session_demo_source_sync(...)`, qui fournit une copie isolée avec token neuf, flags démo et branding source. Seule sa thématique est ensuite remplacée, sur la copie, par `app_programming_quick_hub_suggestion_build(...)` et `app_programming_quick_hub_apply_theme_to_session(...)` avec `force_numeric`.
...
Complément du 07/09 — inscription invitée publique Hub Play (local, non déployé) : Décision utilisateur du 07/09 appliquée : l’inscription invitée publique appelle `app_games_hub_player_register_guest_from_play`, qui réserve dans ce Hub tout pseudo normalisé déjà présent, y compris `left`. Après départ, même avec l’ancienne preuve, le pseudo est refusé (`USERNAME_TAKEN`) ; un autre pseudo libre crée une identité neuve sans historique/photo. Une identité encore active peut rejouer sa propre inscription sans mutation. Les réparations internes EP/papier/runtime gardent leur réactivation par identité ; aucune ligne existante n’est fusionnée ni supprimée. Le garde public utilise un verrou nommé par Hub et renvoie des erreurs explicites si la lecture ou le verrou échoue. Games exige ce nouveau helper : livraison des deux fichiers applicatifs ensemble. Les tests `web/tests/hub_guest_rejoin_test.php` couvrent contrat antérieur interne, nouvelle règle publique et handler Games. [Audit et preuves](../../../notes/hub-play-quit-rejoin-audit-2026-09-07.md).
...
Le helper d'upload Hub réutilise le traitement historique de l'image: stockage par type de jeu, contrôle de fichier, resize 900x900, qualité 90 et consentement historique. L'éligibilité est désormais prouvée en amont par Games depuis le rang 1 à 3 de l'identité active dans `aggregate_context.aggregate_ranking`, source canonique Global chargée côté serveur; aucun rang navigateur ni podium de session ne fait autorité. Global revalide Hub, identité et mapping de la session choisie comme support de consentement/traçabilité, puis applique les validations de fichier et l'écriture existantes. Depuis le correctif du 06/09, chaque tentative utilise un nom de fichier unique : l'ancien média ne partage plus le chemin du nouveau et un upload refusé ne peut pas retrouver l'ancien média comme résultat. Le fichier candidat doit exister et être reconnu comme image avant consentement/bascule. L'écriture de l'ID actif est conditionnée à l'ancien ID et son résultat est contrôlé ; échec SQL ou conflit renvoie `photo_persist_failed` et nettoie seulement le candidat. L'ancienne photo active est supprimée seulement après une bascule réussie ; une sortie ultérieure du Top 3 ne supprime pas la photo. Aucune copie n'est écrite dans les résultats de session.
...
Le resolver général expose désormais `app_general_branding_get_detail_merged(...)`: il ne s'arrête plus au premier branding trouvé, mais complète chaque champ manquant en priorité session, opération/Hub, réseau, compte. Le Hub explicite (`id_type_branding=5`, `id_ref=games_hubs.id` ou ancien `id_related`) précède l'opération événement, puis le réseau et le compte ; l'absence d'opération n'empêche pas l'héritage Hub.

`app_games_hub_sessions_get(...)` et `app_games_hub_get_for_session(...)` lisent exclusivement les memberships actifs persistés. Aucun backfill implicite ne suit une lecture sans relation ; le diagnostic exige un membership explicite. La réparation `app_games_hub_sessions_reconcile(...)` est séparée et refuse plusieurs candidats exacts sans score ni sélection par ID.

Pour l'identification de sa couche propre, le Hub suit le même niveau de granularité que le réseau: le réseau reçoit un `id_client_reseau`, cherche directement `general_branding.id_type_branding=3`, puis hydrate via `app_general_branding_get_complete(...)`; le Hub reçoit un `id_session`, résout légèrement `id_hub` via `games_hubs_sessions`, cherche directement `general_branding.id_type_branding=5 AND id_ref=id_hub`, puis utilise la même hydratation générique. La résolution effective complète ensuite les champs par `app_games_hub_branding_get(...)`, incluant la cascade session/événement/réseau/compte avant les defaults Cotton, sans normalisation des sessions.

L'endpoint `general/branding?action=get`, déjà utilisé par les sessions de jeu pour le branding réseau, passe maintenant par `app_session_branding_get_detail(...)`. `global_ajax.php` charge explicitement les helpers Hub et sessions branding nécessaires à ce chemin réel. La cascade Hub est activée pour les requêtes Canvas via `CanvasBrandingHydrator` ou, pour les appels navigateur de modale, `branding_context=game`. `branding_fetch_session_row(...)` inclut `date` pour les diagnostics de contexte ; aucun rattachement n'est créé par cette lecture.
...
`app_games_hub_prizes_get(...)` lit les lots Hub explicites, puis une compatibilité en lecture seule sur le programme canonique complet. Seul un consensus de toutes les sessions est partageable ; les divergences, y compris une unique session personnalisée, restent session-scoped. Aucun bootstrap n'est appelé par ce getter. Un Hub initialisé vide conserve les fallbacks génériques. Le helper de bootstrap explicite refuse les divergences et n'est pas une migration de lecture.
...
Cette résolution ne change pas les pages publiques événement legacy, mais le contexte Canvas jeu lit la cascade cible `branding_session -> branding_hub -> branding_evenement -> branding_reseau -> branding_client -> fallback Cotton`. Si aucune couche Hub n'existe, la cascade réseau puis compte puis fallback Cotton reste inchangée.
...
`app_games_hub_sessions_reconcile(...)` reste réservé à une réparation explicite des données historiques, jamais à leur simple lecture. Le quick-add ne l'utilise plus comme commande d'ajout et compense sa session si le thème ou le membership explicite échoue. Une projection `games_hubs_publication` seule ne bloque plus la libération d'une ancienne clé de contexte inactive.
