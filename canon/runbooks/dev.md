...
## Schema dev / canon
- Les exports schema dev doivent conserver `ecommerce_offres_to_clients.remise_pourcentage` en `DECIMAL(5,2) UNSIGNED`.
- Cette colonne est un snapshot contractuel de remise appliquee a l'offre client; elle ne doit pas revenir en `tinyint(3) unsigned`, sinon les remises decimales comme `28,61 %` sont arrondies en `29`.
- Si un export DB dev ou une resolution de merge modifie ce type, realigner ensemble `dev_cotton_global_0.sql`, `canon/data/schema/_sources/dev_cotton_global_0.sql` et `canon/data/schema/DDL.sql`.
- Le moteur facture runtime reste volontairement base sur `prix_reference_ht` + remise exacte; ne pas compenser une divergence de schema par un patch de calcul sans audit metier.
