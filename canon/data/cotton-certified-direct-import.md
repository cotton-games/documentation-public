...
L'import DB porte uniquement les champs detectes et valides : serie, questions, propositions, bonne reponse, explication et supports. Pour les supports de question de type `image`, l'importeur telecharge le fichier source, le valide, l'ecrit dans le stockage public Cotton, puis remplace `questions.lien_support` par l'URL Cotton locale. Les supports `audio`, `video` et YouTube ne sont pas telecharges : leurs URLs validees restent stockees comme liens supports. L'illustration generale de la serie est aussi telechargee et ecrite en `.jpg` dans le stockage applicatif attendu par la bibliotheque PRO, afin que la thematique affiche son visuel apres import.
...
- `image` attend une URL image HTTP(S) stable et directement affichable ; l'importeur telecharge l'image au moment de l'import DB, l'ecrit dans `/upload/quiz/images/questions/question/`, met a jour `questions.lien_support` avec l'URL Cotton locale et synchronise `medias_images` en best effort ;
