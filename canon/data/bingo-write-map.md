...
**25/09 — Robustesse locale non déployée.** Auth Player/Paper/Remote : lecture lifecycle sans restore ni publication E. Retry player_register/grid_assign/grid_hydrate conserve E/identité/event_id et vérifie l’admission ; aucun reset ajouté. Open/Abandon et propriété grille/secret inchangés. [Tests et dépendances de backport](../../notes/hub-execution-robustness-backport-2026-09-25.md).
...
## 24/09/2026 — Premier départ Hub-open, local non déployé

Open/Abandon ne déclenchent pas le reset. `game_events` stocke le slot unique Hub et `hub_first_start_accepted` atomiquement avec starting ; seule cette transaction est InnoDB. Sa clé stable sert de request_id au reset canonique Bingo (clé bingo-reset dérivée existante), avec replay/ACK identiques. Aucune modification grille/secret/propriétaire/stock/allocation pendant abandon. Le Hub garde la synchronisation de capacité. [Tests et limites](../../notes/hub-open-players-foundations-2026-09-24.md).
