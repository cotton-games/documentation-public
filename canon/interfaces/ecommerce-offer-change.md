<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-acquisition" owner="codex" -->

## Consommation de la jauge par Hub — 21/09/2026 (local)

Le lot [jauge Hub et upsell](../../notes/hub-capacity-upsell-2026-09-21.md) consomme l’offre effective déjà présente dans Cotton via `app_ecommerce_offre_effective_get_context()`. Il ne livre ni checkout d’upsell ni mutation Stripe. Admission Hub plafonnée aux actifs ; snapshots de session synchronisés vers le haut au lancement/reprise et au prochain contrôle serveur. Downsell hors périmètre, aucune éviction ni diminution de snapshot.


## Frontière avec le patch d’acquisition propre — 10/09/2026

Le patch local Remises2026 ouvre les **nouvelles acquisitions propres** aux affiliés non-sièges. Il ne livre pas le chantier de changement d’offre décrit ci-dessous et ne modifie pas ses exclusions. La reprise du paiement exige une offre pending sans subscription existante ; les pending legacy remisées sont refusées sans migration. Les subscriptions et schedules déjà existants ne sont pas modifiés. [Détails et validation](../../notes/remises-2026-affilies-patch-2026-09-10.md).

<!-- AUTO-UPDATE:END id="remises-affilies-20260910-acquisition" -->
