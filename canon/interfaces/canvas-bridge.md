...
## Endpoint
- `POST /GAMES/global_ajax.php?t=jeux&m=canvas`
...
- `action` (obligatoire): ex `session_update`, `bingo:case_click`, `bingo:end_game`, `phase_winner`
...
- **Symptôme “writes WS bloqués”** : si les writes Canvas idempotents (ex: `bingo.session_update`, `bingo.case_click`, `bingo.end_game`) renvoient 403, les effets DB associés ne se produisent pas → logique aval “bloquée” (ex: progression liée à `session_update` comme le compteur renvoyé `numPassedSongs` sur certains chemins).
...
```bash
curl -i -X POST 'https://games.dev.cotton-quiz.com/global_ajax.php?t=jeux&m=canvas' \
