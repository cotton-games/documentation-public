...
- **Remote papier Bingo `phase_winner`** : l'attribution gagnante avec joueur depuis la remote papier passe par le bridge HTTP avant le WS. Payload attendu: `game=bingo`, `action=phase_winner`, `sessionId`, `player_id` canonique ou `playerId` numerique, `phase`, `event_id`, `role=remote`, `remote_action=paper_phase_winner`, `paperMode=1`. Le WS `admin_phase_winner` porte ensuite `persisted=true`, `wonPhase`, `nextPhase` et `requiresResync=true` pour rafraichir les sockets connectees sans repersister.
