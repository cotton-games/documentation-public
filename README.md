...
- Source canonique : dépôt privé `cotton-games/documentation` ; point d’entrée : [START.md](START.md).
- API Contents : `https://api.github.com/repos/cotton-games/documentation/contents/START.md?ref=develop`, branche à choisir explicitement (`main` = référence publiée PROD avec exclusions ; `develop` = préparation).
- Parcours obligatoire avant le parcours thématique ci-dessus : `START.md` main → `SITEMAP.ndjson` → `README.md` → `DOCS_MANIFEST.md` → `HANDOFF.md` → pages ciblées par leurs chemins, sans supposition (`develop` pour la préparation).
- Codex utilise uniquement `COTTON_DOCS_TOKEN` pour les lectures documentaires GitHub, avec les trois headers décrits dans START. Aucune valeur de token dans les fichiers, logs ou réponses ; accès AI Studio indépendant.
- Preuve d’abord : URL API sans secret + chemin + branche + heading ; information absente = `non trouvé`.
- `documentation-public` et les URLs publiques des anciens comptes rendus sont **legacy / transition**. `SITEMAP.txt` et le champ NDJSON `url` restent compatibles avec le workflow ; les agents authentifiés utilisent `api_url`.
- Génération canonique : `npm run docs:sitemap` régénère les trois sitemaps et les index ; la mention historique « version publique » désigne seulement la copie miroir.
- Suivi de ce dépôt : [README](canon/repos/documentation/README.md), [TASKS](canon/repos/documentation/TASKS.md), [inventaire et recette](notes/documentation-private-access-2026-09-24.md).
