...
for (const resume of [false, true]) test(`promote with ${resume ? 'existing' : 'new'} conflicting merge`, t => {
  ...
  assert.match(index, /https:\/\/api\.github\.com\/repos\/cotton-games\/documentation\/contents\/canon\/topic\.md\?ref=main/);
  assert.ok(readFileSync(path.join(f.root, 'SITEMAP.md'), 'utf8').includes(f.source));
