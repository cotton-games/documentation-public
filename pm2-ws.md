...
**25/09 — Markers locaux robustesse : `restart 25-09-2026/05`.** Quiz/BT `web/server/restart_serveur.txt`, Bingo `version.txt`. Aucun restart ni déploiement. Livrer ultérieurement un sous-lot cohérent, après autorisation ; ne pas copier les moteurs pregame complets vers main. [Deltas nécessaires, adaptations et rollback](notes/hub-execution-robustness-backport-2026-09-25.md).

**25/09 — Markers locaux préparés : `restart 25-09-2026/04` sur Quiz/Blindtest/Bingo.** ACK au secondary Remote courant et relecture STATUS du même E accepté ; aucun nouveau START/primary. Instrumentation /03 conservée, décision5/10/5/90 inchangée. Aucun restart exécuté ; recette réelle et traces requises après livraison autorisée. Note V2 AG.


**25/09 — Diagnostics de convergence : préparation locale uniquement.** Quiz `web/server/restart_serveur.txt`, Blind Test même chemin et Bingo `version.txt` = `restart 25-09-2026/03`. Aucun restart exécuté. Livrer contrôleur et hook logger de chaque jeu avec le bridge Games et la projection Global ; auto-start historique conservé. [Liste exacte / rollback du delta](notes/hub-open-players-foundations-2026-09-24.md#af-hub-native--audit-du-delta-et-instrumentation-préalable-start--25092026).


Auto-start pregame local24/09 : contrôleurs Quiz/BT/Bingo modifiés ; markers quiz/web/server/restart_serveur.txt, blindtest/web/server/restart_serveur.txt et bingo.game/version.txt = restart 24-09-2026/04. Aucun restart/déploiement exécuté. Livraison et prise en compte WS ultérieures par opérateur ; section T de la note V2.


## 24/09/2026 — Protocole prégame local, non déployé

Nouveaux modules requis : Quiz/BT `web/server/actions/hubPregame.js`, Bingo `ws/hub_pregame.js`, avec leurs hooks serveur et bridge Global/Games. Trames de diagnostic `HUB_PREGAME_STATE`, `HUB_PREGAME_START_ACK`, `HUB_PREGAME_ERROR`, `HUB_PREGAME_ABORTED`. Ne pas confondre ready navigateur et preuve d’ouverture. [Lot et qualification](notes/hub-open-players-foundations-2026-09-24.md). Aucune commande PM2 exécutée.
