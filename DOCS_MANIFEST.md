...
Évolution du contrat d’exécution Codex DEV (`/home/romain/Cotton/AGENTS.md`, capacités approuvées de `tools/dev/deploy-dev.sh`, `tools/dev/fetch-dev.sh`, `tools/e2e` et états d’authentification hors Git) → `canon/runbooks/dev.md` (bloc `dev-steps`, contrat central), `canon/runbooks/security.md` (`security-rules`), contraintes de `START.md`, `README.md` (`docs-index`), README/TASKS `canon/repos/documentation/` et `HANDOFF.md` ; régénérer les trois sitemaps et les INDEX via `DOCS_BRANCH=develop npm run docs:sitemap`. Cette règle documente les capacités ; elle n’autorise aucune modification des wrappers, des secrets ou d’AGENTS.md. Aucune nouvelle page dédiée nécessaire. README/TASKS applicatifs seulement si ces dépôts sont réellement modifiés ; aucune livraison produit ni entrée CHANGELOG produit déduite de cette mise à jour documentaire.


Patch de robustesse autorité/retries/probes Hub (registration/lifecycle/dispatch des trois moteurs, register/play-ws, api_client, attentes WS/readiness et boot_lib) → `notes/hub-execution-robustness-backport-2026-09-25.md`, complément audit historique, README/TASKS des repos modifiés, TASKS Global si comparaison seule, interfaces actions/canvas-bridge, bingo-write-map, entrypoints/runbook DEV, markers/pm2-ws, HANDOFF/CHANGELOG ; index/sitemap par générateur. Comparer réellement main/develop/branche de travail ; distinguer applicabilité du diff, dépendances et recette PROD. Ne pas porter main sans accord explicite.

Audit d’autorité Hub inter-moteurs sans patch (`notes/hub-execution-authority-audit-2026-09-25.md`, lectures des snapshots `hub_soiree` Games/Global/Bingo/Quiz/BT) → rapport historique dans `notes/`, TASKS des cinq dépôts et HANDOFF ; sitemap/index par générateur. Distinguer E persistée, objet WS provisoire, autorité locale et bind ; contrat demandé comme cible d’audit, jamais comme comportement canonique déjà livré. Aucun correctif applicatif avant validation du rapport.

Contrat d’accès documentaire privé / générateur (`START.md`, `README.md`, `DOCS_MANIFEST.md`, `scripts/gen-sitemap.mjs`, `scripts/gen-sitemap.test.mjs`, `scripts/promote-docs.test.mjs`) → START, bloc `docs-index` du README, manifest, README/TASKS `canon/repos/documentation/`, `HANDOFF.md`, `notes/documentation-private-access-2026-09-24.md` ; régénérer sitemap/index. Ne pas modifier le workflow de mirroring ; conserver les champs/URLs legacy requis par ses contrôles. Inventaire global des anciennes références autorisé uniquement pour cette migration ; les autres tâches suivent le routing ciblé.


Open Players V2 (`**/hub*pregame*`, `**/hub_embed*`, `**/app_hub_embed_native.php`, `scripts/diagnostics/hub-pregame-waves.py`, `scripts/diagnostics/test_hub_pregame_waves.py`, guards Global, Organizer/Player/Remote et WS Quiz/BT/Bingo) → `notes/hub-open-players-foundations-2026-09-24.md`, README/TASKS des cinq dépôts, interfaces canvas-bridge/actions, bingo-write-map, entrypoints/runbook DEV/markers, HANDOFF/CHANGELOG ; index/sitemap par générateur. Distinguer raccordement local, preuves simulées et recette DEV ; aucune qualification navigateur ni livraison déduite des seuls tests offline.
...
0ter) Après toute modification de documentation : exécuter `npm run docs:sitemap` pour régénérer les trois `SITEMAP.*` et les `INDEX.md` (source privée canonique ; copie publique legacy / transition).
0quater) La source canonique est `cotton-games/documentation` : `START.md` → `SITEMAP.ndjson` → `DOCS_MANIFEST.md` → carte repo/pages ciblées, via API Contents authentifiée sur une branche explicitement choisie. Codex utilise `COTTON_DOCS_TOKEN` uniquement pour ces lectures GitHub ; headers et règles de confidentialité dans START. Le token AI Studio est indépendant.
...
- Start from `START.md` in canonical private `cotton-games/documentation`, using authenticated GitHub Contents API; navigate by `path` / `api_url` in `SITEMAP.ndjson`, on an explicit branch. Public mirror references are **legacy / transition**. Cite the exact API URL (no secret), path, branch and heading; missing information: `non trouvé`.
