# Smoke tests — Canvas API (`GAMES/global_ajax.php?t=jeux&m=canvas`)
...
## g) Bingo `case_click` (write)
```bash
curl -i -X POST 'https://games.dev.cotton-quiz.com/global_ajax.php?t=jeux&m=canvas' \
  -H "X-Service-Token: ${CANVAS_SERVICE_TOKEN}" \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  --data 'game=bingo&action=bingo:case_click&sessionId=SESSION_TOKEN&playerId=PLAYER_ID&gridId=GRID_ID&num_case=1&clicked=1&event_id=00000000-0000-0000-0000-000000000004'
```

### 2e appel (same event_id)
```bash
curl -i -X POST 'https://games.dev.cotton-quiz.com/global_ajax.php?t=jeux&m=canvas' \
  -H "X-Service-Token: ${CANVAS_SERVICE_TOKEN}" \
  -H 'Content-Type: application/x-www-form-urlencoded' \
