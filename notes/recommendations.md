...
## 8) E-commerce ABN: recommandation future sur les abonnements déjà actifs

### 8.1 Besoin métier à garder en backlog
Pouvoir créer, depuis le BO, une remise ciblée qui s'applique non pas au prochain checkout ABN, mais aux **prochaines échéances** d'abonnements déjà actifs sur une période donnée.

Cas d'usage visés :
- gratification ponctuelle de certains clients ABN actifs
- campagne d'upsell / renouvellement / réengagement
- application pendant `N mois` sur un portefeuille déjà abonné

### 8.2 Constat d'architecture actuelle
Le moteur `Remises 2026` livré en V1 agit aujourd'hui au moment du **checkout abonnement** :
- résolution de la remise avant `Checkout Session`
- orchestration éventuelle de `SubscriptionSchedule` après `checkout.session.completed`
- rattachement SI principal sur la `subscription Stripe`

Ce moteur n'agit pas directement sur une `subscription` Stripe déjà existante hors nouveau checkout.

### 8.3 Recommandation d'architecture
Si ce besoin est traité plus tard, le porter comme un **moteur distinct** de celui du checkout initial.

Principe recommandé :
- cible d'entrée = `subscription Stripe` déjà active dans `ecommerce_offres_to_clients`
- remise sans limite :
  - privilégier un flux simple de type coupon / discount durable
- remise limitée mensuelle :
  - privilégier un `SubscriptionSchedule` appliqué à partir de la subscription existante
- annuel avec durée `< 12 mois` :
  - ne jamais chercher un phasage intra-annuel complexe
  - viser uniquement une remise sur la prochaine échéance annuelle

### 8.4 Garde-fous à prévoir
- idempotence stricte par abonnement ciblé
- journal d'exécution de campagne
- détection d'un `schedule` déjà attaché à une subscription
- aucune mise à jour directe incompatible sur une subscription déjà pilotée par `schedule`
- séparation claire entre :
  - remises au checkout d'acquisition
  - remises appliquées à des abonnements déjà actifs

### 8.5 Niveau de complexité attendu
Ce n'est pas un micro-patch :
- difficulté surtout sur l'idempotence, la coexistence avec l'existant et la reprise d'état Stripe
- le principe reste toutefois compatible avec l'architecture actuelle, qui garde déjà la `subscription Stripe` comme référence principale côté SI
