...
**WS (Node)** → **Canvas API (PHP `global_ajax.php?t=jeux&m=canvas`)** → **DB (SQL)**, avec **PRO** pour le rôle organisateur.
...
- Pratique : quand une relance du serveur WS Bingo est nécessaire, bump `bingo-musical/version.txt` (format `restart DD-MM-YYYY/NN`).
