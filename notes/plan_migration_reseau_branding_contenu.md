...
Le contrat canonique est désormais `canon/interfaces/ecommerce-offer-change.md`: Cotton porte l'intention commerciale, Stripe exécute, puis Cotton réingère la réalité confirmée. La V1 est strictement ascendante sur jauge/périodicité et exclut les offres déléguées hors cadre. Le Customer Portal Stripe n'est pas le parcours de changement moderne.

## Update 2026-09-04 - Période courante des subscriptions Stripe en PROD
...
- La période historique de facture vient des Invoice Lines récurrentes de la Subscription et non de `stripe_current_period_start/end`.
...
- La migration PROD à deux colonnes a été confirmée avant code; Global, Pro et www ont été reportés sur `main` et validés.


## Update 2026-09-03 - Renewal Stripe hors cadre reporté sur main

- Les nouvelles subscriptions déléguées hors cadre figent l'assiette HT, l'offre, la jauge, la fréquence, l'offre client et une référence dans `cotton_contract_*`.
- `invoice.upcoming` recalcule seulement la remise réseau courante et écrit le Price net si nécessaire; `invoice.created` et `invoice.paid` contrôlent sans repricing.
- Le support Product vient d'un Price catalogue actif et unique résolu par lookup depuis le snapshot; Product actif obligatoire, montant catalogue ignoré, absence/incompatibilité fail-closed.
- Le snapshot reste stable entre intentions commerciales explicites et peut être remplacé plus tard par une future intention Stripe; un renewal ne le modifie jamais.
- Le code est reporté directement sur `main` sans merge. Activation PROD de `invoice.upcoming`/`invoice.created` et normalisation des trois subscriptions legacy restent des opérations manuelles séparées après déploiement.
