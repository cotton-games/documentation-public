<!-- AUTO-UPDATE:BEGIN id="remises-affilies-20260910-network" owner="codex" -->

## Acquisition propre et remise réseau — patch local du 10/09/2026
...
Le coupon automatique legacy5%1774/1294 est abandonné pour les nouvelles commandes, ainsi que le bandeau et la promesse de doublement V&B. La délégation conserve la chaîne catalogue → pricing réseau → net Cotton → `price_data` Stripe, sans coupon supplémentaire. Les subscriptions historiques restent inchangées.

Une ancienne pending marquée par le coupon legacy ou une remise propre non rattachée à une règle est bloquée au paiement, sans réécriture. Une nouvelle commande doit être préparée explicitement ; aucune régularisation d’ancienne session Stripe déjà créée n’est entreprise.

[Rapport du patch, tests et limites de recette](remises-2026-affilies-patch-2026-09-10.md).

<!-- AUTO-UPDATE:END id="remises-affilies-20260910-network" -->
...
## Futur chantier - changement d'offre des clients en propre

Le contrat canonique est désormais `canon/interfaces/ecommerce-offer-change.md`: Cotton porte l'intention commerciale, Stripe exécute, puis Cotton réingère la réalité confirmée. La V1 est strictement ascendante sur jauge/périodicité et exclut les offres déléguées hors cadre. Le Customer Portal Stripe n'est pas le parcours de changement moderne; l'hypothèse d'une rupture historique de contrat doit être confirmée par audit avant implémentation.

## Update 2026-09-03 - Période courante des subscriptions Stripe

- Les bornes courantes Stripe sont stockées séparément sur l'offre client; elles ne remplacent ni `date_debut` ni `date_fin`.
- Le résolveur commun privilégie les items Basil, accepte des items multiples seulement s'ils portent la même période et conserve les propriétés racine legacy en compatibilité.
- Les webhooks création/mise à jour et `invoice.paid` assurent la synchronisation idempotente. Les vues Pro/BO utilisent la valeur persistée; le fallback legacy des offres Stripe non encore alimentées est transitoire et observable.
- La période courante d'offre et la période historique de facture sont désormais séparées: `invoice.paid` résout cette dernière depuis toutes les Invoice Lines récurrentes de la Subscription, accepte uniquement une période unique et transmet le résultat au snapshot `ecommerce_commandes_lignes.nom`. Une divergence de prorata est explicitement ambiguë; le fallback est Cotton legacy journalisé et jamais `stripe_current_period_start/end`.
- La resynchronisation unitaire et le backfill manuel de 25 offres ont servi uniquement à initialiser l'existant. Après convergence, leurs surfaces BO, endpoints et helpers dédiés sont retirés.
- Le contrat permanent repose sur la synchronisation webhook Stripe vers Cotton. Un éventuel futur outil de réparation devra réutiliser les helpers canoniques sans être exposé en permanence dans le BO.

## Update 2026-09-03 - Révision commerciale et consommateurs BO

- Une variation de remise réseau au renewal révise la même offre client hors cadre : `prix_reference_ht` conserve l’assiette HT, `prix_ht` et `remise_*` deviennent la vérité de la nouvelle période. Une remise inchangée reste un no-op commercial.
- Pro reste l’unique producteur utilisateur du couple subscription Stripe / offre Cotton. Le BO lit l’offre effective et son snapshot; il ne reconstruit pas le prix depuis le catalogue et ne peut ni attribuer ni terminer localement une offre hors cadre.
- `invoice.upcoming` est l’unique writer du Price (`proration_behavior=none`). Les lectures BO/Pro, le refresh de facturation, `invoice.created` et `invoice.paid` ne repricent pas.
- Un write `invoice.upcoming` refusé transitoirement par Stripe (429, réseau/timeout, 5xx) n’est pas acquitté : Stripe peut redélivrer l’événement. Le retry retrouve le même `id_offre_client`; si le premier appel a finalement été commité côté Stripe, la redelivery converge par no-op puis aligne Cotton.
- Une résiliation en fin de période conserve l’offre et la subscription actives jusqu’à échéance. La désaffiliation échoue fermée tant que l’une des deux reste active ou que Stripe ne peut pas confirmer un état terminal.
- Le fallback catalogue n’existe que pour une ancienne ligne sans `prix_reference_ht`; il est exposé par `contract_base_source=legacy_catalog_fallback` et compté pour permettre sa réparation.

## Update 2026-09-02 - Renewal Stripe hors cadre sur assiette contractuelle

- Les nouvelles subscriptions déléguées hors cadre figent dans leurs metadata Stripe l'assiette brute HT, l'offre, la jauge, la fréquence et une référence contractuelle.
- `invoice.upcoming` recalcule uniquement la remise réseau courante et met à jour le Price net avec `proration_behavior=none`; `invoice.created` et `invoice.paid` contrôlent sans write du Price.
- Une référence absente ou une couverture autre que l'offre hors-cadre exacte échoue fermée. Aucun fallback catalogue, coupon réseau nouveau ou migration de coupon legacy n'est introduit.
- Le Product support est résolu depuis `cotton_contract_offer_id/gauge_id/frequency_id` via un lookup `ABN{jauge}M|A`; le Price catalogue doit être actif, unique et compatible, et son Product actif. Son montant est ignoré: l'offre, la jauge, la fréquence et le tarif Cotton courants ne peuvent pas modifier l'assiette du renewal.
- La commande Cotton hors-cadre issue de `invoice.paid` reprend le montant payé et les taxes de l'Invoice Stripe.
- Reste à recetter en Stripe TEST par redelivery de `invoice.upcoming` sur `2708`, puis à reporter sur `main`; les trois subscriptions legacy restent une opération manuelle séparée.

### Évolution future du snapshot contractuel

- Un renewal ne modifie jamais le snapshot `cotton_contract_*` courant.
- Une future intention commerciale explicite initiée ou validée via Stripe pourra remplacer atomiquement ce snapshot; les renewals suivants utiliseront la nouvelle version.
- L'état Cotton courant ne constitue jamais à lui seul une intention commerciale.
- `checkout_v1` provenance seulement le snapshot initial. Le reader accepte une autre référence non vide pour rester compatible avec une future version, sans que le workflow de remplacement soit implémenté aujourd'hui.

## Update 2026-05-14 - BO contrats reseau: hors cadre sans abonnement actif
- le BO `reseau_contrats` distingue maintenant explicitement trois actions:
  - attribution hors cadre;
  - terminaison ciblee d'une offre hors cadre;
  - suppression de l'affiliation reseau;
- l'attribution hors cadre ne depend plus uniquement d'un `id_activation` pour afficher le succes: une creation d'offre deleguee valide suffit dans le cas `force_hors_cadre`;
- la terminaison hors cadre cible un `id_offre_client` et refuse les offres propres, les offres cadre/incluses, les offres non courantes et les offres d'un autre siege/affilie;
- l'absence d'abonnement reseau actif ne bloque pas l'attribution hors cadre.

## Update 2026-05-06 — Affiliation TdR depuis signin compte existant
- le cas `lien affiliation TdR -> signup -> signin -> compte existant` est maintenant explicite cote PRO;
- le lien `/utm/reseau/{seo_slug}` renseigne `$_SESSION['id_client_reseau']` via `pro/web/ec/ec_sign.php`;
- apres connexion reussie, `pro/web/ec/modules/compte/authentification/ec_authentification_script.php` consomme ce contexte et appelle `app_ecommerce_reseau_affilier_client(..., 'signup_affiliation')`;
- les invariants reseau restent portes par le helper central:
  - affiliation distincte de l'acces effectif;
  - offre propre active prioritaire;
  - creation incluse `cadre` uniquement si support reseau actif, offre cible et quota disponible;
  - pas de creation `hors_cadre` automatique depuis ce parcours;
  - pas de remise/repricing reseau sur une offre propre affiliee;
  - sans support actif, cible ou quota, rattachement seul;
- un compte deja rattache a une autre TdR n'est pas reaffecte automatiquement par signin;
- addendum produit valide: ce blocage n'est pas silencieux, le shell PRO affiche un flash explicite sans exposer le nom de l'autre reseau.

## Update 2026-05-06 — Stripe ABN direct: recalcul pipeline apres cloture effective
- hors perimetre reseau, la cloture terminale Stripe d'une offre ABN directe recalcule maintenant le pipeline client depuis l'acces effectif;
- la logique support reseau et delegations conserve les synchronisations existantes;
- `cancel_at_period_end` reste une programmation de fin de periode et ne declassifie pas le client avant la cloture effective Stripe.

## Update 2026-05-11 - Abonnement reseau: fin de cadre a date_fin
- la `date_fin` de l'offre support `Abonnement reseau` est inclusive: le support reste couvert jusqu'au jour indique, puis le cron le cloture a partir du lendemain;
- la cloture automatique passe le support en `Terminee`, archive le runtime reseau et ferme les offres incluses actives rattachees par `reseau_id_offre_client_support_source`;
- les offres deleguees incluses portent la meme `date_fin` que le support lorsqu'elle est renseignee;
- cette regle ne s'applique pas aux offres propres des affilies ni aux offres hors cadre;
- si la date de fin support est supprimee, la date des incluses n'est pas videe automatiquement sans arbitrage supplementaire.
...
- ajustement UX affilié:
  - l'entree bibliothèque sans jeu peut maintenant afficher un bloc `Jeux sélectionnés` qui ouvre un hub global lecture seule tous jeux confondus quand du contenu existe;
- ajustement UX complementaire TdR:
  - la TdR voit elle aussi ce bloc `Jeux sélectionnés` depuis le portail bibliothèque, pour accéder directement a la page de gestion reseau;
...
- l'etat canonique final du lot est maintenant clarifié:
  - entrée TdR via `/account/network`, puis `Jeux sélectionnés` vers `library?network_manage=1`;
  - entrée affilié via la carte portail bibliothèque `Jeux sélectionnés`, en lecture seule;
...
- etapes `4 / 5 / 6` volontairement non ouvertes a ce stade.
# Addendum 2026-05-11 - LP reseau contextualisee par abonnement support

- un distributeur partenaire reste modelise comme une TdR; la page publique principale est la LP reseau `/lp/reseau/{slug}`;
- une operation commerciale ponctuelle est contextualisee en V1 par l'abonnement reseau support, pas par `operations_evenements`;
- la personnalisation LP est stockee dans `ecommerce_reseau_support_lp_settings`, rattachee a `ecommerce_offres_to_clients.id`, pour eviter d'alourdir le support commercial canonique;
- la regle V1 en cas de doublons est explicite: abonnement reseau actif le plus recent (`date_debut DESC`, puis `id DESC`);
- sans abonnement reseau actif, la LP reste une page d'affiliation reseau et ne promet pas d'acces Cotton inclus;
- l'affiliation et l'activation de l'offre incluse restent les flux reseau existants.
