...
## Précondition de cohérence des statuts

Avant promotion, consulter [l’état de livraison](../canon/deployment-status.md). Au23/09/2026 : `hub_soiree` livré selon confirmation opérateur ; **`sas_players`, `hub_session_readiness` et `cast` non déployés en PROD**. La copie des sources develop vers main conserve ces travaux documentés et leurs exclusions. Elle ne les rend pas disponibles en production. Mettre à jour les statuts lors d’un déploiement explicite ultérieur, sans convertir automatiquement tous les anciens « non déployé » en « déployé ».
